module pg_diag
! Parallel solution of the Generalized Eigenproblem
! Real, symmetric Hamiltonian, Positive definitive Overlap matrix
! Time-stamp: "2008-10-29 08:02:27 cjn"
  use precisn, only: wp
...
! control the distribution, diagonalization and output
    ...
    use rd_oz, only: read_dcmat, hmat, desch, b, descb, z, descz
...
! parallel solution of generalized eigenproblem....
    write (*,*) myrow, mycol, ' calling pdsygvd, n, np, nq = ', n, np, nq
    call PDSYGVD (n, 1, 1, desch, hmat, 1, 1, descb, b, &
         1, 1, descz, z, np, nq, eval)
    write (*,*) myrow, mycol, ' pdsygvd done'
    call BLACS_BARRIER (ctxt, 'all')    ! precautionary
...
! form blacs descriptor for vector x:
    ...
    nn = MAX(1, INT(lwork / mm))  ! normally 1
    mb = mm;     nb = nn
    rsrc = p0;   csrc = q0
    ldx = MAX(1, mm)
    ...
    write (*,*) 'eigendata written'
  end subroutine write_evec

!!$  subroutine norm_evecs (n,
!!$    use blacs, only: nb_, csrc_, mycol
!!$    use rd_oz, only: descz
!!$    integer, intent(in)     :: n   ! matrix dimension
!!$    integer                 :: ii, iproc
!!$    logical                 :: ismycol
!!$    real(wp)                :: alpha
!!$
!!$    evecs: do ii = 1, n ! normalize eigenvectors
...
!!$       ismycol = (iproc == mycol)
!!$       if (ismycol) then
!!$          call pdnrm2 (n, dnorm2, z, 1, ii, descz, 1)
!!$          alpha = 1.0_wp / REAL(dnorm2,wp)
!!$          call pdscal (n, alpha, z, 1, ii, descz, 1)
!!$       end if
!!$    end do evecs
!!$  end subroutine norm_evecs
end module pg_diag
