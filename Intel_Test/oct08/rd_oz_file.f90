module rd_oz
! Time-stamp: "2008-10-29 07:06:50 cjn"
...
! mhm = dim of interaction matrix (before deleting B-splines)
       if (allocated(ns2)) then
          ...
          if (status /= 0) call p_error (status,'read_dcmat: deallocation')
       end if
       ...
       if (status /= 0) call p_error (status,'read_dcmat: ns2 allocation')
...
! iprm = pointer to deleted B-splines
       if (allocated(iprm)) then
          ...
          if (status /= 0) call p_error (status,'read_dcmat: iprm deallocation')
       end if
       allocate (iprm(mhm), stat=status)
       if (status /= 0) call p_error (status,'read_dcmat: iprm allocation')
       read (f) iprm(1:mhm)
...
! ishift, jshift = block offsets
       if (ioprocessor) then
          ...
          call igebr2d (ctxt, 'ALL', ' ', 6, 1, parms, 6, irread, icread)
