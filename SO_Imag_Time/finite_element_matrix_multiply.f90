...
!                    General Code
!
         CALL v_m_v_gen(v(locate:ni,1:nj),                    &
                        v_scr(locate:ni,1:nj),                &
...
!
        CALL v_m_v_10(v(locate:ni,1:nj),                     &
                      v_scr(locate:ni,1:nj),                 &
                      mat_reg_d(i,index)%ke_mat_d,           &
                      ni,nj)
    END IF
...
!                        General Case
!
          CALL v_v_m_gen(v(1:ni,locate:nj),                     &
                         v_scr(1:ni,locate:nj),                 &
                         mat_reg_d(i,index)%ke_mat_d,           &
                         ni,nj,nfun_reg(i,index)) 
!
     ELSE IF (nfun_reg(i,index) == 2) then
...
!                       Ten by Ten
!
          CALL v_v_m_10(v(1:ni,locate:nj),                      &
                        v_scr(1:ni,locate:nj),                  &
                        mat_reg_d(i,index)%ke_mat_d,            &
                        ni,nj)
     END IF
