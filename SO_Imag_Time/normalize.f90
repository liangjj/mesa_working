...
!***end prologue       check_norm_2d_d
  SUBROUTINE check_norm_2d_d(v,norm,g_1)
  USE dvrprop_global_it
  USE dvr_shared,           ONLY  : nphy
  IMPLICIT NONE
...
!
  norm = 0.d0
  DO i=1,nphy(1)
     g_1(i)  =    ddot(nphy(2),v(1,i),1,v(1,i),1)      
  END DO
  DO i=1,nphy(1)
     norm = norm + g_1(i)
  END DO
...
!***end prologue       check_norm_3d_d
  SUBROUTINE check_norm_3d_d(v,norm,g_1,g_2)
  USE dvrprop_global_it
  USE dvr_shared,           ONLY  : nphy
  IMPLICIT NONE
...
!
  ...
  DO i=1,nphy(1)
     DO j=1,nphy(2)
        g_2(j,i) =  ddot(nphy(3),v(1,j,i),1,v(1,j,i),1)                
     END DO
  END DO
  g_1(:) = 0.d0
  DO i=1,nphy(1)
...
!**end prologue       rad_packet
  ...
  IF(typ_pak == 'exponential') then
     WRITE(iout,2)
     wave_function = ( grid(1)%pt - x_0(1) )**powr                        &
                                  *                                       &
  ...
  END IF
  WRITE(iout,4)
  WRITE(iout,5) sigma(1), x_0(1), powr
