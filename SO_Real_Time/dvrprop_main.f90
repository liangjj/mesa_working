...
!
  IF(spdim == 2) THEN
     ...
     ALLOCATE(fac(words(1)))
  ELSE IF(spdim == 3) then
     maxdim=max(nphy(3),nphy(2),nphy(1))
     words(1)=max(nphy(2)*nphy(1)*4,maxdim*maxdim*2)
     words(2)=max(nphy(1)*4,maxdim*2)
...
!
  ...
  DO  i=1,spdim
      n3d=n3d*nphy(i)
  END DO
  ALLOCATE(psi(n3d,2),v_scr(n3d,2),v_tot(n3d),           &
           sin_diag(n3d),cos_diag(n3d),tim_pts(ntreg+1))
  CALL so_prop
!
! Deallocate all arrays that have been allocated for the entire
! calculation.
! 
  DEALLOCATE(psi,v_scr,v_tot,sin_diag,cos_diag,tim_pts)
  IF(spdim == 2) THEN
     DEALLOCATE(fac)
  ELSE IF(spdim == 3) then
     DEALLOCATE(fac,v_1)
