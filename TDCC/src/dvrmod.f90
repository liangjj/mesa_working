MODULE dvrmod
  ! contains dvr subroutines which do not depend on other modules
contains
  ! ------------------------------------------------------------------------
  subroutine DVR_reg(nfun,xbounds,pt,wt,df,eddf,ke_mat)
    ...
    real*8, dimension(1:2), INTENT(IN)  :: xbounds
    real*8, dimension(1:nfun), INTENT(OUT) :: pt 
    real*8, dimension(1:nfun), INTENT(OUT) :: wt    
    real*8, dimension(1:nfun,1:nfun), INTENT(OUT) :: ke_mat, df, eddf

    integer :: i,j,k
    real*8, dimension(1:2) :: endpoints
    real*8 :: xmin, xmax
    real*8, dimension(1:nfun)         :: scratch  
    real*8, dimension(1:nfun)         :: gauss_weights
    real*8, dimension(1:nfun)         :: roots
    real*8, dimension(1:nfun,1:nfun)  :: f, ddf
    real*8 :: A,B
...
1001 RETURN
  ...
  ! Code converted using TO_F90 by Alan Miller
  ! Date: 2001-06-21  Time: 12:07:39

  SUBROUTINE prntfm(title,a,n,m,ia,ja,iout)

    IMPLICIT INTEGER (a-z)
    CHARACTER (LEN=80), INTENT(IN OUT)       :: title
    REAL *8, INTENT(IN OUT)                  :: a(ia,ja)
    INTEGER, INTENT(IN)                      :: n
    INTEGER, INTENT(IN)                      :: m
