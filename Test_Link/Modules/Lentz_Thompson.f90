...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!*deck Continued_Fraction_PI
!***begin prologue     Continued_Fraction_PI
!***date written       100206   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords           continued fractions
!***author             schneider, barry (NSF)
...
!***references         none
!***routines called
!***end prologue       Continued_Fraction_PI
  Subroutine Continued_Fraction_PI(CFPI) 
  IMPLICIT NONE
  TYPE(CF_PI)                      :: CFPI
  REAL(idp)                        :: a 
  REAL(idp)                        :: b 
...
!
  ...
  DO While (test > eps )              
     ...
     IF ( iwrite == 50 ) THEN
          iwrite = 0
          Write(iout,2) count, f + three, C, D, test, a, b, n_0
     END IF
  END DO
  f = f + three
  Write(iout,3) f
...
2 Format(5x, 'Loop Values',5x,'Count = ',i5/,10x,                       &
         'f    = ',d20.12,1x,'C = ',d15.8,1x,'D = ',d15.8,/,10x,        &
         'test = ',d15.8,1x,'a  = ',d15.8,1x,'b = ',d15.8,1x,'n_0 = ',i10)
3 Format(15x,'Final Value of Continued Fraction = ',d20.12)
END SUBROUTINE Continued_Fraction_PI
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!*deck Continued_Fraction_e
!***begin prologue     Continued_Fraction_e
!***date written       100206   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords           continued fractions
!***author             schneider, barry (NSF)
...
!***references         none
!***routines called
!***end prologue       Continued_Fraction_e
  Subroutine Continued_Fraction_e(CFe) 
  IMPLICIT NONE
  TYPE(CF_e)                       :: CFe
  REAL(idp)                        :: a 
  REAL(idp)                        :: b 
  ...
  REAL(idp)                        :: Del        
  REAL(idp)                        :: test
  REAL(idp)                        :: e=2.718281828459045235360287d+00
  INTEGER                          :: count         
  INTEGER                          :: iwrite         
!
  f = one
  ...
  DO While (test > eps )              
     ...
     IF ( iwrite == 5 ) THEN
          iwrite = 0
          Write(iout,2) count, f + int_one, C, D, test, a, b
     END IF
  END DO
  f = f + int_one
  Write(iout,3) f
1 Format(5x, 'Initial Values',/,10x,                                    &
         'f    = ',d20.12,1x,'C = ',d15.8,1x,'D = ',d15.8,/,10x,        &
         'test = ',d15.8,1x,'a  = ',d15.8,1x,'b = ',d15.8,1x)
2 Format(5x, 'Loop Values',5x,'Count = ',i5/,10x,                       &
         'f    = ',d20.12,1x,'C = ',d15.8,1x,'D = ',d15.8,/,10x,        &
         'test = ',d15.8,1x,'a  = ',d15.8,1x,'b = ',d15.8,1x,'n_0 = ',i10)
3 Format(15x,'Final Value of Continued Fraction = ',d20.12)
END SUBROUTINE Continued_Fraction_e
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
