...
!
  IF ( dollar('$CC_Prop_Basis',card,cpass,inp) ) then
!
...
!
        nchan=intkey(card,'number-of-channels',1,' ')
        WRITE(iout,2) type_calculation, algorithm, nchan
        system=chrkey(card,'coordinate-system','spherical',' ')
!
...
!       
        ...
        write(iout,3) system, units, genpts
!
...
!
        IF(absorb) THEN
           n_reg_absorb(1)=intkey(card,'number-of-absorbing-'//        &
                                          'regions',0,' ')
        END IF
  ELSE
        write(iout,5)
...
2 FORMAT(/,20X,'solve CC time-dependent schrodinger equation in'    &
         /,20x,'          ',a16,//,20x,                          &
               'algorithm = ',a24,/,20x,                         &
               'number of channels = ',i4)
