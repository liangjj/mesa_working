...
!
                          INTERFACE Channel_Matrices
                   MODULE PROCEDURE Channel_Matrices_d,                                  &
                                    Channel_Matrices_z
                          END INTERFACE Channel_Matrices
!
...
!
  SUBROUTINE Small_Matrix(ham_type,file_directory,ham_file)
  IMPLICIT NONE
...
!     Input the matrices
!
      Call Input_Full_Matrices(triangle_hamiltonian_d, triangle_overlap_d,      &
                               file_directory,ham_file) 
!
...
!
      IF(reformatting_control == 'channel_format') THEN
         ...
         ELSE 
            Write(iout,3)
            Call Channel_Matrices(triangle_hamiltonian_d, triangle_overlap_d)
            n3d = new_size
            tri_size = new_tri_size
         END IF
      END IF
...
!
      ...
      END IF
      Call Input_Full_Matrices(triangle_hamiltonian_z, triangle_overlap_z,         & 
                               file_directory,ham_file) 
      IF(reformatting_control == 'channel_format') THEN
         IF(channel_labels_only) THEN
            Write(iout,2)
         ...
         ELSE 
            Write(iout,3)
            Call Channel_Matrices(triangle_hamiltonian_z, triangle_overlap_z) 
            n3d = new_size
            tri_size = new_tri_size
         END IF
      END IF
...
!
  SUBROUTINE Input_Full_Matrices_d (ham,s,file_directory,ham_file)
  IMPLICIT NONE
...
!
  SUBROUTINE Input_Full_Matrices_z(ham,s,file_directory,ham_file)
  IMPLICIT NONE
...
!***********************************************************************
!***********************************************************************
!deck Channel_Matrices_d
!***begin prologue     Channel_Matrices_d
!***date written       061201   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords
!***author             schneider, barry (nsf)
...
!***references
!***routines called
!***end prologue       Channel_Matrices_d
!
  SUBROUTINE Channel_Matrices_d (ham_tri,s_tri)
  IMPLICIT NONE
  REAL*8, DIMENSION(:)                  :: ham_tri
  REAL*8, DIMENSION(:)                  :: s_tri
  CHARACTER(LEN=3)                      :: itoc
...
!
  ...
  DO ic=1, number_of_channels
     DO jc=1, number_of_channels
        ALLOCATE(channel_mat(ic,jc)%channel_h_matrix_d(channel_size,channel_size))
     END DO
  END DO
  ALLOCATE( h_mat_tri_d(new_tri_size) )
  ...
  IF(print_cc) THEN
     DO ic = 1, number_of_channels
         DO jc = 1, ic
            local_title='Hamiltonian Matrix for channel i = '//itoc(ic)//          &
                  ' channel j = '//itoc(jc)
            call prntfmn(local_title,channel_mat(ic,jc)%channel_h_matrix_d,        &
                         new_size, new_size, new_size, new_size, iout,'e')
        END DO
     END DO
  END IF
  DEALLOCATE( h_mat_tri_d )
  IF (non_orth) THEN
      DO ic=1, number_of_channels
         DO jc=1, number_of_channels
            ALLOCATE(                                                        &
            channel_mat(ic,jc)%channel_s_matrix_d(channel_size,channel_size))
         END DO
      END DO
      ALLOCATE( s_mat_tri_d(new_tri_size) )
      ...
      IF(print_cc) THEN
          DO ic = 1, number_of_channels
             DO jc = 1, ic
                local_title='Overlap Matrix for channel i = '//itoc(ic)//           &
                      ' channel j = '//itoc(jc)
                call prntfmn(local_title,channel_mat(ic,jc)%channel_s_matrix_d,     &
  ...
  END IF
!***********************************************************************
  END SUBROUTINE Channel_Matrices_d
!***********************************************************************
!***********************************************************************
!deck Channel_Matrices_z
!***begin prologue     Channel_Matrices_z
!***date written       061201   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords
!***author             schneider, barry (nsf)
...
!***references
!***routines called
!***end prologue       Channel_Matrices_z
!
  SUBROUTINE Channel_Matrices_z (ham_tri,s_tri)
  IMPLICIT NONE
  COMPLEX*16, DIMENSION(:)              :: ham_tri
  COMPLEX*16, DIMENSION(:)              :: s_tri
  CHARACTER(LEN=3)                      :: itoc
...
!
  ...
  DO ic=1, number_of_channels
     DO jc=1, number_of_channels
        ALLOCATE(channel_mat(ic,jc)%channel_h_matrix_z(channel_size,channel_size))
     END DO
  END DO
  ALLOCATE( h_mat_tri_z(new_tri_size) )
  ...
  IF(print_channel_matrices) THEN
     DO ic = 1, number_of_channels
         DO jc = 1, ic
            local_title='Hamiltonian Matrix for channel i = '//itoc(ic)//           &
                  ' channel j = '//itoc(jc)
            call prntcmn(local_title,channel_mat(ic,jc)%channel_h_matrix_z,         &
                         new_size, new_size, new_size, new_size,              &
                         iout,'e')
        END DO
     END DO
  ...
  IF (non_orth) THEN
      DO ic=1, number_of_channels
         DO jc=1, number_of_channels
            ALLOCATE(                                                        &
            channel_mat(ic,jc)%channel_s_matrix_z(channel_size,channel_size))
         END DO
      END DO
      ALLOCATE( s_mat_tri_z(new_tri_size) )
      ...
      IF(print_channel_matrices) THEN
          DO ic = 1, number_of_channels
             DO jc = 1, ic
                local_title='Overlap Matrix for channel i = '//itoc(ic)//          &
                      ' channel j = '//itoc(jc)
                call prntcmn(local_title,channel_mat(ic,jc)%channel_s_matrix_z,    &
                             new_size, new_size, new_size, new_size,         &
                             iout,'e')
             END DO
          END DO
  ...
  END IF
!***********************************************************************
  END SUBROUTINE Channel_Matrices_z
!***********************************************************************
!***********************************************************************
