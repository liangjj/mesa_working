...
!***********************************************************************
                           ...
                           USE Global_Time_Propagation_Module, ONLY:packed_matrices, non_orth
...
!
  v_i(:) = v_i(:) /sqrt(ddot(n3d,v_i(:),1,v_i(:),1))
  vec_d(:,0) = v_i(:)
...
!    
!
     Call Generalized_Lanczos_Matmul(vec_d(:,it), h_vec_d(:), it)
!
...
!
     local_time=secnds(0.0)
     call Generalized_Lanczos_Convergence_Test(v_o,eigen_vectors,lanczos_tri_d,it)
...
!           have not reached the convergence criterion.
!
        WRITE(iout,4) eig(0)
