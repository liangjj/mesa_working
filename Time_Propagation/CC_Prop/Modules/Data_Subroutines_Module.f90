...
!
!
  IF ( dollar('$channel_parameters',card,cpass,inp) ) then
       input_output_vector=chrkey(card,'input_output_vector','output',' ')
       read_only = logkey(card,'read_only',.false.,' ')
       channel_format = logkey(card,'channel_format',.false.,' ')       
       test_s_inverse = logkey(card,'test_s_inverse',.false.,' ')
       len=lenth(ham_file)
  ...
  IF( ham_file == 'from_input') THEN
      write(iout,*) 'Hamiltonian File from Input_Full'
...
!
!         This type of read is for a B-spline matrix
!         which has no zeros removed and is stored by channels and numbers
!         of correlation terms.  However, there is no infomation on the
!         file itself to actually pick off the channel blocks.
!
          reformat_input_matrix_only = .false.
          READ(50) number_of_splines, number_of_channels, number_of_correlation_terms
          n3d = number_of_splines *  number_of_channels + number_of_correlation_terms
...
!
  ...
  WRITE(iout,*) '    Input/Output Vector = ',input_output_vector
  IF( reformatting_control=='channel_format') THEN
!
!     This section of code is designed to allow the user to reformat the
!     input matrices in a way that explicit channel and correlation
!     information is incorporated.  The information must be input by the
!     user.  It is not assumed to be available on the file.
!
      number_of_channels = intkey(card,'number_of_channels',number_of_channels,' ')
      number_of_correlation_terms = intkey(card,'number_of_correlation_terms',           &
                                                 number_of_correlation_terms,' ')
      number_of_splines = intkey(card,'number_of_splines',number_of_splines,' ')
      channel_labels_only = logkey(card,'channel_labels_only',.false.,' ')
      print_channel_matrices = logkey(card,'print_channel_matrices',.false.,' ')
      L_Max=intkey(card,'L_max',0,' ')
      spline_removal(1) = logkey(card,'remove_first_spline',.false.,' ')
      spline_removal(2) = logkey(card,'remove_last_spline',.false.,' ')
      ALLOCATE( channel(number_of_channels),                                             &
                        channel_buf(number_of_channels,number_of_splines) )
      Write(iout,3) number_of_channels, L_Max
      Write(iout,4) number_of_splines, number_of_correlation_terms, spline_removal(1:2)
      first_spline = 1
      ...
      END IF
      channel_size = last_spline - first_spline + 1
      new_size = channel_size * number_of_channels
      new_tri_size = new_size * ( new_size + 1 ) / 2
      WRITE(iout,5) channel_size, new_size
      WRITE(iout,6)
      DO ic = 1, number_of_channels
         ALLOCATE(channel(ic)%channel_quantum_numbers(4) )
         CALL intarr(card,'channel_quantum_numbers_channel_'//itoc(ic),                  &
                           channel(ic)%channel_quantum_numbers,4,' ')
         WRITE(iout,7) ic, channel(ic)%channel_quantum_numbers(1:4)
      END DO
      count = 0
      DO ic=1, number_of_channels
...
3  FORMAT(/,30X,'Channel Information',/,                                                 &
          /,5x, 'Number of Channels = ',i5,5x,'Maximum L   = ',i3)
4  FORMAT(/,5x,'Number of Splines             = 'i5,                                     &
            5x,'Number of Correlation Terms   = 'i5,                                     &
          /,5x,'Remove First Spline           =     'l1,                                 &
            5x,'Remove Last Spline            =     'l1)
5  FORMAT(/,5x, 'Channel Size= ',i5,5x,'Matrix Size After Spline Deletion   = ', i6)
6  FORMAT(/,10x,'Channel',5x,'   L   ','   S   ',                                        &
              '   M_L  ','   M_S  ')
7  FORMAT(/,9x,i5,7x,i5,2x,i5,3x,i5,3x,i5)
8  Format(/,20x,'Using Packed Forms for all Matrices')
...
!
       ...
       cholesky_file_name = chrkey(card,'cholesky_matrix_file_name','Cholesky_Deomposition',' ')
       state_file_name = chrkey(card,'state_matrix_file_name','State_Matrices',' ')
...
!***********************************************************************************************
!
!            More variables that need to be set.  The default is usually ok for most
!            of them.
!
       ...
       input_matrices=chrkey(card,'input_matrices','packed_in_drake_format',' ')
       output_matrices=chrkey(card,'output_matrices','packed_in_triangular_iosys_format',' ')
       packed_matrices=logkey(card,'packed_matrices',.false.,' ')
...
!***********************************************************************************************
       IF(packed_matrices) THEN
          ...
          packed_file_name = chrkey(card,'packed_matrix_file_name','Packed_Matrices',' ')
          ALLOCATE(mat_var(4))
       END IF
!***********************************************************************************************
!
!       Some print options are set here.  Taking the default(none) will give minimal
...
!***********************************************************************************************
  ...
  IF(packed_matrices) THEN
     Write(iout,4) packed_matrices, drop_overlap, drop_cholesky, drop_hamiltonian,    &
...
!***********************************************************************************************
!     
      time_dependent_potential = logkey(card,'time_dependent_potential',.false.,' ')
      photon_energy = fpkey(card,'photon_energy',1.d0,' ')
      time_one_optical_cycle = two_pi/photon_energy
      number_optical_cycles=intkey(card,'number_of_optical_cycles',10,' ')
      steps_per_optical_cycle=intkey(card,'number_of_steps_per_optical_cycle',10,' ')
      ...
      peak_electric_field=fpkey(card,'peak_electric_field',peak_electric_field,' ')
      peak_intensity = peak_electric_field * peak_electric_field * electric_field_to_intensity
      pulse_duration = fpkey(card,'pulse_duration',pulse_duration,' ')
      ramp_time = fpkey(card,'ramp_time',ramp_time,' ')
      WRITE(iout,6) t_init, ntreg, deltat, photon_energy, time_one_optical_cycle,                    &
                   peak_electric_field, peak_intensity, pulse_duration, ramp_time
  ELSE
      write(iout,2)
      stop
  END IF
