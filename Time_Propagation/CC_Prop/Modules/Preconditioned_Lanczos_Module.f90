  ...
  IF (packed_matrices) THEN 
      ...
      call column_packed_lower_triangle_on_vector                      &
                         (v_out(:),                                    &
                          ...
                          mat_var(1)%matrix_diagonal_z )
  ELSE
      call matrix_vector_multiply( triangle_overlap_z, v_in(:), s_vec_z(:,0) )
  END IF
...
!
        ...
        IF (packed_matrices) THEN 
            ...
            call column_packed_lower_triangle_on_vector                       &
                                (h_vec_z(:),                                  &
                                 ...
                                 mat_var(1)%matrix_diagonal_z )
        ELSE
            call matrix_vector_multiply( triangle_overlap_z,                  &
                                         v_out(:), s_vec_z(:,0) )
        END IF
        ...
        IF(compute_energy) THEN 
           IF(type == 'general') THEN
              ...
              ELSE
                  call matrix_vector_multiply( triangle_hamiltonian_z, v_in(:), &
                                               h_vec_z )
              END IF
           ...
           call column_packed_upper_triangle_on_vector                      &
            ...
            call column_packed_lower_triangle_on_vector                     &
                                (h_vec_z(:),                                &
                                 ...
                                 mat_var(1)%matrix_diagonal_z )
        ELSE
            call matrix_vector_multiply( triangle_overlap_z, v_out, s_vec_z(:,0) )
        END IF 
        norm = 1.d0/sqrt(cdotc(n3d,v_in,1,s_vec_z(:,0),1))
        v_in = v_out
        kewrd='complex'
...
!
     ...
     ELSE
         Call Triangular_Solve(lanczos_basis(:,it+1),v_out,'t',it)
     END IF
!
!
...
!
     ...
     IF (packed_matrices) THEN
         Call Packed_Solve (lanczos_basis(:,it+1),                           &
                            ...
                            mat_var(2)%packed_columns_z,                     &
                            mat_var(1)%matrix_diagonal_z,                    &
                            mat_var(2)%number,'c',it)
     ELSE
         Call Triangular_Solve(lanczos_basis(:,it+1),v_out,'n',it)
     END IF
!
!
!         Compute b(it+1)  b(i) = Sqrt ( <V_i|V_i> )
         ...
         IF (packed_matrices) THEN 
            ...
            call column_packed_lower_triangle_on_vector                      &
                                (h_vec_z(:),                                 &
                                 ...
                                 mat_var(1)%matrix_diagonal_z )
         ELSE
             call matrix_vector_multiply(triangle_overlap_z, v_out,           &
                                         s_on_lanczos_basis(:,it+1) )        
         END IF
