...
!
  ...
  IF (packed) THEN
      ALLOCATE( number(2), non_zero_columns(n3d,2), row_index(lenbuf,2),       &
                packed_columns_d(lenbuf,2), matrix_diagonal_d(n3d) )
      CALL h_pack_upper(upper,non_zero_columns(:,1),row_index(:,1),            &
                        packed_columns_d(:,1),matrix_diagonal_d(:),            &
                        number(1),.true.)
      CALL h_pack_lower(lower,non_zero_columns(:,2),row_index(:,2),             &
                        packed_columns_d(:,2),matrix_diagonal_d(:),             &
                        number(2),.true.)
  END IF
...
!
  ...
  IF (packed) THEN
      ALLOCATE( number(2), non_zero_columns(n3d,2), row_index(lenbuf,2),       &
                packed_columns_z(lenbuf,2), matrix_diagonal_z(n3d) )
      CALL h_pack_upper(upper,non_zero_columns(:,1),row_index(:,1),            &
                        packed_columns_z(:,1),matrix_diagonal_z(:),            &
                        number(1),.true.)
      CALL h_pack_lower(lower,non_zero_columns(:,2),row_index(:,2),             &
                        packed_columns_z(:,2),matrix_diagonal_z(:),             &
                        number(2),.true.)
  END IF
1  Format(/,5x,'Column = ',i4)
2  Format( (15x,5f10.5) )
