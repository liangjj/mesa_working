...
!
      atomic_charge=fpkey(card,'atomic-charge',1.d0,' ')
      ...
      END IF
      WRITE(iout,1) charge, l_orb_max, l_max, ints
      units='atomic-units'
      typke='dvr'
      diag=.true.
      proj=.false.
      ...
      proj=.false.
      WRITE(iout,1) atomic_charge, l_orb_max, l_max, ints, dentyp
      coord(1)='r'
  END IF
  ALLOCATE(Orb(0:l_orb_max))
  WRITE(iout,2)
  DO l=0, l_orb_max
     ALLOCATE(Orb(l)%M_Val(-l:l))
     CALL intarr(card,'M-Values-Shell-'//itoc(l),               &
                 Orb(l)%M_Val,2*l+1,' ')
     WRITE(iout,3) l, (Orb(l)%M_Val(i),i=-l,l) 
  END DO
  ALLOCATE(c_tmp(0:l_orb_max,0:l_orb_max,0:(l_orb_max + l_orb_max)))
  DO l_1=0,l_orb_max
     DO l_2=0,l_1
        l_sum=l_1+l_2
        l_diff=iabs( l_1 - l_2)
        DO l=l_diff, l_sum
           c_tmp(l_1,l_2,l) = Three_J_Coef(l_1,0,l_2,0,l,0)           
           c_tmp(l_2,l_1,l) =c_tmp(l_1,l_2,l)
        END DO
     END DO      
  END DO                 
  count=0
...
1 FORMAT(/,20X,'generating atomic dvr functions and operators', &
         ...
         /,5x,'integrals                        = ',a16,        &
         /,5x,'type density                     = ',a16)
2 FORMAT(/,10x'Allowed M Values-zero is yes, one is no')
3 FORMAT(/,5x,'Shell = ',i3,(/,5x,'Values =',20i1))
!***********************************************************************
...
!***********************************************************************
  ...
  IF( ints == 'all-integrals') THEN
      ...
      CALL Twomat(v_two,grid(1)%ke,grid(1)%ang_pot,grid(1)%pt,           &
