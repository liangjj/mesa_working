subroutine seiv_slice(nro)
  ...
  if (io .AND. rank .EQ. root) then
     ones = max(mod(nro,10),0)
     tens = max(mod(nro - ones,100)/10,0)
     dogs = max(mod(nro - ones - tens, 1000)/100,0)
     ones = ones + 48
     tens = tens + 48
     dogs = dogs + 48
     ...
     ! ========== density slice =================================
     open(unit=11,file= outdir // outfile // '_' // cnro //'.sls')
     i = ceiling(dimx/2d0)
     do j = 1, dimy
        do k = 1, dimz
     ...
     ! ========== column density ================================
     open(unit=11,file= outdir // outfile // '_' // cnro //'.slc')
     do j = 1, dimy
        do k = 1, dimz
           colsum = 0d0
           do i = 1, dimx
              colsum = colsum + abs(savewf(i,j,k))**2 
           enddo
           write(11,101) colsum
        enddo
     enddo
