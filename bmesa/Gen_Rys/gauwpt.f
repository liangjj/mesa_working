*deck @(#)gauwpt.f	1.1 9/9/91
      subroutine gauwpt(ckind, n, alpha, beta, kpts, endpts, 
     1                  b, t, w, prn)
c
...
c
 100      if(prn) then
             title='polynomial alpha recursion coefficient'
             call prntrm(title,t,n,1,n,1,iout)
             title='polynomial beta recursion coefficient'
...
c
 ...
 110      continue   
       ...
       if(ckind.eq.'laguerre'.or.ckind.eq.'hermite') then
          return
       endif	  
       call chnvar (t,w,-1.d0,1.d0,endpts(1),endpts(2),b,n)
c
      return
      end
