...
c \title{Driver for Non-Iterative Integral Equation}
...
c     Start looping over the energies

      do 10 i=1,nen

c        Calculate the regular and irregular solution to the
c        homogeneous free particle equation

         call green(g(r_k),g(i_k),g(r),energy(i),npts)
         if(exch.or.opt) then

c           Solve for $\Psi_{00}$  If there is no exchange or optical
c           potential a much simpler routine is used.
...
c           Solve for $\Psi_{00}$ when there is no exchange or optical
c           potential
