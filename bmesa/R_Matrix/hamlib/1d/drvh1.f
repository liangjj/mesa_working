...
c***end prologue       drvh1
      subroutine drvh1(pham,phone,phamil,n,len,nonzro,ngot,pack,
     1                 drctv,prn)
      implicit integer (a-z)
      real*8 ham, one, hamil
      logical pack, prn
      character*80 title
      character*(*) drctv
      pointer (pham,ham(1))
      ...
      if(pack) then
         ...
         call h1pac(ham(h),ham(v),one(hbuf),ione(buf),one(diag),
     1              n,len,nonzro,prn)
         write(iout,1) nonzro
         if(prn) then
...
c
         call copy(ham(h),hamil(htmp),n*n)
         call vec2di(ham(h),ham(h),ham(v),'subtract',n)
         call dsyev('v','l',n,ham(h),n,hamil(eig),hamil(work),
     1               5*n,info)
         title='free particle eigenvalues'
         call prntrm(title,hamil(eig),n,1,n,1,iout)
         call copy(hamil(htmp),ham(h),n*n)
