...
!
      DO j=1,num_reg(i)
         ...
         ALLOCATE(mat_reg_d(j,i)%ke_mat_d                               &
      ...
      END DO
      IF(absorb) THEN
         j = nfun_reg(num_reg(i),i)
!
!        There is an absorber.  Allocate the complex arrays.
!
         ALLOCATE(mat_reg_z(i)%ke_mat_z(j,j),                           &
                  mat_reg_z(i)%eigval_mat_z(j),                         & 
                  mat_reg_z(i)%eigvec_mat_z_r(j,j),                     & 
                  mat_reg_z(i)%eigvec_mat_z_l(j,j))  
      ...
      ELSE
         mat_typ='banded'
      END IF
      call modify_diag(grid(i)%ke,grid(i)%v,nphy(i),                    &
                       keep_diag,mat_typ)
      start = 1
      ...
      ELSE IF(mat_typ=='banded') THEN
         DO j=1,num_reg(i)
            call ke_reg_fd(grid(i)%ke(start,1),                         &
                           mat_reg_d(j,i)%ke_mat_d,                     &
                           nfun_reg(j,i),j)
            start = start + nfun_reg(j,i) - 1
         END DO
...
!
      ...
      IF(.not.absorb) THEN
         ...
         DEALLOCATE(scr_d)
      ELSE
         CALL diag_reg(mat_reg_z(i)%ke_mat_z,                           &
...
!
      ...
      IF(prop_order ==2) THEN
         DO j=1,num_reg(i)
            ALLOCATE(mat_reg_d(j,i)%cosine_t_mat_2                      &
                    (nfun_reg(j,i),nfun_reg(j,i)),                      &
                     mat_reg_d(j,i)%sine_t_mat_2                        &
                    (nfun_reg(j,i),nfun_reg(j,i)))
         END DO
         CALL propagator_2(i)
      ELSE IF(prop_order ==4) THEN
         DO j=1,num_reg(i),2
            ALLOCATE(mat_reg_d(j,i)%cosine_t_mat_4                      &
                     (nfun_reg(j,i),nfun_reg(j,i),3),                   &
                     mat_reg_d(j,i)%sine_t_mat_4                        &
                     (nfun_reg(j,i),nfun_reg(j,i),3))
         END DO
         DO j=2,num_reg(i),2
            ALLOCATE(mat_reg_d(j,i)%cosine_t_mat_4                      &
                     (nfun_reg(j,i),nfun_reg(j,i),2),                   &
                     mat_reg_d(j,i)%sine_t_mat_4                        &
                     (nfun_reg(j,i),nfun_reg(j,i),2))
         END DO
!         CALL propagator_4(i)
...
!
  IF(spdim == 1) THEN
     ALLOCATE(psi_08_1d(nphy(1),2),                                     &
              v_scr_08_1d(nphy(1),2))
     CALL so_prop(psi_08_1d,v_scr_08_1d,nphy(1))
  ELSE IF(spdim == 2) THEN
     ALLOCATE(psi_08_2d(nphy(2),nphy(1),2),                             &
              v_scr_08_2d(nphy(2),nphy(1),2))
     CALL so_prop(psi_08_2d,v_scr_08_2d,nphy(1:2))
  ELSE IF(spdim == 3) THEN
     ALLOCATE(psi_08_3d(nphy(3),nphy(2),nphy(1),2),                     &
              v_scr_08_3d(nphy(3),nphy(2),nphy(1),2))
     CALL so_prop(psi_08_3d,v_scr_08_3d,nphy(1:3))
  ELSE
     CALL lnkerr('error')
  END IF
!
...
! 
  DO i=1,spdim
     ...
     IF(prop_order ==2) THEN     
        DO j=1,nreg
           DEALLOCATE(mat_reg_d(j,i)%cosine_t_mat_2,                   & 
                      mat_reg_d(j,i)%sine_t_mat_2)
        END DO
     ELSE IF(prop_order ==4) THEN
        DO j=1,num_reg(i),2
           DEALLOCATE(mat_reg_d(j,i)%cosine_t_mat_4,                   &
                      mat_reg_d(j,i)%sine_t_mat_4)
         END DO
         DO j=2,num_reg(i),2
            DEALLOCATE(mat_reg_d(j,i)%cosine_t_mat_4,                  &
