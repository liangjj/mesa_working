...
!**references
!**routines called
!**end prologue       gauss_paket_08
  SUBROUTINE gauss_paket_08(norm)
  USE dvrprop_global
  USE dvr_shared
  USE dvr_global
  IMPLICIT NONE
...
! Go get some needed scratch memory.
  ...
  IF(system == 'cartesian') THEN
     ...
     IF(spdim == 1) THEN
        CALL z_proj_08(grid(1)%pt,grid(1)%wt,grid(1)%f, &
                       alpha(1),sigma(1),x_0(1),beta(1),zloc(1)%z,nphy(1))
        psi_08 = zloc(1)%z
     ELSE IF(spdim == 2) THEN
        CALL z_proj_08(grid(1)%pt,grid(1)%wt,grid(1)%f, &
                       alpha(1),sigma(1),x_0(1),beta(1),zloc(1)%z,nphy(1))
        CALL z_proj_08(grid(2)%pt,grid(2)%wt,grid(2)%f, &
                       alpha(2),sigma(2),x_0(2),beta(2),zloc(2)%z, &
                       nphy(2))
        CALL z_fil_08(zloc(1)%z,zloc(2)%z,zdum)
     ELSE IF(spdim == 3) THEN
        ...
        CALL z_proj_08(grid(2)%pt,grid(2)%wt,grid(2)%f, &
                       alpha(2),sigma(2),x_0(2),beta(2),zloc(2)%z, &
                       nphy(2))
        CALL z_proj_08(grid(3)%pt,grid(3)%wt,grid(3)%f, &
                       alpha(3),sigma(3),x_0(3),beta(3),zloc(3)%z, &
                       nphy(3))
        CALL z_fil_08(zloc(1)%z,zloc(2)%z,zloc(3)%z)
    END IF
  ELSE
    WRITE(iout,8)
