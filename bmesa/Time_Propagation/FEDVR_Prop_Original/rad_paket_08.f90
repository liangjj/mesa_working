...
!**end prologue       rad_paket_08
  ...
  IF(typ_pak == 'exponential') then
     WRITE(iout,2)
     psi_08(:,1) = ( grid(1)%pt - x_0(1) )**powr * &
                exp( - ( sigma(1) ) * ( grid(1)%pt - x_0(1) ) )
     psi_08(:,2) = - psi_08(:,1) * sin( beta(1)  * ( grid(1)%pt - x_0(1) ) )
     psi_08(:,1) =   psi_08(:,1) * cos( beta(1)  * ( grid(1)%pt - x_0(1) ) )
  ELSE IF(typ_pak == 'gaussian') then
     ...
     psi_08(:,2) = - sin ( grid(1)%pt - x_0(1) ) * psi_08(:,1)
     psi_08(:,1) =   cos ( grid(1)%pt - x_0(1) ) * psi_08(:,1)
  END IF
  WRITE(iout,4)
  WRITE(iout,5) sigma(1), x_0(1), beta(1), powr
  psi_08(:,1)  = psi_08(:,1) * grid(1)%wt
  psi_08(:,2)  = psi_08(:,2) * grid(1)%wt
  do i=1,nphy(1)
     psi_08(i,1) = psi_08(i,1) * grid(1)%f(i,i)
  END DO
  norm = 1.d0/sqrt( sdot(nphy(1),psi_08(1,1),1,psi_08(1,1),1) + &
                    sdot(nphy(1),psi_08(1,2),1,psi_08(1,2),1) )
  psi_08 = psi_08 * norm  
  norm = sdot(nphy(1),psi_08(1,1),1,psi_08(1,1),1) + &
         sdot(nphy(1),psi_08(1,2),1,psi_08(1,2),1)
