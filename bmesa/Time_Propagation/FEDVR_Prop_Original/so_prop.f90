...
!              propagation.  The variable n3d is the length of the vector.
!
  ALLOCATE(psi_08(n3d,2),v_scr_08(n3d,2),v_tot(n3d),      &
           sin_diag(n3d),cos_diag(n3d),tim_pts(ntreg+1))
  IF (spdim==2) THEN
      maxdim=max(nphy(2),nphy(1))
      wds_1=max(nphy(1)*4,maxdim**2)
      ALLOCATE(fac(wds_1))
  ELSE IF(spdim==3) THEN
      maxdim=max(nphy(3),nphy(2),nphy(1))
      wds_1=max(nphy(2)*nphy(1)*4,maxdim*maxdim*2)
      wds_2=max(nphy(1)*4,maxdim*2)
      ALLOCATE(fac(wds_1),v_1(wds_2))
  END IF 
...
!
      ...
      END IF
      call iosys('write real solution to bec',2*n3d,psi_08,0,' ')
...
!
  ...
  ELSE IF(spdim==3) THEN
      DEALLOCATE(fac,v_1)
  END IF 
  DEALLOCATE(psi_08,v_scr_08,v_tot,sin_diag,cos_diag,tim_pts)
!
