...
!***end prologue       rsp_2_1d
  SUBROUTINE rsp_2_1d(wave_function,scratch_vector)
     ...
     USE exp_off_diagonal
  IMPLICIT NONE
  REAL*8, DIMENSION(nphy(1),2)                       :: wave_function, scratch_vector
!
...
!-----------------------------------------------------------------------
  ...
  call diagonal_mul(wave_function,scratch_vector,sin_diag,cos_diag,nphy(1))
!-----------------------------------------------------------------------
  IF(log_main(9)) then
