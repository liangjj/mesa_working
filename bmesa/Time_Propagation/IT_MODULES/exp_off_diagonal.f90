...
!                       General Code
!
           CALL v_m_v_gen(v(locate(trips),1),                           &
                          v_scr(locate(trips),1),                       &
...
!
           CALL v_m_v_10(v(locate(trips),1),                            &
                         v_scr(locate(trips),1),                        &
                         mat_reg_d(i,index)%exp_t_mat(:,:,prop_point),  &
                         ni,nj)
        END IF
...
!                        General Case
!
           CALL v_v_m_gen(v(1,locate(trips)),                           &
                          v_scr(1,locate(trips)),                       &
                          mat_reg_d(i,index)%exp_t_mat(:,:,prop_point), &
                          ni,nj,nfun_reg(i,index)) 
!
        ELSE IF (nfun_reg(i,index) == 2) then
...
!                       Ten by Ten
!
           CALL v_v_m_10(v(1,locate(trips)),                            &
                         v_scr(1,locate(trips)),                        &
                         mat_reg_d(i,index)%exp_t_mat(:,:,prop_point),  &
                         ni,nj)
        END IF
