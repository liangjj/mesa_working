...
!
   INTEGER, PARAMETER :: np8=5101, nprmx=10
!
...
!
       ...
       REAL*8, DIMENSION(np8) :: vij,vijt
       REAL*8 :: x_shift,phase,phase_coeff,t_restart,&
                 alpha,scale_ho,velocity
       INTEGER :: n_shift,mask
!
...
!
          call timfn(t,delt,vij,vpt,nrp1,vijt,wfr,wfi, &
                     term_nl,upr,upi)
!
! propagate the right-hand kinetic energy operator
!
          call expk(nrp1,fc2,fs2,fc4,fs4,wfr,wfi)
!
! propagate the diagonal potential operator
!
          call expu(nrp1,wfr,wfi,upr,upi)
!
! propagate the left-hand kinetic energy operator
!
          call expk(nrp1,fc2,fs2,fc4,fs4,wfr,wfi)
!
!
          if(mod(it,ntime) .eq. 0) then
              call prbfn(it,nrp1,iwftyp,t,period,delr,wfr,wfi,pbxx)
          endif
!
!
       enddo
...
!
 ...
 1466  format(///'error in continuum mesh'// &
     ' npair =',i10,5x,' npair(input-45)=',i10///)
...
!
      return
       ...
       subroutine expk(np,c2,s2,c4,s4,wfr,wfi)
!
       USE gen_data
!
       IMPLICIT NONE
...
!
         tm1(1:np-1:2) = c4*wfr(1:np-1:2) - s4*wfi(2:np:2)
         tm2(1:np-1:2) = c4*wfi(1:np-1:2) + s4*wfr(2:np:2)
         tm3(2:np:2) = -s4*wfi(1:np-1:2) + c4*wfr(2:np:2)
         tm4(2:np:2) = s4*wfr(1:np-1:2) + c4*wfi(2:np:2)
         wfr(1:np-1:2) = tm1(1:np-1:2)
         wfi(1:np-1:2) = tm2(1:np-1:2)
         wfr(2:np:2) = tm3(2:np:2)
         wfi(2:np:2) = tm4(2:np:2)
         wfr(np) = zero
         wfi(np) = zero
!
!
!
!
!* operate with k(even) for delt
!
!
         wfr(1) = zero
         wfi(1) = zero
         tm1(2:np:2) = c2*wfr(2:np:2) - s2*wfi(3:np-1:2)
         tm2(2:np:2) = c2*wfi(2:np:2) + s2*wfr(3:np-1:2)
         tm3(3:np-1:2) = -s2*wfi(2:np:2) + c2*wfr(3:np-1:2)
         tm4(3:np-1:2) = s2*wfr(2:np:2) + c2*wfi(3:np-1:2)
         wfr(2:np:2) = tm1(2:np:2)
         wfi(2:np:2) = tm2(2:np:2)
         wfr(3:np-1:2) = tm3(3:np-1:2)
         wfi(3:np-1:2) = tm4(3:np-1:2)
!
!
!
!* operate with k(odd) on delt/2
!
         tm1(1:np-1:2) = c4*wfr(1:np-1:2) - s4*wfi(2:np:2)
         tm2(1:np-1:2) = c4*wfi(1:np-1:2) + s4*wfr(2:np:2)
         tm3(2:np:2) = -s4*wfi(1:np-1:2) + c4*wfr(2:np:2)
...
!
      ...
      subroutine prbfn(it,np,iwftyp,t,period,delr,wfr,wfi,pbb)
!
       USE gen_data
       USE splint_arrays
!
...
!
       ...
       REAL*8, INTENT(IN) :: t,period,delr
...
!
       ...
       REAL*8 :: tpp,tpx,ptot,pbr1,pbi1,pbb,prbr,exp_nlr, &
