...
c \title{Main Routine to Drive Propagation}
...
c     The equation we are solving here is;
c\begin{equation}
c  \big ( i \frac{d}{dt} - H \big ) \mid \Xi (\bf{r},t) \rangle \big )
c              = H \mid \Psi_{0} (\bf{r},t_{0} ) \rangle
c\end{equation}
cwhere we write,
c\begin{equation}
c   \mid \Psi( \bf{r},t ) \rangle = \mid \Psi_{0}( \bf{r},t_{0}) \rangle
c                                   + \mid \Xi (\bf{r},t) \rangle
c\end{equation}
c and $t_{0}$ is the initial time. For a time independent Hamiltonian or
c for small time steps, we have
c\begin{eqnarray}
c   \mid \Psi(\bf{r},t) \rangle &=& \mid \Psi_{0} (\bf{r},t_{0} ) \rangle
c                               + \mid  \Xi(\bf{r},t) \rangle \\ \nonumber
c                               &=& 
c   \mid \Psi_{0} (\bf{r},t_{0} ) \rangle 
c                       +  
c   \big ( e^{iH(t-t_{0})} - 1 \big ) H \mid \Psi_{0} (\bf{r},t_{0} ) \rangle
c\end{eqnarray}
c
...
c     set up a scratch file to hold the wavefunction for plotting
...
c     initialize the wavefunction at the first time or read in its value.
...
c        Calculate the time dependent perturbation.  It consists of a  
c        space and a time part.                                        
...
c        Calculate the Hamiltonian times the initial state             
...
c        Propagate to the next time.  the approach depends on whether
c        you use iterative or non iterative methods
