!***********************************************************************
! cc_regional_module
!**begin prologue     cc_regional_module
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!deck cc_regional_matrices.f
!***begin prologue     cc_regional_matrices
...
!***end prologue       space
  Subroutine cc_regional_matrices
  IMPLICIT NONE
  INTEGER                                  :: i, j, start, row_dim
  CHARACTER(LEN=8)                         :: mat_typ
!
!
  ALLOCATE(mat_reg(num_reg))
  IF(algorithm == 'split_operator') THEN
     write(iout,1) 
     WRITE(iout,2)
     maxmem_reg=0
     DO i=1,n_reg_real
        write(iout,3) i, nfun_reg(i), 'real'
        ALLOCATE(mat_reg(i)%pt_d                             &
                (nfun_reg(i)),                               &
                 mat_reg(i)%ke_mat_d                         &
                (nfun_reg(i),nfun_reg(i)),                   &
                 mat_reg(i)%eigval_mat_d                     &
                (nfun_reg(i)),                               &
                 mat_reg(i)%eigvec_mat_d                     &
                (nfun_reg(j,i),nfun_reg(j,i)))
        maxmem_reg=max(maxmem_reg,nfun_reg(i))
     END DO
...
!
        DO i=n_reg_real+1, num_reg
           write(iout,3) i, nfun_reg(i),'complex'
           maxmem_reg=max(maxmem_reg,nfun_reg(i))
           ALLOCATE(mat_reg(i)%pt_d                          &
                   (nfun_reg(i)),                            &
                    mat_reg(i)%v_add_z                       &
                   ...
                   (nfun_reg(i),nfun_reg(i)),                &
                    mat_reg(i)%ke_mat_z                      &
                   (nfun_reg(i),nfun_reg(i)),                &
                    mat_reg(i)%eigval_mat_z                  &
                   (nfun_reg(i)),                            &
                    mat_reg(i)%eigvec_mat_z_r                &
                   (nfun_reg(i),nfun_reg(i)),                &
                    mat_reg(i)%eigvec_mat_z_l                &
                   (nfun_reg(i),nfun_reg(i)))                
        END DO
        ALLOCATE(scr_z(5*maxmem_reg))
     END IF
     IF(diag_mod /= 'none') THEN
        call modify_diag(grid%ke,grid%v,nphy,diag_mod)
     END IF
     start = 1
     DO i=1,num_reg
        call pt_reg(grid%pt(start),mat_reg(i)%pt_d,          &
                    nfun_reg(i),nphy)
        start = start + nfun_reg(i) - 1
     END DO
!
...
!
     start = 1
     DO i=1,num_reg
        call ke_reg_dvr(grid%ke(start,start),                &
                        mat_reg(i)%ke_mat_d,                 &
                        nfun_reg(i),nphy,i)
        start = start + nfun_reg(i) - 1
     END DO
     IF(absorb) THEN
        DO i=n_reg_real+1,num_reg
           call add_absorb(mat_reg(i)%pt_d,                  &
                           mat_reg(i)%ke_mat_d,              &
                           mat_reg(i)%v_add_z,               &
                           mat_reg(i)%ke_mat_z,              &
                           nfun_reg(i))
...
!        Diagonalize
!
        call regional_diag_d(mat_reg(i)%ke_mat_d,            &
                             mat_reg(i)%eigval_mat_d,        &
                             mat_reg(i)%eigvec_mat_d,        &
                             nfun_reg(i),i)
     END DO
...
!          Diagonalize
!
           call regional_diag_z(mat_reg(i)%ke_mat_z,         &
                                mat_reg(i)%eigval_mat_z,     &
                                mat_reg(i)%eigvec_mat_z_r,   &
                                mat_reg(i)%eigvec_mat_z_l,   &
                                nfun_reg(i),i)
...
!
     ...
     END IF
     IF(diag_mod /= 'none') THEN
        call modify_diag(grid%ke,grid%v,nphy(i),diag_mod)
     END IF
     start = 1
     DO i=1,num_reg
        call pt_reg(grid%pt(start),mat_reg(i)%pt_d,          &
...
END Subroutine cc_regional_matrices
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
