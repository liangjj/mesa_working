...
!***end prologue       v_tim
  ...
  ELSE IF(typ == 'cosine') THEN
     omega_t=fpkey(card,'electric-field-frequency',10.d0,' ')
     omega_t=omega_t/timau
     scale_t=fpkey(card,'scale-of-cosine-pulse',1.d0,' ')
     IF(fretyp) then
  ...
  ELSE IF(typ == 'gaussian-pulse') THEN
     omega_t=fpkey(card,'electric-field-frequency',10.d0,' ')
     omega_t=omega_t/timau
     IF(fretyp) then
        omega_t = 2.d0 * pi * omega_t
     ...
     IF(units == 'atomic-units') THEN
        omega_t = omega_t * timau
     END IF
     width_t=fpkey(card,'width-of-gaussian-pulse',0.d0,' ')
     shift_t=fpkey(card,'shift-of-gaussian-pulse',0.d0,' ')
     scale_t=fpkey(card,'scale-of-gaussian-pulse',1.d0,' ')
     WRITE(iout,3)
