...
c
         xmin = 1.571d0 * exp (max(log(r1mach(1)), -log(r1mach(2))) +
     1     0.01d0)
         xsml = sqrt(4.0d0*r1mach(3))
      endif
      first = .false.
c
      if (x .le. 0.d0) call lnkerr ('besy1:x zero or negative')
      if (x.gt.4.0d0) go to 20
c
      if (x .lt. xmin) call lnkerr ('besy1:x small y1 overflows')
      y = 0.d0
      if (x.gt.xsml) y = x*x
      besy1 = twodpi * log(0.5d0*x)*besj1(x) + (0.5d0 +
     1  csevl (.125d0*y-1.d0, by1cs, nty1))/x
      return
c
