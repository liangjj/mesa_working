...
c***end prologue       match
      subroutine match(rmat,kmat,coef,kmatc,tmat,xsect,y0,dy0,y1,
     1                 dy1,j,jp,y,yp,wron,scr,ipvt,l,ec,energy,
     2                 rbox,nc,nopen,prnt)
      implicit integer (a-z)
      common /io/ inp, iout
      real*8 rmat, kmat, y0, dy0, y1, dy1, ec, energy, rbox, tmp
     ...
     1                  'derivatives',.false.)
             ...
             if (prnt) then
                 write(iout,1) i
                 write(iout,3) i, ekc, l(i)
                 write(iout,4) y0(i), dy0(i), y1(i), dy1(i)
             endif
         else
             ekc=-ekc
             ekc=sqrt(ekc)
  ...
  100 continue
      ...
      return
    1 format(/,50x,'channel = ',i2,' is open')
    2 format(/,50x,'channel = ',i2,' is closed')      
    3 format (/,1x,'channel = ',i3,1x,'k or kappa = ',e15.8,1x,
     1             'l = ',i2)   
    4 format (/,1x,'y0 = ',e15.8,5x,'dy0 = ',e15.8,/,1x,
