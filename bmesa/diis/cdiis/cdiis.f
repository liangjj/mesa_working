...
c***source             
c***purpose            complex version of diis algorithm
c
c***references       
c
c***routines called    iosys, util and mdutil
c***end prologue       cdiis
      program cdiis
c
     ...
     1                    pleft, pright, nrhs, maxit, nen
      ...
      do 10 i=1,2
         ...
         eigcm=eigc+nmax
         tham=eigcm+2*nmax
         matrix=tham+2*nmax*nmax
         rhs=matrix+2*nmax*nmax
         xold=rhs+2*nmax*nrhs
         err=xold+2*nmax*(maxit+1)
         xcur=err+2*nmax*(maxit+1)
         errcur=xcur+2*nmax
         bb=errcur+2*nmax
         bbtmp=bb+2*(maxit+1)*(maxit+1)
         sol=bbtmp+2*(maxit+1)*(maxit+1)
...
c
     ...
     1          nmax,npt,pottyp,.false.)
      if(tsteig) then
         call cgeev(z(tham),nmax,nmax,z(eigcm),z(rhs),nmax,z(work(1)),0,
     1              info)
         title='eigenvalues of complex hamiltonian'
         call prntcm(title,z(eigcm),nmax,1,nmax,1,iout)
      endif 
     ...
     3               trunc,prnt,drctv,type,dirslv)
 20   continue   
