...
c***end prologue       linsys
     ...
     2                  trunc,prnt,drctv,type,dirslv,precon)
      implicit integer (a-z)
      real*8 ham, rhs
      real*8 matrix, xold, err, b, btmp, sol
      real*8 xcur, errcur
      ...
      if(dirslv) then
         ...
         call sgefa(matrix,n,n,ipvt,info)
         do 10 i=1,m
            call sgesl(matrix,n,n,ipvt,rhs(1,m),0)
 10      continue   
...
c                     
c
c           precondition and form new rhs and guess
c
            call precnd(matrix,rhs(1,num),xold,n,precon)
            if(prnt) then
               title='initial guess'
