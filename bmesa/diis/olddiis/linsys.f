...
c***end prologue       linsys
     ...
     2                  trunc,prnt,drctv,type)
      implicit integer (a-z)
      real*8 ham, rhs, matrix, xold, err, b, btmp, sol
      real*8 todiis, cnverg, error, test, crit
      real*8 xcur, errcur, energy, norm0
      character*80 title
      logical drctv, prnt, flag, switch, type
...
c
         call copy(rhs(1,num),xold(1,1),n)
         if(prnt) then
            title='initial guess'
            call prntrm(title,xold(1,1),n,1,n,1,iout)
         endif
         crit=todiis
         test=crit
         begin=1
...
c            
               if(test.le.crit) then
                  call diis(xcur,errcur,xold,err,b,btmp,sol,norm0,test,
     1                      cnverg,ipvt,begin,nobj,nerr,trunc,
     2                      maxit,prnt,flag)
                  if(flag) then
                     begin=1
                     call copy(xcur,xold(1,begin),nobj)
                  else
                     begin=begin+1
                     call copy(xcur,xold(1,begin),nobj)   
                  endif
...
c
     ...
     1                  n,type,prnt)
            call copy(xcur,xold(1,begin),n)
 10      continue
         write(iout,6) iter
         return
 20      write(iout,5) iter
         title='converged solution'
         call prntrm(title,xcur,n,1,n,1,iout)
 1000 continue  
      return
