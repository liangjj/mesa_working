*deck linslv
      subroutine linslv (oper,status,a1,a2,a3,a4,v1,v2,v3,ia,n,m,nmax,
     1                   nvec,prdir,schmdt)
...
c***                   saxpy(clams)
      ...
      real*8  ovtol, cnverg, delmax
      real*8 vv, sdot, diff, sum, zero, one, mone
...
c
          ...
          prnt(8)=prdir(8)          
c
          write (iout,1) mxiter, cnverg, ovtol, iout, nrhs, ntrial,
     1                   slndir
c
c----------------------------------------------------------------------c
...
c
              ...
              if (prnt(1).or.prnt(2)) then
                  ...
                  call prntrm(title,a1(1,nstart),n,ntrial,n,ntrial,iout)
              endif
              call gschmt(a1,ovtol,n,nstart,nfin,nout,schmdt)
              if(nfin.eq.0) then
                 title='final vectors'
                 call prntrm(title,a1(1,nstart),n,ntrial,n,ntrial,iout)
...
c
              if (prnt(1).or.prnt(4)) then
                  title='small matrix'
                  call prntrm(title,a3,nvec,nvec,m,nvec,iout) 
                  title='small rhs'
                  call prntrm(title,v1,nvec,nrhs,m,nrhs,iout)
              endif
c----------------------------------------------------------------------c
...
c----------------------------------------------------------------------c
             ...
             if(slndir.eq.'matrix') then
                do 190 irhs=1,nrhs
                   diff=0.d+00
                   do 200 i=1,n
                      sum=sdot(nvec,a2(i,1),n,v1(1,irhs),1)
     1                               -v3(i,irhs)
                      diff=diff+sum*sum
  200              continue
                   diff=sqrt(diff)
  ...
  190           continue     
             elseif (slndir.eq.'one-plus-matrix') then
                 do 210 irhs=1,nrhs
                    diff=0.d+00
                    do 220 i=1,n
                       sum=sdot(nvec,a1(i,1),n,v1(1,irhs),1) +
     1                     sdot(nvec,a2(i,1),n,v1(1,irhs),1) - 
     2                                  v3(i,irhs)
                       diff=diff+sum*sum
  220               continue
                    diff=sqrt(diff)
...
c             call sgemm('n','n',n,nrhs,nvec,one,a1,n,v1,m,zero,v3,n)
c             if (prnt(1).or.prnt(8)) then
c                 title='best solutions'
c                 call prntrm(title,v3,n,nrhs,n,nrhs,iout)  
c             endif         
...
c----------------------------------------------------------------------c
                 ...
                 if (prnt(1).or.prnt(7)) then
                     ...
                     call prntrm(title,v3,n,nrhs,n,nrhs,iout)  
                 endif
             endif
      elseif(oper.eq.'new trials') then
             ...
             if(prnt(1).or.prnt(6)) then
                ...
                call prntrm(title,a1,n,ntrial,n,ntrial,iout)
             endif   
...
c
    1 format(/,5x,'initialize linslv:',/,5x,
     ...
     6            '      number of trial vectors      = ',i4,/,5x,
     7            '      form of equations            = ',a32)
    2 format(/,5x,a32)   
    3 format(5x,'i = ',i3,2x,'j = ',i3,2x,'overlap = ',e15.8)   
    4 format (/,1x,25('-'),'cycle ',i3,' using ',i4,' vectors ',10('-'),
     1 /,t3,'solution',t30,'convergence')
    5 format (t4,i4,t26,e15.8)
    6 format(/,1x,'converged. number of vectors = ',i4)                      
    7 format (//,15x,'*****  error in call to linslv  *****')
      end
