...
c***end prologue       rdiis
      subroutine rdiis(pvec,dlpvec,vec,dvec,b,btmp,sol,error,ipvt,iter,
     1                 n,maxit)
      implicit integer (a-z)
      real*8 pvec, dlpvec, vec, dvec, b, btmp, sol
      real*8 zero, one, mone
      real*8 sdot
      real*8 sum, error, xerror
      save xerror
      dimension pvec(n,maxit+1), dlpvec(n,maxit+1), vec(n), dvec(n)
...
c        *****  extrapolate the objects:                        *****
         call ebcxx(vec,pvec,sol,n,iter,1,n,n,maxit+1)
         call ebcxx(dvec,dlpvec,sol,n,iter,1,n,n,maxit+1)
         xerror = sqrt(sdot(n,dvec,1,dvec,1))
...
c        *****  may have gone south.                            *****
 ...
 2    format(/,'begin rdiis extrapolation')
