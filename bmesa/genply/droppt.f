...
c***end prologue       droppt
      ...
      if(fl.eq.0.and.fr.eq.0) then
         ...
         elseif(nfix(1).and.nfix(2)) then
            ...
            call lnkerr('error in call to droppt')
         endif
      endif
 1    format(/,5x,'neither boundary a nodal point',/,5x,
     1            '             or ',/,5x,
     2            'basis has nodal structure built in.',/,5x,
     3            '      no functions dropptped')
 2    format(/,5x,'dropptping function at left endpoint')
 3    format(/,5x,'dropptping function at right endpoint')
 4    format(/,5x,'dropptping function at left and right endpoint')
