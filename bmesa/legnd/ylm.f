c $Header: ylm.f,v 1.6 93/01/20 20:58:07 bis Exp $
*deck ylm.f
...
c***end prologue       m6201
      ...
      character*4096 ops
      character*80 title
      logical logkey, prnt, prntlm, posinp, tstnrm, tstint, extra
      real*8 z, rms
      dimension z(1)
...
c----------------------------------------------------------------------c
     ...
     1                  lmax,mu,maxfac)
            wds=nth(ns)*(lmax+1-mu)
            wds1=nph(ns)+nph(ns)
            if (prnt) then
                title='p(l,m) functions for m = '//itoc(mu)
                lentit=length(title)
                len=lmax-mu+1
                call prntfm(title,z(plm),nth(ns),len,nth(ns),len,
     1                      iout)
...
c
                call scalfn(z(plm),z(wtthe+nth(ns)),nth(ns),lmax,mu)
                call scalfn(z(phifn),z(wtphi+nph(ns)),nph(ns),num,1)
                call legint(z(plm),z(phifn),z(scr),z(scr),
...
c           easily vectorized.
c           write out scaled functions for later use.
c 
            call scalfn(z(plm),z(wtthe),nth(ns),lmax,mu)
