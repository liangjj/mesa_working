...
c                      u(n,n) = transformation matrix from dvr to h0
c                               representation. 
c***routines called    
c***end prologue       preslv
      subroutine preslv(dinv,v,vecold,vecnew,u,t1,t2,
     1                  ndvr,nh0,nvc,prnt)
      implicit integer (a-z)
      character*80 title
      logical prnt
      real*8 dinv, v, vecold, vecnew, u
      real*8 t1, t2
      dimension dinv(ndvr), v(ndvr,ndvr), vecold(ndvr,nvc)
      dimension vecnew(ndvr,nvc), u(ndvr,nh0)
   ...
   20    continue
   10 continue
      call ebtc(t2,u,t1,nh0,ndvr,nvc)
      do 30 i=1,nh0
         do 40 j=1,nvc
            t2(i,j) = dinv(i)*t2(i,j)
 40      continue
 30   continue   
      call ebc(vecnew,u,t2,ndvr,nh0,nvc)
      if(prnt) then
