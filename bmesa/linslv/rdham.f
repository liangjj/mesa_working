...
c***end prologue       rdham
      subroutine rdham(hbuf,ibuf,diag,q3,lenbuf,matdim)
     ...
     2               ibuf,0,' ')
         call iosys('read integer "'//itoc(q3)//'d buffers" from '//
     1              'lamdat without rewinding',wptoin(lenbuf),
     2               hbuf,0,' ')
         do 30 j=1,lenbuf
 ...
 20   continue   
      if(left.ne.0) then
         call iosys('read integer "'//itoc(q3)//'d buffers" from '//
     1              'lamdat without rewinding',2*left,ibuf,0,' ')
         call iosys('read integer "'//itoc(q3)//'d buffers" from '//
     1              'lamdat without rewinding',wptoin(left),hbuf,0,' ')
         do 40 j=1,left
            write(iout,3) ibuf(1,j),ibuf(2,j), hbuf(j)
 40      continue   
      endif
