...
c***end prologue       dvdddat
     ...
     1                  thresh,niter,nvec,lenbuf,cgrid,prbufh,
     2                  hamd,n0,prdvd,dvdall,filham)
      implicit integer (a-z)
      real*8 cnverg, thresh, fpkey
      character*128 filham
      character*(*) card, cpass
      logical dollar, logkey, cgrid, prbufh, hamd, prdvd, dvdall
      dimension prdvd(11)      
      common/io/inp, iout
     ...
     1                 min(1000000,n*n),' ')
         ...
         prdvd(2)=logkey(card,'print=davidson=vectors',
     1                   .false.,' ')
         prdvd(3)=logkey(card,'print=davidson=h-on-vectors',
     1                   .false.,' ')
         prdvd(4)=logkey(card,'print=davidson=hamiltonian',
     1                   .false.,' ')
         prdvd(5)=logkey(card,'print=davidson=iteration-'//
     1                        'information',.false.,' ')
         prdvd(6)=logkey(card,'print=davidson=residuals',
     1                   .false.,' ')
         prdvd(7)=logkey(card,'print=davidson=transformed-vectors',
     1                  .false.,' ')
         prdvd(8)=logkey(card,'print=davidson=transformed-h-'//
     1                   'on-vectors',.false.,' ')               
         prdvd(9)=logkey(card,'print=davidson=new-trial-vectors',
     1                   .false.,' ')
         prdvd(10)=logkey(card,'print=davidson=overlaps',
     1                   .false.,' ')
