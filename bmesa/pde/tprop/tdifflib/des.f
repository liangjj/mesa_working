*deck des
      subroutine des ( neq, t, y, tout, info, rtol, atol, idid,
     ...
     +   h01, h02, h03, eig1, eig2, eig3, v0, vt, tpert,
     +   efield, omega, width, shift, hbar, dim, nmax, n)
...
c
c***see also  ddeabm
c***routines called  r1mach, dintp, dsteps, xermsg
...
c
      ...
      real*8 a, absdel, alpha, atol, beta, r1mach,
...
c
      if (info(1) .ne. 0  .and.  info(1) .ne. 1) then
         ...
         call xermsg ('diffeqlib', 'ddes', 'in ddeabm, info(1) must '//
     *      'be set to 0 for the start of a new problem, and must '//
     *      'be set to 1 following an interrupted task.  you are ' //
...
c
      if (info(2) .ne. 0  .and.  info(2) .ne. 1) then
         ...
         call xermsg ('diffeqlib', 'ddes', 'in ddeabm, info(2) must '//
     *      'be 0 or 1 indicating scalar and vector error '//
     *      'tolerances, respectively.  you have called the code '//
     *      'with info(2) = ' //xern1, 4, 1)
         idid=-33
         call lnkerr('quit')
      endif
c
      if (info(3) .ne. 0  .and.  info(3) .ne. 1) then
         ...
         call xermsg ('diffeqlib', 'ddes', 'in ddeabm, info(3) must '//
     *      'be 0 or 1 indicating the interval or '//
...
c
      if (info(4) .ne. 0  .and.  info(4) .ne. 1) then
         ...
         call xermsg ('diffeqlib', 'ddes', 'in ddeabm, info(4) must '//
     *      'be 0 or 1 indicating whether or not the integration ' //
...
c
      if (neq .lt. 1) then
         write (xern1, '(i8)') neq
         call xermsg ('diffeqlib', 'ddes', 'in ddeabm,  the number '//
     *      'of equations neq must be a positive integer.  you have ' //
...
c
      ...
      do 90 k=1,neq
         if (nrtolp .eq. 0 .and. rtol(k) .lt. 0.d0) then
            ...
            call xermsg ('diffeqlib', 'ddes', 'in ddeabm, the '//
     *         'relative error tolerances rtol must be non-negative.'//
     *         '  you have called the code with '//
     *         'rtol(' // xern1 // ') = ' // xern3 // 
...
c
         if (natolp .eq. 0 .and. atol(k) .lt. 0.d0) then
            ...
            call xermsg ('diffeqlib', 'ddes', 'in ddeabm, the '//
     *         'absolute error tolerances atol must be non-negative.'//
     *         ' you have called the code with  '//
     *         'atol(' // xern1 // ') = ' // xern3 // '.  in the '//
     *         'case of vector error tolerances, no further checking '//
...
c
  100 if (info(4) .eq. 1) then
     ...
     1      .or. abs(tout-t) .gt. abs(tstop-t)) then
            ...
            call xermsg ('diffeqlib', 'ddes', 'in ddeabm, you have ' //
...
c
         if (init .ne. 1) then
            if (delsgn*(tout-t) .lt. 0.d0) then
               write (xern3, '(1pe15.6)') tout
               call xermsg ('diffeqlib', 'ddes', 'in ddeabm, by ' //
...
c
      if (idid .eq. (-33)) then
         if (iquit .ne. (-33)) then
            ...
            call xermsg ('diffeqlib', 'ddes', 'in ddeabm, invalid ' //
...
c
  210 init=1
      a=t
      call df(a,y,yp,rpar,ipar,neq,
     +        h01,h02,h03,eig1,eig2,eig3,v0,vt,tpert,efield,omega,width,
     +        shift,hbar,dim,nmax,n)
...
c
  250 if(abs(x-t) .lt. absdel) go to 260
      call dntp(x,yy,tout,y,ypout,neq,kold,phi,ivc,iv,kgi,gi,
     1                                       alpha,g,w,xold,p)
      idid = 3
...
c
  ...
  270   y(l) = yy(l) + dt*yp(l)
      call df(tout,y,ypout,rpar,ipar,neq,
     +        h01,h02,h03,eig1,eig2,eig3,v0,vt,tpert,efield,omega,width,
     +        shift,hbar,dim,nmax,n)
      idid = 3
      t = tout
...
c                       relative error criterion inappropriate
  ...
  370   ypout(l) = yp(l)
      ...
      return
c
  380 call dstps(neq,yy,x,h,eps,wt,start,hold,kord,kold,crash,phi,p,
     1          yp,psi,alpha,beta,sig,v,w,g,phase1,ns,nornd,ksteps,
     2          twou,fouru,xold,kprev,ivc,iv,kgi,gi,rpar,ipar,
     3          h01,h02,h03,eig1,eig2,eig3,v0,vt,tpert,
     4          efield,omega,width,shift,hbar,dim,nmax,n)
c
c.......................................................................
c
