*deck dstrt
      subroutine dstrt ( neq, a, b, y, yprime, etol, morder, small,
     +   big, spy, pv, yp, sf, h, rpar, ipar, 
...
c
      ...
      real*8 a, absdx, b, big, da, delf, dely,
     1      dfdub, dfdxb, dhvnm,
...
c
     ...
     1                    100.0d0*small*abs(a)),dx)
         ...
         call df(a+da,y,sf,rpar,ipar,neq,
     +           h01,h02,h03,eig1,eig2,eig3,v0,vt,tpert,efield,omega,
     +           width,shift,hbar,dim,nmax,n)
         do 10 j = 1, neq
            yp(j) = sf(j) - yprime(j)
...
c                                       values.
         dely = relper*dhvnm(y,neq)
         if (dely .eq. 0.0d0) dely = relper
         dely = sign(dely,dx)
         delf = dhvnm(yprime,neq)
         fbnd = max(fbnd,delf)
...
c                                    in computing one estimate
               call df(a+da,pv,yp,rpar,ipar,neq,
     +                 h01,h02,h03,eig1,eig2,eig3,v0,vt,tpert,
     +                 efield,omega,width,shift,hbar,dim,nmax,n)
               do 90 j = 1, neq
                  pv(j) = yp(j) - sf(j)
