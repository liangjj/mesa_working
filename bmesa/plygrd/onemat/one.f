...
c***source             one
c***purpose            matrix elements of first and second
c***                   derivatives in dvr basis.
c
c***references       
c
...
c     1                     nmax(j),nmax(j),iout)
               v=1
               ham=v+nmax(j)
               call memory(words,p2,ngot2,'ham',0)
     ...
     1                 y(ham),y(v),hbar,amass,nmax(j),fl,fr,qtyp,
     2                 jc,.false.)
 ...
 10      continue
     ...
     1                  0,0,0,title)
            do 40 j=1,nsubg
               ...
               zddp=zdp+npt(j)*npt(j)
               words=wpadti(zddp+npt(j)*npt(j))
               call memory(words,p1,ngot1,'time',0)
               call iosys('read real '//title//' from bec',npt(j),
     1                     z(zq),q(j)-1,' ')
               call iosys('read real '//title//' from bec',npt(j),
     1                     z(zwt),wt(j)-1,' ')
...
c     1                                 nmax(j),nmax(j),iout)  
               v=1
               ham=v+nmax(j)
               words=wpadti(ham+nmax(j)*nmax(j)*2)
               call memory(words,p2,ngot2,'hamt',0)
