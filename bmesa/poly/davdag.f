...
c*********************************************************************c
         call iosys('does "davidson files" exist on '//file,0,0,
     1               0,truth)
         if(truth.eq.'no') then
            call iosys('create real "davidson eigenvalues" on '//file,
     1                  n,0,0,' ')
            call iosys('create real "davidson eigenvectors" on '//file,
     1                  n*n,0,0,' ')
            call iosys('create real "davidson iterates" on '//file,
     1                  n*n,0,0,' ')
            call iosys('create real "hamiltonian on davidson '//
     1                 'iterates" on '//file, n*n,0,0,' ')
         endif
         call iosys('write character "davidson files" to '//file,
     1               0,0,0,'yes')                       
...
c*********************************************************************c
c          
            if(status.eq.'diagonal') then
               call diaslv(resid(1,pt),root(pt),d,n)
            elseif(status.eq.'gauss-seidel') then
               call seidel(resid(1,pt),d,hv,root(pt),cvn,n,i5)
            elseif(status.eq.'tridiagonal') then
               call tridag(resid(1,pt),d,hv,root(pt),cvn,n)
            elseif(status.eq.'exact') then
               call exact(resid(1,pt),d,hv,root(pt),cvn,ipvt,n)
            else
               call lnkerr('error in approximate solver')
            endif 
c
c*********************************************************************c
...
c
      elseif(oper.eq.'cleanup') then
             nvecs=i3
             call iosys('write integer "number of davidson '//
     1                  'iterates" to '//file,1,nvecs,0,' ')
             call iosys('write real "davidson iterates" to '//file,
     1                  n*nvecs,v,0,' ')            
             call iosys('write real "hamiltonian on davidson '//
     1                  'iterates" to '//file,n*nvecs,hv,0,' ')
             call iosys('write real "davidson eigenvalues" '//
     1                  'to '//file,nroots, root,0,' ')
             call iosys('write real "davidson eigenvectors" '//
     1                  'to '//file,n*nroots,v,0,' ')
c
c                       'finish'
c
