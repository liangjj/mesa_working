...
c
      ...
      logical prgues, prvec, toau, useau, hamd, incore
...
c 
      do 50 i=1,2
         ...
         if(.not.itdiag) then
            ...
            if(calnls) then
               psi=words
               fpsi=psi+n3d*(maxit+1)
               fmat=fpsi+n3d*(maxit+1)
               err=fmat+n3d*n3d
               bb=err+n3d*n3d
 ...
 60   continue
     ...
     2             numdim,prnh0,search,ops)  
      ...
      if(calnls) then
         if(.not.itdiag) then
            if(numdim.eq.1) then
               call nlschr(z(fmat),z(eigtot),z(psi),z(fpsi),
     1                     z(vnl),z(pn(1)),dum,dum,
     2                     z(eigc(1)),dum,dum,
     3                     z(ham(1)),dum,dum,z(vtot),
     4                     z(bb),z(bbtmp),z(hh),z(pp),z(ph),z(hp),
     5                     z(sol),z(work(1)),z(work(2)),
     6                     table,ia(ipvt),natmin,natmax,
     7                     hbar,amass,omegat,scatl,todiis,cnvrg,n3d,
     8                     maxit,nstep,number,numdim,nmax,trunc,
     9                     prdiis,dodiis)
            elseif(numdim.eq.2) then
               call nlschr(z(fmat),z(eigtot),z(psi),z(fpsi),
     ...
     1                    ndum,n3d,nmax,numdim,prnht,.true.)
            elseif(numdim.eq.2) then
