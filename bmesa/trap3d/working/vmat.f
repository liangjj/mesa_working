...
c***end prologue       vmat
     ...
     1                n1,n2,n3,numdim,prnt,search,ops,parity)
      implicit integer (a-z)
      real*8 eigc1, eigc2, eigc3, v, work
      real*8 hbar, mass, scale, chrg
...
c
c
c        if model potential is coulomb the form is:
c
c             v = -z/r
c

      if(search.eq.'look for v0') then
         if( dollar('$v0',card,cpass,inp) ) then
             pot1=chrkey(card,'unperturbed-potential-type','none',' ')
             if(parity) then
                if(pot1.eq.'exponential') then
                   write(iout,7)
                   call lnkerr('quit')
                endif
             endif                      
             ...
             chrg(1,1)=fpkey(card,'charge',-1.d0,' ')
      ...
      elseif(search.eq.'look for v1') then
         if( dollar('$v1',card,cpass,inp) ) then
 ...
 540           continue
      ...
      elseif(pot1.eq.'coulomb') then
         do 600 i=1,numdim
            write(iout,8) i, chrg(i,i)
 600     continue            
         do 610 i=1,n1
            work(i,1) = chrg(1,1)/eigc1(i)
 610     continue                  
         if(numdim.le.1) then
            call copy(work(1,1),v,n1)
            return
         ...
         do 620 i=1,n2
            work(i,2) = chrg(2,2)/eigc2(i)
 620     continue
         if(numdim.le.2) then
            call v2d(v,work,n,n1,n2)
         elseif(numdim.eq.3) then
            do 630 i=1,n3
               work(i,3)= chrg(3,3)/eigc3(i)
 630        continue   
            call v3d(v,work,n,n1,n2,n3)
         else
            call lnkerr('error in number of dimensions')    
 ...
 6    format(/,5x,'off diagonal well constants:',/,5x,
     ...
     2                                                       /,5x, 
     3            '                           vij = ',e15.8)
 7    format(/,5x,'form of interaction not consistent with use of '
     1            'parity')
 8    format(/,5x,'diagonal charge:',/,5x,
     1            '               i = ',i1,/,5x,
     2            '               zii = ',e15.8)                  
      end       
