...
! ** this is the beginning of a new congfiguration
         ...
         IF (ConfigCount > 0) THEN
            WRITE (UNIT = 6, FMT = "('Configurations found ', i10)") ConfigCount
            ...
            ConfigLoop2:  DO
               ...
               END IF
               IF (iConf /= iConfig) STOP 'Bad configuration count'
               DO
...
! ** this is the beginning of a new congfiguration
                  ...
                  IF (ANY(ConfigOcc(:,:,:,iConf) > 1) .OR. ANY(ConfigOcc(:,:,:,iConf) < 0)) THEN
                     WRITE (UNIT = 6, FMT = "('Bad configuration ', i8)") iConf
                     STOP
                  END IF
               END DO
            END DO ConfigLoop2
            ...
            END DO
            WRITE (UNIT = 6, FMT = "('Reference Configuration ', a)") TRIM(ConfigLab(1))
         END IF
         CYCLE MainLoop
      END IF
      IF (LineIn(1:23) /= '     ------------------') THEN
         ...
         DO j = 1, NKeep
            IF (Row(j, iCol) /= 0) THEN
               IF (ConfigCount > 0 .AND. Row(j, iCol) <= ConfigCount) THEN
                  WRITE (UNIT = 6, FMT = "(i7, 2x, f12.7, 2x, a)") Row(j, iCol), Val(j, iCol), &
