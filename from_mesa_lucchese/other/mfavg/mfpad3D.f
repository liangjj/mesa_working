...
c Starting loop on phi of photo-e => perad, ppe(phi); (0~2Pi) ###################
      do 888 iphi=1,ipts1
c        pephi=((dble(iphi)/dble(ipts1))+0.0001d0)*359.9999d0
         pephi=(iphi-1)*3+.0001d0
         if(iphi.eq.ipts1)pephi=359.9999
c shift phi angle of electron so that it is measured relative to phi angle of
c the polarization vector
c***************************
c         pephi=pephi+phi
c***************************
c        ppe(iphi)= pephi*pi/180.
        perad=pephi*pi/180.
c Starting loop on theta of photo-e => theta, th(ipts); (0~Pi) ##################
        do 1000 ithe=1,ipts
        cpkp=cos(perad)
        spkp=sin(perad)
c          theta=((dble(ithe)/dble(ipts))+0.0001d0)*179.9999d0
           theta=(ithe-1)*3+.0001d0
           if(ithe.eq.ipts)theta=179.9999
c          th(ithe)= theta*pi/180.
...
c  ***********
          ...
          s=sin(theta)
          c=cos(theta)
c redefine electron angles
          cnew=s*spkp
          snew=sqrt(1.d0-cnew**2)
          cpnew=s*cpkp/snew
          spnew=-c/snew
          write(6,*)c,s,cpkp,spkp
          write(6,*)cnew,snew,cpnew,spnew
          sss=asin(snew)
          th(ithe)=sss
          ccc=acos(cpnew)
          ppe(iphi)=ccc
          write(6,*)sss,ccc
c          if((ppe(iphi).lt.0.d0))then
c            if(asin(spnew).lt.0.d0)ppe(iphi)=ppe(iphi)+1.570796
c          endif
          c=cnew
          s=snew
          cpkp=cpnew
