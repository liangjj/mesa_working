...
c
      ...
      logical debug
      parameter (half=0.5d+00,one=1.0d+00,two=2.0d+00,debug=.false.)
c
      common/io/inp,iout
c
...
c
      do 70 m=0,mmax
         ...
         call vfill(xyz(1,0,0,1,m),f(1,m),n)
         call vfill(xyz(1,0,0,2,m),f(1,m),n)
...
c              this generates all (a,0) integrals
         ...
         do 170 li=0,imax-1
            do 160 m=0,mmax-li-lj-1
               call vwxy(xyz(1,li+1,lj,coord,m),xyz(1,li+1,lj,coord,m),
     $                   expon,xyz(1,li,lj,coord,m+1),+1,n)
               if(li.gt.0) then
                  scalri=float(li)*half
                  call smul(t,ainv(1,1),scalri,n)
                  call vwxy(xyz(1,li+1,lj,coord,m),
     $                      xyz(1,li+1,lj,coord,m),
     $                      t,xyz(1,li-1,lj,coord,m),+1,n)
                  call vmul(t,t,ainv(1,4),n)
                  call vwxy(xyz(1,li+1,lj,coord,m),
     $                      xyz(1,li+1,lj,coord,m),
...
c              this generates all (a,b) integrals
         ...
         do 200 lj=0,jmax-1
            do 190 li=0,imax
c              i am undoubtedly doing too much work here.
c              what should the maximum m be?
               do 180 m=0,mmax-li-lj-1
