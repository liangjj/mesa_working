...
c   changes denoted by c..bhl to skip subroutine calls if
c   no integrals past the screening of the prefactors
c
c   17 may 1986        pws at lanl
c   modification of driver from m312.
c
c***keywords
c***author             saxe, paul    (lanl)
c***source             @(#)driver.f	1.1   11/30/90
c***purpose
c***description
c***references         (none)
c
...
c
      ...
      real*8 cutexp,pi252
      real*8 cont(ncont),ints(lenbuf),cutoff
      integer acore(*),labels(*)
...
c
c      call       sizer1(ptprim,noprim,nbtype,nocont,ptcont,ex,cont,
c     #                  ncont,c,ints,labels,lenbuf,start,nx,ny,nz,
c     #                  lenxyz,nobf,nocart,mintyp,minmom,maxmom,
c     #                  z,maxcor,nat,num,nnp,nprim,ops,cutexp,need1)
c
c      call       sizer2(ptprim,noprim,nbtype,nocont,ptcont,ex,cont,
c     #                  ncont,c,ints,labels,lenbuf,start,nx,ny,nz,
c     #                  lenxyz,nobf,nocart,mintyp,minmom,maxmom,
c     #                  z,maxcor,nat,num,nnp,nprim,ops,cutexp,need2)
c
c     ----- check that can get enough core -----
c
      call getscm(0,z,canget,'m313:driver',0)
...
c    #         '       maximum core useable is:',i9)
      if (need1.gt.canget) call lnkerr('not enough core available'//
     #          'for m313')
c
...
c     ----- get the memory -----
c
      call getscm(needed,z,maxcor,'m313:driver',0)
c
      cencod=-1
c
...
c
      ...
      ints(1)=-numbuf
      call iosys('write real "unsorted derivative integrals" on ints '
     #     //'without rewinding',lenbuf,labels,0,' ')
      call iosys('write real "unsorted derivative integrals" on ints '
     #     //'without rewinding',lenbuf,ints,0,' ')
      call iosys('endfile "unsorted derivative integrals" on ints',
     $     0,0,0,' ')
c
