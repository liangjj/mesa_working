...
c.sun      data mask10 /o'1777'/
c.cray     data mask10 /1023/
c.cray     data mask10 /1777b/
c
c     ----- statement functions for sun -----
c
c..bhl..unicos      shiftl(i,j)=lshift(i,j)
c..bhl..unicos      shiftr(i,j)=rshift(i,j)
c
c     call lnkerr(' m333: sort32: no shift function')
c
c     ----- start timing -----
c
      nnpsq=nnp**2
...
c
         ...
         shift2=shiftl(1,20)
         do 2 i=2,n
            iork(i)=and(shiftr(labels(1,i),10),mask10)
    ...
    7    continue
         do 9 i=2,n
            labels(1,i)=shiftr(labels(1,i),20)*nnpsq-nnpsq
    9    continue
         do 23 i=1,n-1
            labs(i)=(ijv(i+1)-1)*nnp+klv(i+1)+labels(1,i+1)
            vals(i)=values(i+1)
   23    continue
         junk=n-1
         do 24 i=1,n-1
            labs(i+junk)=(klv(i+1)-1)*nnp+ijv(i+1)+labels(1,i+1)
