      subroutine mcdnkk(txk,xk,hess,den,bii,bij,
...
c
            ...
            factk=four*deg(mk)
c
            call mckac(dab(lden1),hess(lh),xk,naok,mrs,itran,
     $           thrsh,factk)
            factk=-deg(mk)
            call mcktac(dab(lden1),hess(lh),xk,naok,nbr,nbs,itran,
     $           thrsh,factk)
c
            go to 490
c
...
c
            ...
            call mcstv(hess(lh),xk,bii(mk),mrs)
c
            go to 490
c
 485        continue
c
            ...
            call mcstv(hess(lh), xk, bij(mk,mk),mrs)
            call mcstvt(hess(lh),xk,bijt(mk,mk),nbr,nbs)
c
 490        continue
c
c--------------------------------------
...
c
      do 650 l=1,nol
         ...
         do 640 k=1,nok
            ka=noba(lnoba(mk)+k)
            iread=iread+1
            if(iread.le.iijt) go to 601
            lxk=1
            iread=1
            call mcrdjk(txk,nbufk,ndab,kst,jkcore)
 601        continue
            call scopy(knij,txk(iread),kmkl,xk,1)
c
c
            kala=ka+la
...
cc
            nstart=nstart+1
            call mcdxaa(den(nd),xk,nij,mrs,nstep,itran,thrsh,
     $           hess(lhaa),nstart,ntot)
cc
            go to 640
 610        continue
...
c
            ...
            factk=-deg(ml)
            call mckac(dab(lden),hess(lh),xk,naok,mrs,itran,
     $           thrsh,factk)
            go to 640
 615        continue
            factk=four*deg(ml)
            call mckac(dab(lden),hess(lh),xk,naok,mrs,itran,thrsh,
     $           factk)
            go to 640
c
 620        continue
            ...
            factk=-deg(mk)
            call mckac(dab(lden),hess(lh),xk,naol,mrs,itran,
     $           thrsh,factk)
            go to 640
 625        continue
            factk=four*deg(mk)
            call mckac(dab(lden),hess(lh),xk,naol,mrs,itran,thrsh,
     $           factk)
            go to 640
c
 630        continue
...
c-------------------------c
            call mcstv(hess(lh),xk,bijt(mk,ml),mrs)
            go to 640
 635        continue
c-------------------------c
c   iijj symmetry block
c-------------------------c
            call mcstv(hess(lh),xk, bij(mk,ml),mrs)
c
 640     lxk=lxk+nrs
 650  continue
 1000 continue
