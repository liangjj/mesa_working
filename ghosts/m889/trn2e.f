...
c..bhl
      mostor=.false.
      if (.not.toguga.or.
     $    logkey(ops,'ciopt',.false.,' ').or.
     $    logkey(ops,'mrciopt',.false.,' ')) then
          mostor=.true.
...
c    2       continue
   ...
   23          continue
   22       continue
            if(ij.ne.numij) call lnkerr('ij.ne.numij in trn2e')
...
c
     ...
     #        0,0,0,'guga integrals','ints',.false.)
      end if
c..bhl
      if (mostor) then
