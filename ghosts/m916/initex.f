...
c***********************************************************************
  318 z=val1*(int(arr+tr1)+val2*int(arr+tr2))
      do 8033 ivc=1,nvc
      call saxpy(n1,z,c(ia,ivc),1,s(ja,ivc),1)
      call saxpy(n1,z,c(ja,ivc),1,s(ia,ivc),1)
 8033 continue
      return
c
c***********************************************************************
...
c...     dir.$ ivdep
c      do 3999 i=1,n1
c            s(jj,ivc)=s(jj,ivc)+z*c(ii,ivc)
c            s(ii,ivc)=s(ii,ivc)+z*c(jj,ivc)
c            ii=ii+1
c 3999       jj=jj+1
c
      return
...
c   zz   arrival,for completeness
      call lnkerr('  stop  zz arrival in external')
      end
