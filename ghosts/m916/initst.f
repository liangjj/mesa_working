...
c
      ...
      nkl=nklxx(is+1,n)
c
       iqs=max(jming,mn(js+1))
       iqe=min(jmaxg,mn(js+1)+numsym(js+1)-1)
c
      do 1008 ivc=1,nvc
      jja=ja
c
...
c
      ...
      ipt=ist+i
         do 1009 ivc=1,nvc
            ...
            xqx=0.0
            do 421 j=jinf,jsup
               xqx=xqx+int(ipt)*c(jja,ivc)
               ipt=ipt-nkloff
               jja=jja+1
  421       continue
         s(iia,ivc)=s(iia,ivc)+xqx*val1
 1009    continue
      iia=iia+1
  422 continue
      return
...
c
   98 continue
c      write (6,99) iseg
c   99 format (//,' ***** error in xx entry, iseg=',i5,//)
      call lnkerr(' stop error in xx entry ')
c
c********************************** ww *********************************
c
      entry ww
...
c
  198 continue
c      write (6,199) iseg
c  199 format (//,' ***** error in ww entry, iseg=',i5,//)
      call lnkerr(' stop error in ww entry ')
c      stop
c
c******************************* wx ************************************
c
...
c
  ...
  299 format (//,' ***** error in wx entry, iseg=',i5,//)
      call lnkerr(' stop error in wx entry ')
c      stop
c
c******************************* xw ************************************
c
...
c
  398 continue
c      write (6,399) iseg
c  399 format (//,' ***** error in xw entry, iseg=',i5,//)
      call lnkerr(' stop error in xw entry ')
c      stop
c
      end
