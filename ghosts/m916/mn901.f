...
c
      cnverg=fpkey(ops,'ci=convergence',cnverg,' ')
c
...
c
      iout=itape6
      write(iout,*)' fci901: nuclear repulsion energy ',rep
c
...
c
      call roots(z(ints),z(vector),z(sigma),z(root),z(dvdvec),z(dvdmat),
     #              ndvdmx,prtflg)
c
      call iosys('destroy bliu',0,0,0,' ')
c
cc final timing values are returned here
c      write (itape6,891) icount,ixx4,ixx5,ixx6,ixx8,iww4,iww5,iww6,iww8
...
c    #,       f10.3,f7.2))
cc
c     write (itape6,893) (i-1,ipgs(i),ipgd(i),ipgo(i),ipgij(i),ipgout(i)
c    #,                   i=nlevs,2,-1)
