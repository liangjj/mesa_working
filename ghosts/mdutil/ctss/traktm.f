...
c***source             @(#)traktm.f	1.1   11/30/90
...
c
  ...
  204 continue
      ...
      pt=0
      nrtn=0
      call charges(cpusys,io1,mem1)
      call timing(cpu1,sys1,iomem)
      return
c
  300 continue
      if (dead) return
...
c
      call charges(cpusys,io,mem)
      call timing(cpu,sys,iomem)
      if (pt.gt.0) then
  ...
  301    format (/,' ##### to many levels for stak in timing:',i5,/)
      ...
      stak(5,pt)=0
      stak(6,pt)=0
      call charges(cpusys,oldio,oldmem)
      call timing(oldcpu,oldsys,iomem)
      return
c
  400 continue
      if (dead) return
