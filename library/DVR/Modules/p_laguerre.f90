...
!***references
!***routines called
!***end prologue       p_laguerre
!\begin{eqnarray}
!\end{eqnarray}
  SUBROUTINE p_laguerre(p_momr,fa,fb,dfb,wt,n,region)
...
!
  ...
  END DO
  DO i=1,n
     p_momr(i,i) = p_momr(i,i) - half * fa(i,i) * wt(i) * fb(i,i)
  END DO
  scale= .5D0
  p_momr = scale*p_momr
  IF(prn(3)) THEN
     title='unnormalized first derivative matrix for '// 'region = '//itoc(region)
     CALL prntrm(title,p_momr,n,n,n,n,iout)
  END IF
END SUBROUTINE p_laguerre
