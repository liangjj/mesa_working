...
!***end prologue       dvr_input
  ...
  INTEGER                          :: intkey, lenth, len, len_1
  INTEGER                          :: i
...
!
  keywrd='$'//keywrd(1:len)//'('//coord(1:len_1)//')'
...
!
      ...
      END IF
      prloc(12)=chrkey(card,'sector_print',secprn,' ')
      prn(12)=.false.
      IF(prloc(12) == 'sector_details') then
         prn(12)=.true.
      END IF
...
!    
        ...
        endpts(1)=edge(1)
        endpts(2)=edge(2)
        n(1)=intkey(card,'polynomial_order',1,' ')
        typwt='one'
        reuse=logkey(card,'reuse_time_data',.false.,' ')
