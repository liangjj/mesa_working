...
!***end prologue       lobatto
  ...
  SUBROUTINE lobatto(pt_0,pt_n,pt,wt,f,df,ddf,ke,p_mom,eigv_0,eigvec_0,h,eigv, &
...
!
    ...
    ELSE IF(typwt == 'laguerre' ) then
        ...
        CALL p_laguerre (mat(p_momr(i)),grid(pr(i)),grid(pr(i)), &
                         grid(dpr(i)),grid(wtr(i)),npt(i),i,unit_weight)
    ELSE IF(typwt == 'r'.or.typwt == 'cylindrical' ) then
        CALL ke_cyl(mat(tr(i)),grid(pr(i)),grid(dpr(i)), grid(ddpr(i)),   &
                    grid(qr(i)),grid(wtr(i)),coord,npt(i),i,nreg,unit_weight)
    ELSE IF(typwt == 'spherical' ) then
        CALL ke_spherical(mat(tr(i)), grid(dpr(i)), grid(qr(i)),          &
                          grid(wtr(i)), coord, npt(i), i, nreg, unit_weight)
    ELSE IF(typwt == 'laguerre' ) then
        CALL ke_laguerre(mat(tr(i)),grid(pr(i)),grid(pr(i)), &       
                         grid(dpr(i)),grid(ddpr(i)),          &
                         grid(wtr(i)),npt(i),i,unit_weight)
