...
c       clog(1+z) = cmplx  (alog(r), c_arg(1+z))
c                 = cmplx  (0.5*alog(r**2), c_arg(1+z))
c                 = cmplx  (0.5*alnrel(2*x+rho**2), c_arg(1+z))
c***references  (none)
c***routines called  alnrel,c_arg,r1mach,xerror
...
c
      x = real(z)
      clnrel = dcmplx (0.5d0*alnrel(2.d0*x+rho**2), c_arg(1.0d0+z))
c
      return
      end
