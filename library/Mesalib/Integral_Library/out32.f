*deck @(#)out32.f	5.1  11/6/94
      subroutine out32(conint,nconti,ncontj,ncontk,ncontl,lenblk,
     #                  symcen,angmom,start,nat,nbtype,nnp,ints,labels,
     #                  lenbuf,numbuf,ncint,flbl,numint,
     #                  plbls,num,nobf,nactul,drop,pkindx)
c***begin prologue     out32
c***date written       840807  (yymmdd) 
c***revision date      910702  (yymmdd) 
c
c   2 july     1991    rlm at lanl
c      adding capability to drop components of basis functions ala bhl.
c***keywords           
c***author             saxe, paul (lanl) 
c***source             @(#)out32.f	5.1   11/6/94
...
c
      if (drop) then
c
c        drop components given by pkindx.
         do 410 i=1,numint
         if (abs(conint(i)).lt.cutoff) go to 410
            io=shiftr(plbls(1,i),10)
            jo=and(plbls(1,i),mask10)
            ko=shiftr(plbls(2,i),10)
            lo=and(plbls(2,i),mask10)
c
