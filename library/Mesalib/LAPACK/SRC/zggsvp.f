...
*
         CALL ZLAZRO( P, P, CZERO, CZERO, V, LDV )
         IF( P.GT.1 )
...
*
   ...
   40 CONTINUE
      IF( P.GT.L )
     $   CALL ZLAZRO( P-L, N, CZERO, CZERO, B( L+1, 1 ), LDB )
*
      IF( WANTQ ) THEN
*
*        Set Q = I and Update Q := Q*P
*
         CALL ZLAZRO( N, N, CZERO, CONE, Q, LDQ )
...
*        Clean up B
*
         CALL ZLAZRO( L, N-L, CZERO, CZERO, B, LDB )
         DO 60 J = N - L + 1, N
            DO 50 I = J - N + L + 1, L
               B( I, J ) = CZERO
   50       CONTINUE
...
*
         CALL ZLAZRO( M, M, CZERO, CZERO, U, LDU )
         IF( M.GT.1 )
...
*
  ...
  100 CONTINUE
      IF( M.GT.K )
     $   CALL ZLAZRO( M-K, N-L, CZERO, CZERO, A( K+1, 1 ), LDA )
*
      IF( N-L.GT.K ) THEN
*
...
*        Clean up A
*
         CALL ZLAZRO( K, N-L-K, CZERO, CZERO, A, LDA )
