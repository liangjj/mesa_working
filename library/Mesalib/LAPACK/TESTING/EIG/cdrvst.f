...
*          The unitary matrix of eigenvectors computed by CHEEV,
*          CHEEVX, CHPEV, CHPEVX, CHBEV, and CHBEVX.
*          Modified.
*
...
*     .. External Subroutines ..
      EXTERNAL           CHBEV, CHBEVX, CHEEV, CHEEVX, CHET21, CHET22,
     $                   CHPEV, CHPEVX, CLACPY, CLATMR, CLATMS, CLAZRO,
     $                   SLABAD, SLAFTS, ALASVM, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
            DO 940 IUPLO = 0, 1
               ...
               END IF
*
*              Call CHEEV and CHEEVX.
*
               CALL CLACPY( ' ', N, N, A, LDA, V, LDU )
*
               NTEST = NTEST + 1
               CALL CHEEV( 'V', UPLO, N, A, LDU, D1, WORK, NWORK, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 2
               CALL CHEEV( 'N', UPLO, N, A, LDU, D3, WORK, NWORK, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 1
               CALL CHPEV( 'V', UPLO, N, WORK, D1, Z, LDU, V, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
...
*
               NTEST = NTEST + 2
               CALL CHPEV( 'N', UPLO, N, WORK, D3, Z, LDU, V, RWORK,
     $                     IINFO )
               IF( IINFO.NE.0 ) THEN
