...
*     (2)     | A' VL - VL W | / ( n |A| ulp )
*
*       Here VL is the matrix of unit left eigenvectors, A' is the
...
*     .. External Subroutines ..
      EXTERNAL           DGEEV, DGET22, DLABAD, DLACPY, DLASUM, DLATME,
     $                   DLATMR, DLATMS, DLAZRO, XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
     ...
     $                      ZERO, ANORM, 'NO', A, LDA, IWORK, IINFO )
               IF( N.GE.4 ) THEN
                  CALL DLAZRO( 2, N, ZERO, ZERO, A, LDA )
                  CALL DLAZRO( N-3, 1, ZERO, ZERO, A( 3, 1 ), LDA )
                  CALL DLAZRO( N-3, 2, ZERO, ZERO, A( 3, N-1 ), LDA )
                  CALL DLAZRO( 1, N, ZERO, ZERO, A( N, 1 ), LDA )
               END IF
*
