...
*
     ...
     $         CALL DCOPY( 3*N-2, A, 1, AF, 1 )
            SRNAMT = 'DGTTRF'
            CALL DGTTRF( N, AF, AF( N ), AF( 2*N ), AF( 3*N-1 ), IWORK,
     $                   INFO )
*
...
*
     ...
     $                      1, -1, IMAT, NFAIL, NERRS, NOUT )
            TRFCON = INFO.NE.0
*
            CALL DGTT01( N, A, A( N ), A( 2*N ), AF, AF( N ), AF( 2*N ),
...
*
                  ANORM = DLANGT( NORM, N, A, A( N ), A( 2*N ) )
*
...
*
   ...
   40                CONTINUE
                     X( I ) = ONE
                     CALL DGTTRS( TRANS, N, 1, AF, AF( N ), AF( 2*N ),
...
*
               ...
               SRNAMT = 'DGTTRS'
               CALL DGTTRS( TRANS, N, NRHS, AF, AF( N ), AF( 2*N ),
     $                      AF( 3*N-1 ), IWORK, X, LDA, INFO )
*
...
*
               ...
               CALL DGTT02( TRANS, N, NRHS, A, A( N ), A( 2*N ), X, LDA,
...
*
               SRNAMT = 'DGTRFS'
               CALL DGTRFS( TRANS, N, NRHS, A, A( N ), A( 2*N ), AF,
...
*
               CALL DGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                      RESULT( 4 ) )
               CALL DGTT05( TRANS, N, NRHS, A, A( N ), A( 2*N ), B, LDA,
...
*
               IF( ITRAN.LE.2 ) THEN
                  SRNAMT = 'DGTCON'
                  CALL DGTCON( NORM, N, AF, AF( N ), AF( 2*N ),
