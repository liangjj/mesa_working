...
*
               IF( N.GT.1 ) THEN
                  ...
                  CALL DCOPY( N-1, AF( 3 ), 3, A( 2*N ), 1 )
               END IF
               CALL DCOPY( N, AF( 2 ), 3, A( N ), 1 )
            ELSE
*
...
*
               ELSE IF( IFACT.EQ.1 ) THEN
                  IF( N.GT.0 )
     $               CALL DCOPY( 3*N-2, A, 1, AF, 1 )
*
*                 Compute the 1-norm and infinity-norm of A.
*
                  ANORMO = DLANGT( '1', N, A, A( N ), A( 2*N ) )
                  ANORMI = DLANGT( 'I', N, A, A( N ), A( 2*N ) )
*
*                 Factor the matrix A.
*
                  CALL DGTTRF( N, AF, AF( N ), AF( 2*N ), AF( 3*N-1 ),
     $                         IWORK, INFO )
*
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL DGTTRS( 'No transpose', N, 1, AF, AF( N ),
...
*
   ...
   50                CONTINUE
                     X( I ) = ONE
                     CALL DGTTRS( 'Transpose', N, 1, AF, AF( N ),
     $                            AF( 2*N ), AF( 3*N-1 ), IWORK, X, LDA,
     $                            INFO )
                     AINVNM = MAX( AINVNM, DASUM( N, X, 1 ) )
   60             CONTINUE
...
*
                  CALL DLAGTM( TRANS, N, NRHS, ONE, A, A( N ), A( 2*N ),
     $                         XACT, LDA, ZERO, B, LDA )
*
...
*
                     SRNAMT = 'DGTSV '
                     CALL DGTSV( N, NRHS, AF, AF( N ), AF( 2*N ), X,
     $                           LDA, INFO )
*
...
*
     ...
     $                               LDA )
                        CALL DGTT02( TRANS, N, NRHS, A, A( N ),
...
*
                  SRNAMT = 'DGTSVX'
                  CALL DGTSVX( FACT, TRANS, N, NRHS, A, A( N ),
...
*                    residual.
*
                     CALL DGTT01( N, A, A( N ), A( 2*N ), AF, AF( N ),
...
*
                     ...
                     CALL DGTT02( TRANS, N, NRHS, A, A( N ), A( 2*N ),
...
*
                     CALL DGTT05( TRANS, N, NRHS, A, A( N ), A( 2*N ),
