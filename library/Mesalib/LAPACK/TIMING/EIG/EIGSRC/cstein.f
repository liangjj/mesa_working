...
*  =======
*
*  CSTEIN computes the eigenvectors of a symmetric tridiagonal
*  matrix corresponding to specified eigenvalues using inverse
*  iteration.  Although the eigenvectors are real, they are stored
*  in a complex array, which may be passed to CUNMTR for the back
*  transformation.
*
*  Arguments
*  =========
...
*          On output, the real part of Z contains the orthogonal
*          eigenvectors associated with the specified eigenvalues.  Any
*          vector which fails to converge is set to its value on the
*          fifth iteration.  The complex part of Z is set to zero.
*
*  LDZ     (input) INTEGER
...
*     .. Parameters ..
      ...
      PARAMETER          ( ZERO = 0.0, ONE = 1.0, TEN = 1.0E1,
...
*
            CALL SLARNV( 3, ISEED, BLKSIZ, WORK( INDRV1+1 ) )
*
*           Increment opcount for getting random starting vector.
*           ( SLARND(3,.) requires 23 flops. )
*
            OPS = OPS + BLKSIZ*24
*
