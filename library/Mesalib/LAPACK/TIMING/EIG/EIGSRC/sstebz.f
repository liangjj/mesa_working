...
*
     ...
     $           LOG( TWO ) ) + 2
         ...
         END IF
*
*        Increment opcount for determining Gershgorin intervals
...
*
   ...
   30    CONTINUE
*
*        Increment opcount for estimating the norm of T.
*
         OPS = OPS + 2*( N-1 )
*
         IF( ABSTOL.LE.ZERO ) THEN
...
*
*           Increment opcount for computing Gershgorin interval.
*
            OPS = OPS + 4*( IEND-IBEGIN ) + 14
*
            IF( IRANGE.GT.1 ) THEN
               GL = MAX( GL, WL )
               GU = MIN( GU, WU )
...
*
   ...
   60       CONTINUE
*
*           Increment opcount for taking midpoints as eigenvalues.
*
            OPS = OPS + 2*IOUT
*
            M = M + IM
