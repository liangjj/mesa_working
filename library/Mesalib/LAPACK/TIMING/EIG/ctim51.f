...
*          10: CTGEVC(JOB='A', SIDE='L')               'CTGEVC(A,L)'
*          11: CTGEVC(JOB='B', SIDE='L')               'CTGEVC(B,L)'
*          12: CTGEVC(JOB='A', SIDE='R')               'CTGEVC(A,R)'
*          13: CTGEVC(JOB='B', SIDE='R')               'CTGEVC(B,R)'
...
*     .. Data statements ..
     ...
     $                   'CTGEVC(A,L)', 'CTGEVC(B,L)', 'CTGEVC(A,R)',
     $                   'CTGEVC(B,R)', 'CQZHES(F)', 'CQZHES(T)',
...
*
                     IF( TIMSUB( 10 ) ) THEN
                        ...
                        S1 = SECOND( )
  550                   CONTINUE
                        CALL CTGEVC( 'A', 'L', LLWORK, N, A, LDA, B,
...
*
  ...
  560                   CONTINUE
                        ...
                        CALL CTGEVC( 'B', 'L', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 12 ) ) THEN
                        ...
                        S1 = SECOND( )
  580                   CONTINUE
                        CALL CTGEVC( 'A', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  590                   CONTINUE
                        ...
                        CALL CTGEVC( 'B', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, RWORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  820                CONTINUE
                     ...
                     CALL SLACPY( 'Full', N, N, TR, LDH, BR, LDA )
                     CALL SLACPY( 'Full', N, N, TI, LDH, BI, LDA )
                     CALL SLAZRO( N, N, ZERO, ONE, QR, LDA )
                     CALL SLAZRO( N, N, ZERO, ONE, QI, LDA )
                     CALL CQZVAL( LDA, N, AR, AI, BR, BI, ZERO, WR,
...
*
                     ...
                     DO 830 J = 1, IC
                        ...
                        CALL SLACPY( 'Full', N, N, TR, LDH, ZR, LDA )
                        CALL SLACPY( 'Full', N, N, TI, LDH, ZI, LDA )
                        CALL SLAZRO( N, N, ZERO, ONE, ZR, LDA )
                        CALL SLAZRO( N, N, ZERO, ONE, ZI, LDA )
  830                CONTINUE
                     S2 = SECOND( )
                     UNTIME = S2 - S1
*
  ...
  840          CONTINUE
            ELSE IF( RUNQZ ) THEN
               ...
               CALL SLAZRO( N, N, ZERO, ONE, QR, N1 )
               CALL SLAZRO( N, N, ZERO, ONE, QI, N1 )
