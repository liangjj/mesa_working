...
*  GEP:   6 N**2 + 3*N
*
*  MAXN is currently set to 400 and LWORK = MAXN*(4*MAXN+2) in the
*  parameter statements below.  For SVD, we assume NRHS may be as big
...
*     .. Parameters ..
      INTEGER            MAXN, LDAMAX
      PARAMETER          ( MAXN = 400, LDAMAX = MAXN+20 )
      INTEGER            NEED
      PARAMETER          ( NEED = 6 )
      INTEGER            LIWRK2
      PARAMETER          ( LIWRK2 = MAXN*7 )
      INTEGER            LWORK
...
*     .. Local Arrays ..
     ...
     $                   LDAVAL( MAXPRM ),
     $                   MVAL( MAXIN ), MXBVAL( MAXPRM ), MXTYPE( 4 ),
     ...
     $                   RWORK( 4*MAXN ), RWORK1( MAXIN*MAXT*MAXPRM ),
     $                   WORKR( LWORK, 2 )
...
*
     ...
     $                DR( 1, 3 ), DR( 1, 4 ), E2, A( 1, 2 ),
     $                AR( 1, 3 ), AR( 1, 4 ), D( 1, 1 ), DR( 1, 1 ),
     $                A( 1, 3 ), AR( 1, 5 ), AR( 1, 6 ), WORK, LWORK,
     $                RWORK1, LOGWRK, IWORK2, RESULT, MAXPRM, MAXT,
...
*
 ...
 9985 FORMAT( / ' LAPACK VERSION 1.1, released March 31, 1993 ' )
*
*     End of CTIMEE
*
      END
