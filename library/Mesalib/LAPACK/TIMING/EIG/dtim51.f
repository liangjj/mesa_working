...
*          10: DTGEVC(JOB='A', SIDE='L')               'STGEVC(A,L)'
*          11: DTGEVC(JOB='B', SIDE='L')               'STGEVC(B,L)'
*          12: DTGEVC(JOB='A', SIDE='R')               'STGEVC(A,R)'
*          13: DTGEVC(JOB='B', SIDE='R')               'STGEVC(B,R)'
...
*     .. External Subroutines ..
      EXTERNAL           ATIMIN, DHGEQZ, DLACPY, DLAQZH, DLARFG, DLATM4,
     $                   DLAZRO, DORM2R, DPRTBG, DTGEVC, QZHES, QZIT,
     $                   QZVAL, QZVEC, XLAENV
*     ..
*     .. Intrinsic Functions ..
...
*     .. Data statements ..
     ...
     $                   'DTGEVC(A,L)', 'DTGEVC(B,L)', 'DTGEVC(A,R)',
     $                   'DTGEVC(B,R)', 'QZHES(F)', 'QZHES(T)',
     $                   'QZIT(F)', 'QZIT(T)', 'QZVEC' /
...
*
                     IF( TIMSUB( 10 ) ) THEN
                        ...
                        S1 = DSECND( )
  550                   CONTINUE
                        CALL DTGEVC( 'A', 'L', LLWORK, N, A, LDA, B,
...
*
  ...
  560                   CONTINUE
                        ...
                        CALL DTGEVC( 'B', 'L', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 12 ) ) THEN
                        ...
                        S1 = DSECND( )
  580                   CONTINUE
                        CALL DTGEVC( 'A', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  590                   CONTINUE
                        ...
                        CALL DTGEVC( 'B', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  810                CONTINUE
                     CALL DLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL DLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL DLAZRO( N, N, ZERO, ONE, Q, LDA )
...
*
                     ...
                     DO 820 J = 1, IC
                        CALL DLACPY( 'Full', N, N, H, LDH, Z, LDA )
                        CALL DLACPY( 'Full', N, N, T, LDH, Z, LDA )
                        CALL DLAZRO( N, N, ZERO, ONE, Z, LDA )
  820                CONTINUE
                     S2 = DSECND( )
                     UNTIME = S2 - S1
*
