...
*          EISPACK:
*            7: HTRIDI (compare with ZHETRD)
*            8: IMTQL1 (compare w/ ZSTEQR -- VECT='N')
*            9: IMTQL2+HTRIBK (compare w/ ZUNGTR+ZSTEQR(VECT='V') )
...
*     .. External Subroutines ..
      EXTERNAL           ATIMIN, DCOPY, DLAZRO, DPRTBE, DSTEBZ, HTRIBK,
     $                   HTRIDI, IMTQL1, IMTQL2, XLAENV, ZHETRD, ZLACPY,
     $                   ZLATMS, ZPTEQR, ZSTEIN, ZSTEQR, ZUNGTR, ZUNMTR
*     ..
*     .. Intrinsic Functions ..
...
*
  ...
  520                CONTINUE
                     CALL DCOPY( N, D, 1, RWORK, 1 )
                     CALL DCOPY( N-1, E, 1, RWORK( LDA+1 ), 1 )
                     CALL DLAZRO( N, N, ZERO, ONE, ZRE, LDA )
