...
c
c     parameter (maxcor=1)
      parameter (maxcor=4000000)
c
      character*(*) name
      logical called
      logical debug
...
c        z(offset).
c        call malloc(maxsiz*wptbyt(1),locf)
c        locf=malloc(maxsiz*wptbyt(1))
c        locz=loc(z(1))
c        offset=(locf-locz)/wptbyt(1)
         offset=0
c
c        if (locf.eq.0) then
c           write(iout,*) 'problem getting ',maxsiz,' real*8 words'
c           call lnkerr(' malloc could not get the core requested.')
c        end if
c
         ioff=offset+1
         ngot=wptoin(maxsiz)
