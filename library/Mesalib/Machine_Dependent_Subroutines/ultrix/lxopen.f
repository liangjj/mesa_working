...
c           first get the default library names and append the process id
c           to keep one jobs files separate from another
      call getenv('TMPDIR',tmp)
      ltmp=cskipb(tmp,' ')
...
c
     ...
     #      form='formatted',err=99,status='old')
      open (unit=iout,file=names(2),organization='sequential',
     #      access='append',form='formatted',err=100,
     $      status='unknown')
c
