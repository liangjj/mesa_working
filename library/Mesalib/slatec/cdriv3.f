...
c***end prologue  cdriv3
     ...
     8     g, glast, gnow, h, hmax, hold, hsign, hused, nround, rc, re,
...
c                                       reset ywt array.  looping point.
 ...
 290    if (iwork(ijtask) .eq. 0) then
     ...
     8                  work(isave2), t, h, real(work(iel)), impl, npar,
     8                  ndecom, iflag)
...
c
c     call cdstp (eps, f, fa, hmax, impl, ierror, jacobn, matdim,
...
c
     ...
     8           ndecom, work(iywt), uround, users,  avgh, avgord, h,
     8           hused, iwork(ijtask), iwork(imntld), iwork(imtrld),
     8           iwork(infe), iwork(inje), iwork(inquse), iwork(instep),
     8           t, y, work(iyh),  work(ia), convrg, work(idfdy), el,
     8           work(ifac), hold, iwork(indpvt), jstate, iwork(ijstpl),
...
c                                                  select the first root
          ...
          troot = t + h
          do 510 i = 1,nroot
            if (real(work(i+itroot-1))*hsign .lt. troot*hsign) then
