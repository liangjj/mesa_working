      SUBROUTINE CGGBAK( JOB, SIDE, N, ILO, IHI, LSCALE, RSCALE, M, E,
     $                   LDE, INFO )
*
...
*  =======
*
*  CGGBAK forms the right or left eigenvectors of the generalized
*  eigenvalue problem by backward transformation on the computed
*  eigenvectors of the balanced matrix output by CGGBAL.
*
*  Arguments
*  =========
*
...
*
*  E       (input/output) COMPLEX array, dimension (LDE,M)
...
*
         IF( RIGHTV ) THEN
            DO 10 I = ILO, IHI
               CALL CSSCAL( M, RSCALE( I ), E( I, 1 ), LDE )
   10       CONTINUE
         END IF
*
...
*
         IF( LEFTV ) THEN
            DO 20 I = ILO, IHI
               CALL CSSCAL( M, LSCALE( I ), E( I, 1 ), LDE )
   20       CONTINUE
         END IF
      END IF
*
...
*
     ...
     $         GO TO 50
            DO 40 I = ILO - 1, 1, -1
               ...
               IF( K.EQ.I )
     $            GO TO 40
               CALL CSWAP( M, E( I, 1 ), LDE, E( K, 1 ), LDE )
   40       CONTINUE
*
   50       CONTINUE
            IF( IHI.EQ.N )
