...
*     .. External Subroutines ..
      EXTERNAL           CAXPY, CCOPY, CGEMV, CGGRQF, CUNMQR, CUNMRQ,
     $                   CTRMV, CTRSV, XERBLA
*     ..
*     .. Intrinsic Functions ..
      INTRINSIC          MAX, MIN
...
*     Update c = Z'*c
*
      CALL CUNMQR( 'Left', 'Conjugate Transpose', M, 1, MIN( M, N ), A,
...
*     Update c(1:N-P)
*
      CALL CGEMV( 'No transpose', N-P, P, -ONE, A( 1, N-P+1 ), LDA, D,
     $            1, ONE, C, 1 )
*
...
*
      IF( M.GE.N ) THEN
         CALL CTRMV( 'Upper', 'No transpose', 'Non unit', P,
     $               A( N-P+1, N-P+1 ), LDA, D, 1 )
         CALL CAXPY( P, -ONE, D, 1, C( N-P+1 ), 1 )
      ELSE
         CALL CTRMV( 'Upper', 'No transpose', 'Non unit', M+P-N,
     $               A( N-P+1, N-P+1 ), LDA, D, 1 )
         CALL CAXPY( M+P-N, -ONE, D, 1, C( N-P+1 ), 1 )
         CALL CGEMV( 'No transpose', M+P-N, N-M, -ONE, A( N-P+1, M+1 ),
