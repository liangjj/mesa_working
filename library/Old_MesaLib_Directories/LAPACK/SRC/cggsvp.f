...
*  A23 is upper trapezoidal.  K+L = the effective rank of the (M+P)-by-N
*  matrix (A',B')'.  Z' denotes the conjugate transpose of Z.
*
...
*          to TOLA = MAX(M,N)*norm(A)*MACHEPS,
...
*     .. External Subroutines ..
      EXTERNAL           CGEQPF, CGEQR2, CGERQ2, CLACPY, CLAPMT, CLAZRO,
...
*
         CALL CLAZRO( P, P, CZERO, CZERO, V, LDV )
         IF( P.GT.1 )
...
*
   ...
   40 CONTINUE
      IF( P.GT.L )
     $   CALL CLAZRO( P-L, N, CZERO, CZERO, B( L+1, 1 ), LDB )
*
      IF( WANTQ ) THEN
*
...
*        Clean up B
*
         CALL CLAZRO( L, N-L, CZERO, CZERO, B, LDB )
         DO 60 J = N - L + 1, N
            DO 50 I = J - N + L + 1, L
               B( I, J ) = CZERO
   50       CONTINUE
...
*
         CALL CLAZRO( M, M, CZERO, CZERO, U, LDU )
         IF( M.GT.1 )
...
*
  ...
  100 CONTINUE
      IF( M.GT.K )
     $   CALL CLAZRO( M-K, N-L, CZERO, CZERO, A( K+1, 1 ), LDA )
*
      IF( N-L.GT.K ) THEN
*
...
*        Clean up A
*
         CALL CLAZRO( K, N-L-K, CZERO, CZERO, A, LDA )
