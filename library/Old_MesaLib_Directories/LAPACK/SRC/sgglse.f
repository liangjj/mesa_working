...
*     Update c = Z'*c
*
      CALL SORMQR( 'Left', 'Transpose', M, 1, MIN( M, N ), A, LDA,
...
*
      IF( M.GE.N ) THEN
         CALL STRMV( 'Upper', 'No transpose', 'Non unit', P,
     $               A( N-P+1, N-P+1 ), LDA, D, 1 )
         CALL SAXPY( P, -ONE, D, 1, C( N-P+1 ), 1 )
      ELSE
         CALL STRMV( 'Upper', 'No transpose', 'Non unit', M+P-N,
     $               A( N-P+1, N-P+1 ), LDA, D, 1 )
         CALL SAXPY( M+P-N, -ONE, D, 1, C( N-P+1 ), 1 )
         CALL SGEMV( 'No transpose', M+P-N, N-M, -ONE, A( N-P+1, M+1 ),
