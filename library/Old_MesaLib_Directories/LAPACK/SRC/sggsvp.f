...
*     .. External Subroutines ..
      EXTERNAL           SGEQPF, SGEQR2, SGERQ2, SLACPY, SLAPMT, SLAZRO,
...
*
         CALL SLAZRO( P, P, ZERO, ZERO, V, LDV )
         IF( P.GT.1 )
...
*
   ...
   40 CONTINUE
      IF( P.GT.L )
     $   CALL SLAZRO( P-L, N, ZERO, ZERO, B( L+1, 1 ), LDB )
*
      IF( WANTQ ) THEN
*
...
*        Clean up B
*
         CALL SLAZRO( L, N-L, ZERO, ZERO, B, LDB )
         DO 60 J = N - L + 1, N
            DO 50 I = J - N + L + 1, L
               B( I, J ) = ZERO
   50       CONTINUE
...
*
         CALL SLAZRO( M, M, ZERO, ZERO, U, LDU )
         IF( M.GT.1 )
...
*
  ...
  100 CONTINUE
      IF( M.GT.K )
     $   CALL SLAZRO( M-K, N-L, ZERO, ZERO, A( K+1, 1 ), LDA )
*
      IF( N-L.GT.K ) THEN
*
...
*        Clean up A
*
         CALL SLAZRO( K, N-L-K, ZERO, ZERO, A, LDA )
