...
*  maximum "absolute value" norm of a vector v  M(v).  *Exception*:
*  if a(i)=b(i)=0, then the eigenvector is identically zero.  The
*  normalization test is:
*
*          RESULT(2) =      max       | M(v(i)) - d(i) | / ( n ulp )
*                     eigenvectors v(i)
*
*               { 0   if a(i)=b(i)=0
*  where d(i) = {
...
*
   ...
   20    CONTINUE
         IF( ABS1( ALPHA( JVEC ) ).LE.SAFMIN .AND.
     $       ABS1( BETA( JVEC ) ).LE.SAFMIN ) THEN
            ENRMER = MAX( ENRMER, TEMP1 )
         ELSE
