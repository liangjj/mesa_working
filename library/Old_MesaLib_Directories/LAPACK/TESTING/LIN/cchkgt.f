...
*
     ...
     $         CALL CCOPY( 3*N-2, A, 1, AF, 1 )
            SRNAMT = 'CGTTRF'
            CALL CGTTRF( N, AF, AF( N ), AF( 2*N ), AF( 3*N-1 ), IWORK,
     $                   INFO )
*
...
*
     ...
     $                      1, -1, IMAT, NFAIL, NERRS, NOUT )
            TRFCON = INFO.NE.0
*
            CALL CGTT01( N, A, A( N ), A( 2*N ), AF, AF( N ), AF( 2*N ),
...
*
                  ANORM = CLANGT( NORM, N, A, A( N ), A( 2*N ) )
*
...
*
   ...
   40                CONTINUE
                     X( I ) = ONE
                     CALL CGTTRS( TRANS, N, 1, AF, AF( N ), AF( 2*N ),
...
*
               ...
               SRNAMT = 'CGTTRS'
               CALL CGTTRS( TRANS, N, NRHS, AF, AF( N ), AF( 2*N ),
     $                      AF( 3*N-1 ), IWORK, X, LDA, INFO )
*
...
*
               ...
               CALL CGTT02( TRANS, N, NRHS, A, A( N ), A( 2*N ), X, LDA,
...
*
               SRNAMT = 'CGTRFS'
               CALL CGTRFS( TRANS, N, NRHS, A, A( N ), A( 2*N ), AF,
...
*
               CALL CGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                      RESULT( 4 ) )
               CALL CGTT05( TRANS, N, NRHS, A, A( N ), A( 2*N ), B, LDA,
...
*
               IF( ITRAN.LE.2 ) THEN
                  SRNAMT = 'CGTCON'
                  CALL CGTCON( NORM, N, AF, AF( N ), AF( 2*N ),
