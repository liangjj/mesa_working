...
*
               IF( N.GT.1 ) THEN
                  ...
                  CALL CCOPY( N-1, AF( 3 ), 3, A( 2*N ), 1 )
               END IF
               CALL CCOPY( N, AF( 2 ), 3, A( N ), 1 )
            ELSE
*
...
*
               ELSE IF( IFACT.EQ.1 ) THEN
                  IF( N.GT.0 )
     $               CALL CCOPY( 3*N-2, A, 1, AF, 1 )
*
*                 Compute the 1-norm and infinity-norm of A.
*
                  ANORMO = CLANGT( '1', N, A, A( N ), A( 2*N ) )
                  ANORMI = CLANGT( 'I', N, A, A( N ), A( 2*N ) )
*
*                 Factor the matrix A.
*
                  CALL CGTTRF( N, AF, AF( N ), AF( 2*N ), AF( 3*N-1 ),
     $                         IWORK, INFO )
*
...
*
   ...
   30                CONTINUE
                     X( I ) = ONE
                     CALL CGTTRS( 'No transpose', N, 1, AF, AF( N ),
...
*
                  CALL CLAGTM( TRANS, N, NRHS, ONE, A, A( N ), A( 2*N ),
     $                         XACT, LDA, ZERO, B, LDA )
*
...
*
                     SRNAMT = 'CGTSV '
                     CALL CGTSV( N, NRHS, AF, AF( N ), AF( 2*N ), X,
     $                           LDA, INFO )
*
...
*
     ...
     $                               LDA )
                        CALL CGTT02( TRANS, N, NRHS, A, A( N ),
...
*
                  SRNAMT = 'CGTSVX'
                  CALL CGTSVX( FACT, TRANS, N, NRHS, A, A( N ),
...
*                    residual.
*
                     CALL CGTT01( N, A, A( N ), A( 2*N ), AF, AF( N ),
...
*
                     ...
                     CALL CGTT02( TRANS, N, NRHS, A, A( N ), A( 2*N ),
...
*
                     CALL CGTT05( TRANS, N, NRHS, A, A( N ), A( 2*N ),
