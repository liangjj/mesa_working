      SUBROUTINE ZCHKPB( DOTYPE, NN, NVAL, NNB, NBVAL, NRHS, THRESH,
...
*     .. External Functions ..
      ...
      EXTERNAL           DGET06, ZLANGE, ZLANSB
*     ..
*     .. External Subroutines ..
      EXTERNAL           ALAERH, ALAHD, ALASUM, XLAENV, ZCOPY, ZERRPO,
     $                   ZGET04, ZLACPY, ZLARHS, ZLATB4, ZLATMS, ZLAZRO,
     $                   ZPBCON, ZPBRFS, ZPBT01, ZPBT02, ZPBT05, ZPBTRF,
     $                   ZPBTRS, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*
                     CALL ZLAZRO( N, N, DCMPLX( ZERO ), DCMPLX( ONE ),
     $                            AINV, LDA )
                     SRNAMT = 'ZPBTRS'
...
*
                     ANORM = ZLANSB( '1', UPLO, N, KD, A, LDAB, RWORK )
...
*
                     SRNAMT = 'ZPBTRS'
                     CALL ZPBTRS( UPLO, N, KD, NRHS, AFAC, LDAB, X, LDA,
     $                            INFO )
*
*                    Check error code from ZPBTRS.
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ZPBTRS', INFO, 0, UPLO, N,
...
*
                     IF( INFO.NE.0 )
     $                  CALL ALAERH( PATH, 'ZPBRFS', INFO, 0, UPLO, N,
     $                               N, KD, KD, NRHS, IMAT, NFAIL,
     $                               NERRS, NOUT )
*
                     CALL ZGET04( N, NRHS, X, LDA, XACT, LDA, RCONDC,
     $                            RESULT( 4 ) )
                     CALL ZPBT05( UPLO, N, KD, NRHS, A, LDAB, B, LDA, X,
     $                            LDA, XACT, LDA, RWORK,
