...
*     .. External Subroutines ..
      EXTERNAL           ZGEMM, ZGEQLF, ZHERK, ZLACPY, ZLAZRO, ZUNGQL
*     ..
*     .. Intrinsic Functions ..
...
*     Copy details of Q
*
      CALL ZLAZRO( M, M, ROGUE, ROGUE, Q, LDA )
      IF( M.GE.N ) THEN
...
*     Copy L
*
      CALL ZLAZRO( M, N, DCMPLX( ZERO ), DCMPLX( ZERO ), L, LDA )
      IF( M.GE.N ) THEN
         IF( N.GT.0 )
...
*     Compute I - Q'*Q
*
      CALL ZLAZRO( M, M, DCMPLX( ZERO ), DCMPLX( ONE ), L, LDA )
