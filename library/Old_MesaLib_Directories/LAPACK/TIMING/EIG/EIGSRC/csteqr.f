...
*  be found if CSYTRD or CSPTRD or CSBTRD has been used to reduce this
...
*     .. External Subroutines ..
      EXTERNAL           CLASR, CLAZRO, CSWAP, SLAE2, SLAEV2, SLARTG,
     $                   XERBLA
*     ..
*     .. Intrinsic Functions ..
...
*
*              Increment opcount for solving 2 by 2 eigenproblem.
*
               OPS = OPS + 22 + N*12
            ELSE
...
*
     ...
     $                  Z( 1, L ), LDZ )
*
*           Increment opcount for applying rotations.
*
            OPS = OPS + N*( MM-1 )*12
         END IF
*
...
*
  110    CONTINUE
*
*        Increment opcount for finding small superdiagonal element.
*
         IF( L.NE.LEND )
     $      OPS = OPS + 2*( L-MAX( M, LENDP1 )+1 )
         IF( M.GT.LEND )
     $      E( M-1 ) = ZERO
         P = D( L )
         IF( M.EQ.L )
