...
*  =======
*
*  CTGEVC computes selected left and/or right generalized eigenvectors
...
*          triangular with real diagonal entries.
*
*  LDB     (input) INTEGER
...
*             matrix found in VL on entry to CTGEVC.  Usually, this
*             will be the Q matrix computed by CGGHRD and CHGEQZ,
...
*             will be computed and multiplied (on the left) by the
*             matrix found in VR on entry to CTGEVC.  Usually, this
*             will be the Z matrix computed by CGGHRD and CHGEQZ,
...
*
                  DO 80 JR = JE, J - 1
                     SUMA = SUMA + CONJG( A( JR, J ) )*WORK( JR, 1 )
                     SUMB = SUMB + CONJG( B( JR, J ) )*WORK( JR, 1 )
   80             CONTINUE
...
*
               DO 170 JR = 1, JE - 1
                  WORK( JR,1 ) = ACOEFF*A( JR,JE ) - BCOEFF*B( JR,JE )
  170          CONTINUE
               WORK( JE, 1 ) = CONE
*
               DO 210 J = JE - 1, 1, -1
...
*
                     IF( ABS1( WORK( J, 1 ) ).GT.ONE ) THEN
                        TEMP = ONE / ABS1( WORK( J, 1 ) )
                        IF( ACOEFA*RWORK( J, 1 )+BCOEFA*RWORK( J, 2 )
     $                      .GE.BIGNUM*TEMP ) THEN
                           DO 190 JR = 1, JE
                              WORK( JR, 1 ) = TEMP*WORK( JR, 1 )
  190                      CONTINUE
