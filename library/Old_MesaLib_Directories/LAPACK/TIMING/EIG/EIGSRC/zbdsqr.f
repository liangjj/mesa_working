...
*     .. External Subroutines ..
      EXTERNAL           DLARTG, DLAS2, DLASV2, XERBLA, ZDROT, ZDSCAL,
     $                   ZLASR, ZSWAP
*     ..
*     .. Intrinsic Functions ..
...
*        Increment opcount for zero-shift QR sweep
         OPS = OPS + ( DBLE( M-LL )*( 18+6*( NCVT+NRU+NCC ) )+2 )
         IF( IDIR.EQ.1 ) THEN
*
...
*        Increment opcount for standard shifted QR sweep
         OPS = OPS + ( DBLE( M-LL )*( 30+6*( NCVT+NRU+NCC ) )+2 )
*
         IF( IDIR.EQ.1 ) THEN
*
*           Chase bulge from top to bottom
