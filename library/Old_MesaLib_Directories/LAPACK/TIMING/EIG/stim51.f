...
*          10: STGEVC(JOB='A', SIDE='L')               'STGEVC(A,L)'
*          11: STGEVC(JOB='B', SIDE='L')               'STGEVC(B,L)'
*          12: STGEVC(JOB='A', SIDE='R')               'STGEVC(A,R)'
*          13: STGEVC(JOB='B', SIDE='R')               'STGEVC(B,R)'
...
*     .. External Subroutines ..
     ...
     $                   SLACPY, SLAQZH, SLARFG, SLATM4, SLAZRO, SORM2R,
...
*     .. Data statements ..
     ...
     $                   'STGEVC(A,L)', 'STGEVC(B,L)', 'STGEVC(A,R)',
     $                   'STGEVC(B,R)', 'QZHES(F)', 'QZHES(T)',
     $                   'QZIT(F)', 'QZIT(T)', 'QZVEC' /
...
*
                     IF( TIMSUB( 10 ) ) THEN
                        ...
                        S1 = SECOND( )
  550                   CONTINUE
                        CALL STGEVC( 'A', 'L', LLWORK, N, A, LDA, B,
...
*
  ...
  560                   CONTINUE
                        ...
                        CALL STGEVC( 'B', 'L', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
                     IF( TIMSUB( 12 ) ) THEN
                        ...
                        S1 = SECOND( )
  580                   CONTINUE
                        CALL STGEVC( 'A', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  590                   CONTINUE
                        ...
                        CALL STGEVC( 'B', 'R', LLWORK, N, A, LDA, B,
     $                               LDA, H, LDA, T, LDA, N, ITEMP,
     $                               WORK, IINFO )
                        IF( IINFO.NE.0 ) THEN
...
*
  ...
  810                CONTINUE
                     CALL SLACPY( 'Full', N, N, H, LDH, A, LDA )
                     CALL SLACPY( 'Full', N, N, T, LDH, B, LDA )
                     CALL SLAZRO( N, N, ZERO, ONE, Q, LDA )
...
*
                     ...
                     DO 820 J = 1, IC
                        CALL SLACPY( 'Full', N, N, H, LDH, Z, LDA )
                        CALL SLACPY( 'Full', N, N, T, LDH, Z, LDA )
                        CALL SLAZRO( N, N, ZERO, ONE, Z, LDA )
  820                CONTINUE
                     S2 = SECOND( )
                     UNTIME = S2 - S1
*
