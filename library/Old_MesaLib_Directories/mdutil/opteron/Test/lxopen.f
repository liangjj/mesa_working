...
c               names(33)        'bec'        
c
c      new file added by bis for partitioning
c
c               names(34)        'hpart' 
c
...
c
      ...
      if (names(30).eq.'optint') names(30)=tmp(1:ltmp)//'/mesa.optint'
      if (names(31).eq.'atomci') names(31)=tmp(1:ltmp)//'/mesa.atomci'
      if (names(32).eq.'lamdat') names(32)=tmp(1:ltmp)//'/mesa.lamdat'
      if (names(33).eq.'bec')    names(33)=tmp(1:ltmp)//'/mesa.bec'
      if (names(34).eq.'hpart')  names(34)=tmp(1:ltmp)//'/mesa.hpart'
c
