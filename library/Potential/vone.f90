...
!***end prologue       vone
      ...
      if( dollar('$v0('//coord(1:lenkey)//')',card,atom,inp) ) &
         ...
         if(typ.eq.'harmonic_oscillator') then
!
...
!
      ...
      elseif(typ.eq.'well') then
         ...
         len=fpkey(card,'well_size',pt(nglobal),' ')
      ...
      elseif(typ.eq.'power_exponential') then
         amp(1)=fpkey(card,'amplitude',amp,' ')
         expnt(1)=fpkey(card,'exponent',expnt,' ')
         n_p=intkey(card,'power',0,' ')
         call v_pow_exp(v,pt,amp,expnt,n_p,nglobal,prnt)
      elseif(typ.eq.'sum_exponential') then
      ...
      elseif(typ.eq.'morse') then
         ...
         amp(3)=fpkey(card,'static_field',0.d0,' ')
         expnt(1)=fpkey(card,'first_exponent',2.d0/sqrt(8.d0*amp(1)),' ')
         expnt(2)=fpkey(card,'second_exponent',.5d0*expnt(1),' ')
