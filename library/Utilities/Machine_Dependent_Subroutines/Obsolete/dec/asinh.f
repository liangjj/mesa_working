...
c
c series for asnh       on the interval  0.          to  1.00000e+00
c                                        with weighted error   2.26e-32
c                                         log weighted error  31.65
c                               significant figures required  30.59
c                                    decimal places required  32.44
c
...
c
      ...
      nterms = initds (asnhcs, 39, 0.1*sngl(d1mach(3)) )
      sqeps = sqrt (d1mach(3))
      xmax = 1.0d0/sqeps
c
 10   y = abs(x)
...
c
      asinh = x
      if (y.gt.sqeps) asinh = x*(1.0d0 + dcsevl (2.d0*x*x-1.d0,
     1  asnhcs, nterms) )
      return
 20   if (y.lt.xmax) asinh = dlog (y + sqrt(y*y+1.d0) )
