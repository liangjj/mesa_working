...
!***end prologue       Legendre
      Subroutine Legendre (Fun,Leg)
      IMPLICIT NONE
      TYPE(Special_Functions)                 :: Fun 
      TYPE(LegPly)                            :: Leg
      INTEGER                                 ::  i
      INTEGER                                 ::  n_1
      INTEGER                                 ::  n_2
...
!                      and recur upward                                c
!----------------------------------------------------------------------c
      Fun%Leg%P_LM(:,m:l_max) = zero
      IF ( m == int_zero ) THEN 
         Fun%Leg%P_LM(:,m)=one
      ELSE
         Fun%Leg%P_LM(:,m) = ddfct(m) * (one - x(:)*x(:))**.5d+00*m
      END IF
      IF (l_max /= m) THEN
          Fun%Leg%P_LM(:,m+1) = ( m + m + int_one) * x(:) * Fun%Leg%P_LM(:,m)
          IF (l_max /= ( m + int_one)) THEN
              n_1 = int_two
              n_2 = m + m + int_three
              n_3 = n_2 - int_two
              DO i = m+int_two,l_max
                 Fun%Leg%P_LM(:,i) = ( n_2 * x(:) * Fun%Leg%P_LM(:,i - int_one) - n_3 * Fun%Leg%P_LM(:,i - int_two) ) / n_1
                 n_1 = n_1 + int_one
                 n_2 = n_2 + int_two
                 n_3 = n_3 + int_one
              END DO
