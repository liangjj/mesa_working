*deck @(#)classp.f	1.1 9/9/91
      subroutine classp(kind, n, alpha, beta, b, a, muzero)
c
...
c
      nm1 = n - 1
      if(kind.eq.'legendre'.or.kind.eq.'one') then
c
...
c
   ...
   11       b(i) = abi/ sqrt(4.d0*abi*abi - 1.0d0  )
         a(n) = 0.0d0
         return
      elseif(kind.eq.'chebyshev-1') then
c
...
c
   ...
   21       b(i) = 0.5d0
            ...
            a(n) = 0.0d0
            return
      elseif(kind.eq.'chebyshev-2') then
c
...
c
     ...
     1                        gamfun(alpha + 1.0d0  ) * 
     2                        gamfun( beta + 1.0d0  ) / gamfun(abi)
         a(1) = (beta - alpha)/abi
