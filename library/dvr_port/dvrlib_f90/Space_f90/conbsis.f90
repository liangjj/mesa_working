...
!
  USE dvr_global,    ONLY   : nreg, npt, output
  USE dvr_prnt
  IMPLICIT NONE
  INTEGER                            :: nglobal
...
!     calculate the needed functions
  ...
  DO  reg=1,nreg
      ...
      last = row + nfun - 1
      IF (prn(12)) then
          WRITE(output,1) reg, nfun, start, END, &
                        row, last, bridge
      END IF
...
!        from region i and region i + 1.
  ...
  IF(prn(1)) THEN
     ...
     CALL prntrm(title,x,nglobal,1,nglobal,1,output)
     title='global weights'
     CALL prntrm(title,xwt,nglobal,1,nglobal,1,output)
     title='global norm'
     CALL prntrm(title,norm,nglobal,1,nglobal,1,output)
  END IF
  IF(prn(2)) THEN
     title='global basis'
     CALL prntrm(title,f,nglobal,nglobal,nglobal,nglobal,output)
     title='1 derivative of global basis'
     CALL prntrm(title,df,nglobal,nglobal,nglobal,nglobal,output)
     title='2 derivative of global basis'
    CALL prntrm(title,ddf,nglobal,nglobal,nglobal,nglobal,output)
  END IF
