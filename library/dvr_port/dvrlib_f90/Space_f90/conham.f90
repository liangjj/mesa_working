...
!
      last=row+nfun-1
      IF(prn(12)) then
         WRITE(output,2) reg, nfun, start, END, row, last, bridge
      END IF
!
...
!
  IF(prn(8)) THEN
     ...
     CALL prntrm(title,kmat,nglobal,nglobal,nglobal,nglobal,output)
  END IF  
!
!     add the potential
!
   ...
   IF(prn(8)) THEN
      ...
      CALL prntrm(title,hmat,nglobal,nglobal,nglobal,nglobal,output)
      title='full first derivative matrix'
      CALL prntrm(title,pmat,nglobal,nglobal,nglobal,nglobal,output)
   END IF
