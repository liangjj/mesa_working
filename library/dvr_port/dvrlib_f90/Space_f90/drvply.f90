...
!     recursion relationship.
  ...
  SUBROUTINE drvply(q,wt,p,dp,ddp,edge,typwt,parity,angmom,nord,nq)
  USE dvr_global,       ONLY  : input, output, l_val, m_val
  USE dvr_prnt
  IMPLICIT NONE
  INTEGER                                :: nord
...
!
     CALL gaussq(typwt,nord,0.d0,0.d0,2,ptfix,b,q,wt)
     CALL cnvtpt(q,wt,edge,nord)
  ELSE IF(typwt == 'legendre') THEN
     IF(m_val == 0) THEN
        CALL gaussq(typwt,nord,0.d0,0.d0,0,ptfix,b,q,wt)  
     ELSE
        CALL gaussq(typwt,nord,0.d0,0.d0,2,ptfix,b,q,wt)  
     END IF
  ...
  ELSE IF(typwt == 'laguerre') THEN
     ptfix(1)=0.d0
     CALL gaussq(typwt,nord,0.d0,0.d0,1,ptfix,b,q,wt)  
  ELSE
!  
...
!  
     ALLOCATE(r(nq),rwt(nq),wtfn(nq),scr(nq),ply(nq,0:nord), &
              arg(nq),stat=err)
     if(err /= 0) then
        call lnkerr('allocation error')
     end if
...
!  
     ...
     IF(prn(1)) THEN
        ...
        CALL prntfm(title,r,nq,1,nq,1,output)
        title='reference weights'
        CALL prntfm(title,rwt,nq,1,nq,1,output)
     END IF
!  
!        Compute the weight function.
     ...
     IF(prn(2)) THEN
        ...
        CALL prntfm(title,wtfn,nq,1,nq,1,output)
     END IF
!  
...
!  
     ...
     WRITE(output,7) mu
!
!    Check to see if polynomials of a quadratic variable are requested
!
     IF(parity == 'none') THEN
        arg = r
     ELSE
...
!  
    ...
    CALL modab(a,b,2,endpts,nord)
    IF(prn(3)) THEN
        title='lanczos a coefficients'
        CALL prntfm(title,a,nord,1,nord,1,output)
        title='lanczos b coefficients'
        CALL prntfm(title,b,nord-1,1,nord-1,1,output)
    END IF
!  
...
!  
  ...
  IF(prn(4)) THEN
     title='final nodes'
     CALL prntfm(title,q,nord,1,nord,1,output)
     title='final weights'
     CALL prntfm(title,wt,nord,1,nord,1,output)
  END IF
!  
