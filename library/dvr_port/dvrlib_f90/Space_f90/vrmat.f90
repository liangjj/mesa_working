...
!
      ...
      elseif(typ == 'well') then
         ...
         len=fpkey(card,'well-size',pt(nr),' ')
         call vwell(v,pt,len,depth,nr,prnt)
...
!         
         ...
         if(units == 'atomic-units') then
            write(output,*) '   converting to atomic units'
            omega=omega*timau
            mass=mass/massau
            fac=mass*omega*omega*half
            hbar=one
         endif
         write(output,2) mass, omega
  ...
  if(prn(3)) then
     ...
     call prntrm(title,v,nr,1,nr,1,output)
  endif
