...
!***end prologue       fd_basis
  ...
  USE fd_prnt
  IMPLICIT NONE
  INTEGER                               :: nphy, nglobal, row, len, length
  CHARACTER(len=*)                      :: coord
  ...
  if(prn_fd_log(4)) then
     title='ke'
     call prntrm(title,ke,row,nphy,row,nphy,output)
     title='h'
     call prntrm(title,h,row,nphy,row,nphy,output)
  endif
  IF(.not.nodiag) then
