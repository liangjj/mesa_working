...
c
      common /io/inp, iout
c     
c      title='trial vectors before orthonormalization'
c      call prntrm(title,v,n,nfin,n,nfin,iout)
      ntrial=nstart-1
      nout=0      
      do 10 i=nstart,nfin
         norm=sqrt(ddot(n,v(1,i),1,v(1,i),1))
         if(norm.lt.thresh) go to 10
         call dscal(n,1.d0/norm,v(1,i),1)
         if(ntrial.ne.0) then
            do 20 j=1,ntrial
               ...
               if (norm.lt.thresh) go to 10
   20       continue
            call dscal(n,1.0d+00/norm,v(1,i),1)
         endif
         ntrial=ntrial+1                
         nout=nout+1
