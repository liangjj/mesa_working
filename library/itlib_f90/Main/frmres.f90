...
!***routines called
!***end prologue       frmres
  SUBROUTINE frmres(type)
  USE io
  USE dvd_prnt
  USE dvd_global
  IMPLICIT NONE
  INTEGER                                :: i, n2chk
  CHARACTER (LEN=*), DIMENSION(2)        :: type
...
!        to the new basis.
  ...
  IF(log_dvd(6)) THEN
     title='information for iteration = '//itoc(iter)
     WRITE(iout,1) title
  END IF
!     form residuals for the desired targeted nroots.
  ...
  END IF
  IF(log_dvd(8)) THEN
     title='hamiltonian on transformed vectors iteration = ' //itoc(iter)
...
!           write out converged eigenpairs to rwf
         CALL iosys('write real "'//type(1)//itoc(rtdone+i)  &
                                 //'" to rwf',1,temp,0,' ')
         CALL iosys('write real "'//type(2)//itoc(rtdone+i)  &
                                 //'" to rwf',n_dvd,vec(1,i),0,' ')
