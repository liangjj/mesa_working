...
c-----------------------------------------------------------------------
c
c     write (iout,9876) (nbf(i),i=1,nsym)
c     write (iout,9875) (nob(i),i=1,nsym)
c     write (iout,9874) (ncob(i),i=1,nsym)
c     write (iout,9873) (naob(i),i=1,nsym)
...
c      write(iout,*)'  b  vec '
      ntoter=nob(1)*nocc
c       write(iout,70701)(b(i),i=1,ntoter)
70701 format(5(1x,e15.7))
c
c       write(iout,*)' update vector '
c       call vecout(cm,nob(1),nocc)
c
c-------------------------------------------------------c
c                                                       c
...
c-------------------------------------------------------c
      npq = mtab(nsym+1)
c
c -----   read fock matrix from tape16
c
c
...
c
c     write(iout,*)' mcgvcb:  gg '
c     write(iout,70701)(g(i),i=1,nijij)
c     write(iout,*)' g2 '
c     write(iout,70701)(g2(i),i=1,nijnij)
c
c
c     ----- write the triangles of integrals to mcscr for l912
c
      call iosys('write real "h mcscf" to mcscr',nij,g1,0,' ')
      call iosys('write real "g mcscf" to mcscr',nij**2,g2,0,' ')
c
c ----- sort integrals into drt order -----
c
c..bhl      call sorti(g,g,ncore,g1,g2,intape)
c
 4500 continue
c
      return
