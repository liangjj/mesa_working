...
c***begin prologue     getbas.f
c***date written       850601  (yymmdd)
c***revision date      11/6/94
c***keywords           basis set, integrals
c***author             saxe, paul (lanl).
c***source             @(#)getbas.f	5.1 11/6/94
...
c                      skipln(chr)
c***end prologue       getbas.f
      implicit none
c     --- input variables -----
      integer inp,iout,nshell,mxsh,mxpr,mxcf,ntypes,atomno
      integer namdat
...
c     --- output arrays ---
      ...
      integer ptcont(mxsh),nctype(ntypes)
...
c
      integer stakln(stakmx),stakfl(stakmx)
      integer totpr,totco,stakpt,i
      integer namfil,lineno,pos,start,end
      integer ncore,numtyp,n,nc
      integer ctoi
      logical streqc
      real*8 atomz
