*deck @(#)sox.f	5.1  11/6/94
      subroutine sox(nprim,nprimi,nprimj,pointi,pointj,exp,xyz,
     #               itype,jtype,
     #               ncarti,ncartj,zso)
c
c***module to form the primitive one-electron spin-orbit integrals
c
c operator lx - integrals are done analytically
c
...
c
     ...
     #                       (exp(pointi+(l-1))+exp(pointj+(k-1)))
                ...
                if (i.eq.3.and.j.eq.2) then
                  zso(icount)=facp/(exp(pointi+(l-1))+exp(pointj+(k-1)))
                end if
40            continue
30          continue
20        continue
...
c
        ...
        do 50 i=1,ncarti
          do 60 j=1,ncartj
            do 70 k=1,nprimj
              do 80 l=1,nprimi
                icount=icount+1
                if (i.eq.2.and.j.eq.6) then
                  zso(icount)=-facd6/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
                if (i.eq.3.and.j.eq.6) then
                  zso(icount)=facd6/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
     ...
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                ...
                if (i.eq.6.and.j.eq.2) then
                  zso(icount)=facd4/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
                if (i.eq.6.and.j.eq.3) then
                  zso(icount)=-facd4/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
80           continue
...
c
     ...
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                ...
                if (i.eq.7.and.j.eq.10) then
                  zso(icount)=f3/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.8.and.j.eq.3) then
                  zso(icount)=(-3.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
     ...
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                ...
                if (i.eq.9.and.j.eq.2) then
                  zso(icount)=(3.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.9.and.j.eq.8) then
                  zso(icount)=(-2.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.10.and.j.eq.4) then
                  zso(icount)=(2.0*f8)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.10.and.j.eq.7) then
                  zso(icount)=(-2.0*f8)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
120          continue
