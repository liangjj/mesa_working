*deck @(#)soy.f	5.1  11/6/94
      subroutine soy(nprim,nprimi,nprimj,pointi,pointj,exp,xyz,
     #               itype,jtype,
     #               ncarti,ncartj,zso)
c
c***module to form the primitive one-electron spin-orbit integrals
c
c operator ly - integrals are done analytically
c
...
c
     ...
     $     facd2/0.4188790205/,facd4/0.8377580410/,facd6/1.256637061/,
...
c
      if (itype.eq.3.or.itype.eq.4) then
        write(iout,*)'warning - d,f functions not yet tested'
      end if
...
c
     ...
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                ...
                if (i.eq.5.and.j.eq.3) then
                  zso(icount)=facd4/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
                if (i.eq.6.and.j.eq.4) then
                  zso(icount)=-facd2/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**2
                end if
80           continue
...
c
     ...
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                ...
                if (i.eq.3.and.j.eq.7) then
                  zso(icount)=-f1/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.4.and.j.eq.9) then
                  zso(icount)=f3/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.5.and.j.eq.10) then
                  zso(icount)=f3/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.6.and.j.eq.1) then
                  zso(icount)=(-3.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.6.and.j.eq.7) then
                  zso(icount)=(2.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.7.and.j.eq.3) then
                  zso(icount)=(3.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.7.and.j.eq.6) then
                  zso(icount)=(-2.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.8.and.j.eq.10) then
                  zso(icount)=(-f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.9.and.j.eq.4) then
                  zso(icount)=(-2.0*f3)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.10.and.j.eq.5) then
                  zso(icount)=(-2.0*f8)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
                if (i.eq.10.and.j.eq.8) then
                  zso(icount)=(2.0*f8)/
     #                        (exp(pointi+(l-1))+exp(pointj+(k-1)))**3
                end if
120          continue
