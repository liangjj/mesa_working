*deck @(#)sort32.f	5.1  11/6/94
      subroutine sort32(labels,values,acore,lenbuf,nnp,ints,iout,
     $                 maxcor,vals,labs,bins,lenscr,ijv,klv,
     $                 iork,jorl,prnt,ops,killr)
c
c***begin prologue     sort32.f
c***date written       yymmdd  
c***revision date      11/6/94      
c
c***keywords           
c***author             saxe, paul(lanl)
c***source             @(#)sort32.f	5.1   11/6/94
c***purpose            
c***description
...
c            the input file is no longer needed. blow it away to release
c            disk space for the output file.
      if(killr) then
