*deck @(#)pm412.f	5.2  4/17/95
      program m412
c***begin prologue     m412
c***date written       850810   (yymmdd)
c***revision date      931994   (yymmdd)
c
c  20 june    2002     bis at nsf
c     changed the symmetry routines to deal with reduced 
c     basis sets and thus combined the old m401 and m402
...
c
c***keywords           m412, link 412, guess, $alter, huckel, core,
c                      rdchk, rdrwf, rdinp, $vectors
c***author             martin, richard    (lanl)
c***source             @(#)pm412.f	5.2   4/17/95
...
c
 1000 format(1x,'m412:initial guess')
...
c
      ...
      if(drop) then
         call iosys('write character reduced to rwf',0,0,0,'reduced')
      endif
c
...
c     checkpoint guess.
c
c      call getscm(0,z,maxcor,'m412',0)
      call getmem(top,p,ngot(1),'m412',0)
c
c     form s**-1/2.
...
c     obtain the guess.
     ...
     $                 //' from rwf',len(bflabl(1))*nbasis,0,0,bflabl)
         ...
         call lnkerr('unrecognized guess type in m412:'//gestyp)
      endif
c
...
c     --- project pure symmetry vectors if using symmetry ---
      if(usesym) then
         call symtriz(ops,.false.,lirrep,symlabl,bflabl,nbasis,
     $                nnp,maxrep,maxnbf,z(s),z(smhalf),z(c))
c
      endif
c
...
c
c
      call getmem(-ngot(1),p,idum,'m412',idum)
      call chainx(0)
c
c
      stop
