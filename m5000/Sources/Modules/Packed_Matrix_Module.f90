...
!
  SUBROUTINE Packed_Matrix(ham_type)
  IMPLICIT NONE
...
!*********************************************************************************************
  IF (input_matrices(11:22) == 'iosys_format') THEN  ! Matrices have already been packed.  Just read them in.
      ...
      Call Input_Packed_IOsys_Matrix(ham_type)
...
!        non-orthogonal basis is used.
      ...
      IF (ham_file == 'from_input' ) THEN
          IF (ham_type == 'real') THEN
              Call Read_Input_Matrices_d
          ELSE IF(ham_type == 'complex') THEN
              Call Read_Input_Matrices_z
          END IF
      ELSE
          Call pakstr(ham_file,len(1))    
          Call pakstr(species,len(2))    
          WRITE(iout,*) '        1. Hamiltonian File Name = '//ham_file(1:len(1))
      ...
      write(iout,*) file_loc
          ...
          READ(50)
          IF (ham_type == 'real') THEN
              Call Input_Packed_Matrices_d
          ELSE IF(ham_type == 'complex') THEN
              Call Input_Packed_Matrices_z
          END IF
          write(iout,*)
...
!
  SUBROUTINE Input_Packed_IOsys_Matrix(ham_type)
  IMPLICIT NONE
  CHARACTER(LEN=*)                      :: ham_type
  INTEGER                               :: max_buf
!
...
!
  SUBROUTINE Input_Packed_Matrices_d
  IMPLICIT NONE
  REAL*8                                :: gbytes
...
!
  SUBROUTINE Input_Packed_Matrices_z
  IMPLICIT NONE
  REAL*8                                :: gbytes
  CHARACTER(LEN=80)                     :: matrix_file_name
...
!
  SUBROUTINE Read_Input_Matrices_d
  IMPLICIT NONE
!
! This is the option to reformat the input matrix in IOsys format and then return 
! to the calling routine.
...
!
  SUBROUTINE Read_Input_Matrices_z
  IMPLICIT NONE
!
! This is the option to reformat the input matrix in IOsys format and then return 
! to the calling routine.
...
!
  ...
  DEALLOCATE(mat_var(3)%non_zero_columns,                                                &
             ...
             mat_var(3)%matrix_diagonal_z )
  DEALLOCATE(triangle_hamiltonian_z)
  Call Diagonalize_Packed_Matrices_z('buffers',                                         &
                                      n3d,                                              &
                                     .false.)
!***********************************************************************
