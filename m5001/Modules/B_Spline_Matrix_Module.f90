...
!
  ...
  Read(50)
  read(50)
  ALLOCATE(mat_d%overlap(n3d,n3d))
  mat_d%overlap = 0.d0
  Call Read_Mat(mat_d%overlap,spline_array_j)
  ALLOCATE(mat_d%hamiltonian(n3d,n3d))
  mat_d%hamiltonian = 0.d0
  Call Read_Mat(mat_d%hamiltonian,spline_array_j)
  stop
!***********************************************************************
...
!
  DO
    ...
    END IF
    IF(print_cc) THEN          
       Call Print_Matrix(type_real_matrix,matrix(i_1:i_2,j_1:j_2),size_i,size_j,   &
                         title='Matrix Channel 1 = '//itoc(i_ch)//'Channel 2 = '//itoc(j_ch) )
    END IF
  END DO
!***********************************************************************
