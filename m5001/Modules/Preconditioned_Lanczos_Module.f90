...
!
                           INTERFACE Preconditioned_Lanczos_Conv_Test
             MODULE PROCEDURE Preconditioned_Lanczos_Conv_Test_d,                &
                              Preconditioned_Lanczos_Conv_Test_z  
                       END INTERFACE Preconditioned_Lanczos_Conv_Test
!
!***********************************************************************
!***********************************************************************
                              CONTAINS
...
!***********************************************************************
  ...
  REAL(idp), DIMENSION(:)                        :: v_in
  REAL(idp), DIMENSION(:)                        :: v_out
  ...
  IF (packed_matrices) THEN 
      ...
      call column_packed_lower_triangle_on_vector                      &
                         (v_out(:),                                    &
                          ...
                          mat_var(1)%mat_d%matrix_diagonal )
  ELSE
      call matrix_vector_multiply( prop_mat_d%triangle_overlap,        &
                                   v_in, prop_mat_d%s_vec(:,0) )
  END IF
  anorm = 1.d0 / sqrt (ddot(n3d , v_in(:) , 1, prop_mat_d%s_vec(:,0) , 1 ))
  prop_mat_d%vec(:,0) = anorm * v_in(:)
  prop_mat_d%s_vec(:,0) = anorm * prop_mat_d%s_vec(:,0)
  v_in = anorm * v_in
...
!
     IF(log_iterative(10).or.log_iterative(2)) THEN
        call Print_Matrix(type_real_vector,prop_mat_d%vec(:,i),title='Lanczos Vector iteration = '//itoc(i))
     END IF
!
!           Begin Lanczos iterations
!
     call preconditioned_lanczos_vec(v_out,prop_mat_d%vec,prop_mat_d%s_vec,i)
!
!
...
!
    ...
    IF( convergence == 'maximal') THEN
        ...
        ELSE
            call matrix_vector_multiply( prop_mat_d%triangle_overlap,         &
                                         v_out(:), prop_mat_d%s_vec(:,0) )
        END IF
        norm = 1.d0 / sqrt (ddot(n3d , v_out(:) , 1, prop_mat_d%s_vec(:,0) , 1 ))
        v_in = norm * v_out
...
!***********************************************************************
  ...
  TYPE(REAL_PROP_VAR)                   :: prop_mat_d
  COMPLEX(idp), DIMENSION(:)            :: v_in
  COMPLEX(idp), DIMENSION(:)            :: v_out
  CHARACTER (LEN=5)                     :: itoc
  INTEGER                               :: i
  COMPLEX(idp)                          :: cdotc
  CHARACTER(LEN=8)                      :: kewrd
...
!
  ...
  ELSE
      call matrix_vector_multiply( prop_mat_d%triangle_overlap,        &
                                   v_in, prop_mat_z%s_vec(:,0) )
  END IF
  anorm = 1.d0 / sqrt (cdotc(n3d , v_in(:) , 1, prop_mat_z%s_vec(:,0) , 1 ))
  prop_mat_z%vec(:,0) = anorm * v_in(:)
  prop_mat_z%s_vec(:,0) = anorm * prop_mat_z%s_vec(:,0)
  v_in = anorm * v_in
  total_time(1) = secnds(0.0) - local_time + total_time(1)
  DO i=0, maxvec-1
!
...
!
     IF(log_iterative(10).or.log_iterative(2)) THEN
        call Print_Matrix(type_complex_vector,prop_mat_z%vec(:,i),title='Lanczos Vector iteration = '//itoc(i))
     END IF
!
!           Begin Lanczos iterations
!
     call preconditioned_lanczos_vec(v_out,prop_mat_z%vec,prop_mat_z%s_vec,i)
!
!
!           If the number of iterations has reached the size of the
!           original matrix without breakdown, we have to be converged.
!
    ...
    IF( convergence == 'maximal') THEN
        ...
        ELSE
            call matrix_vector_multiply( prop_mat_d%triangle_overlap,         &
                                         v_out(:), prop_mat_z%s_vec(:,0) )
        END IF
        norm = 1.d0 / sqrt (cdotc(n3d , v_out(:) , 1, prop_mat_z%s_vec(:,0) , 1 ))
        v_in = norm * v_out
        total_time(8) = secnds(0.0) - local_time + total_time(8)
        total_number_of_iterations = total_number_of_iterations + i + 1
        return
...
!***********************************************************************
  ...
  REAL(idp), DIMENSION(n3d,0:maxvec)             :: s_on_lanczos_basis
  REAL(idp), DIMENSION(n3d)                      :: v_out
  INTEGER                                        :: i
  INTEGER                                        :: it
  INTEGER                                        :: low
  REAL(idp)                                      :: ddot
...
!
  ...
  ELSE
      call matrix_vector_multiply( prop_mat_d%triangle_hamiltonian,    &
                                   lanczos_basis(:,it), prop_mat_d%h_vec )
  END IF
...
!
  local_time=secnds(0.0)
  call preconditioned_lanczos_conv_test(eigen_vectors,prop_mat_d%lanczos_tri,it)
...
!                                               -   b_{i} S |V_{i-1}>
     v_out(:) = prop_Mat_d%h_vec(:)  - a(it) * s_on_lanczos_basis(:,it)
     IF( it > 0 ) THEN
!
...
!
         IF(log_iterative(10)) THEN
            call Print_Matrix(type_real_vector,v_out,title='before reorthogonalization')
         END IF
!
...
!
         ...
         IF(log_iterative(10)) THEN
            call Print_Matrix(type_real_vector,v_out,title='after reorthogonalization')
         END IF
     END IF
!
...
!***********************************************************************
  ...
  COMPLEX(idp), DIMENSION(n3d,0:1)               :: s_on_lanczos_basis
  COMPLEX(idp), DIMENSION(n3d)                   :: v_out
  INTEGER                                        :: i, j
  INTEGER                                        :: it
  INTEGER                                        :: low
  COMPLEX(idp)                                   :: cdotc
...
!
  ...
  ELSE
      call matrix_vector_multiply( prop_mat_d%triangle_hamiltonian,    &
                                   lanczos_basis(:,it), prop_mat_z%h_vec )
  END IF
  total_time(2) = secnds(0.0) - local_time + total_time(2)
  IF(log_iterative(10).or.log_iterative(3)) THEN
...
!
  local_time=secnds(0.0)
  call preconditioned_lanczos_conv_test(eigen_vectors,prop_mat_z%lanczos_tri,it)
  total_time(4) = secnds(0.0) - local_time + total_time(4)
!
!
  IF(convergence == 'unconverged') THEN
...
!                                               -   b_{i} S |V_{i-1}>
!
     v_out(:) = prop_mat_z%h_vec(:)  - a(it) * s_on_lanczos_basis(:,it)
     IF( it > 0 ) THEN
!
!        v_out is proportional to S|V(it+1)>
!
...
!
         IF(log_iterative(10)) THEN
            Call Print_Matrix(type_complex_vector,v_out,title='before reorthogonalization')
         END IF
!
!        Perform an S reorthogonalization to  prevent linear dependence.
!
...
!
         total_time(5) = secnds(0.0) - local_time + total_time(5)
         IF(log_iterative(10)) THEN
            Call Print_Matrix(type_complex_vector,v_out,title='after reorthogonalization')
         END IF
     END IF
!
!        To compute the next beta, we use the formula,
         ...
         IF (packed_matrices) THEN 
            ...
            call column_packed_lower_triangle_on_vector                      &
                                (prop_mat_z%h_vec(:),                        &
                                 ...
                                 mat_var(1)%mat_d%matrix_diagonal )
         ELSE
             call matrix_vector_multiply(prop_mat_d%triangle_overlap, v_out,           &
                                         s_on_lanczos_basis(:,it+1) )        
         END IF
...
!               
  ...
  IF(log_iterative(10).or.log_iterative(5)) THEN
     call Print_Matrix(type_real_vector,a(0:it),title='Diagonal Element Lanczos H iteration = '//itoc(it)) 
     call Print_Matrix(type_real_vector,sub_diagonal,title = 'Off-Diagonal Element Lanczos H iteration = '//itoc(it))
  END IF
!
...
!***********************************************************************
!***********************************************************************
!**begin prologue     Preconditioned_Lanczos_Conv_Test_d
!**date written       960718   (yymmdd)
!**revision date               (yymmdd)
!**keywords           time, dvr, Lanczos, propagation
!**
...
!**references
!**routines called    iosys, util and mdutil
!**end prologue       Preconditioned_Lanczos_Conv_Test_d
!***********************************************************************
  SUBROUTINE Preconditioned_Lanczos_Conv_Test_d(eigen_vectors,lanczos_tri,it)
  IMPLICIT NONE
  TYPE(REAL_PROP_VAR)                            :: prop_mat_d
  REAL(idp), DIMENSION(0:maxvec,0:maxvec)        :: eigen_vectors
...
!
  prop_mat_d%local_scratch(1:it+1) = eigen_vectors(0,0:it)
  Call Exponential_on_Vector(prop_mat_d%work,prop_mat_d%local_scratch,eig,eigen_vectors,it)    
  ...
  IF( it+1 == maxvec) THEN
      ...
      DO i = 1, maximum_number_of_time_subintervals
         ...
         deltat=.5d0*deltat
         prop_mat_d%local_scratch(1:it) = eigen_vectors_previous(0,0:it-1)
         Call Exponential_on_Vector(prop_mat_d%work,prop_mat_d%local_scratch,eig_previous,eigen_vectors_previous,it-1)    
...
!         write(iout,*) work_d(0:it)
      ...
      call ebcx(prop_mat_d%local_scratch,n3d,prop_mat_d%vec,n3d,eigen_vectors,maxvec+1,n3d,it+1,1)
      return
  END IF
!
...
!  END IF
  ...
  ELSE
       IF(log_iterative(10).or.log_iterative(8)) THEN     
          call Print_Matrix(type_real_vector,lanczos_tri(0:it),title='lanczos_tri iteration = '//itoc(it)) 
          call Print_Matrix(type_real_vector,prop_mat_d%work,title='work iteration = '//itoc(it)) 
       END IF
       lanczos_tri(0:it) = lanczos_tri(0:it) - prop_mat_d%work(0:it)
       IF(log_iterative(10).or.log_iterative(8)) THEN     
          call Print_Matrix(type_real_vector,lanczos_tri(0:it),title='lanczos_tri iteration = '//itoc(it)) 
...
!
     ...
     call ebcx(prop_mat_d%local_scratch,n3d,prop_mat_d%vec,n3d,eigen_vectors,maxvec+1,n3d,it+1,1)
     IF(log_iterative(10).or.log_iterative(9)) THEN     
         call Print_Matrix(type_real_vector,prop_mat_d%local_scratch,title='Final Solution')
     END IF
     RETURN
  END IF
...
END SUBROUTINE Preconditioned_Lanczos_Conv_Test_d
!***********************************************************************
!***********************************************************************
!**begin prologue     Preconditioned_Lanczos_Conv_Test_z
!**date written       960718   (yymmdd)
!**revision date               (yymmdd)
!**keywords           time, dvr, Lanczos, propagation
!**
...
!**references
!**routines called    iosys, util and mdutil
!**end prologue       Preconditioned_Lanczos_Conv_Test_z
!***********************************************************************
  SUBROUTINE Preconditioned_Lanczos_Conv_Test_z(eigen_vectors,lanczos_tri,it)
  IMPLICIT NONE
  TYPE(COMPLEX_PROP_VAR)                             :: prop_mat_z
  REAL(idp), DIMENSION(0:maxvec,0:maxvec)            :: eigen_vectors
...
!
!               
  prop_mat_z%local_scratch(1:it+1) = eigen_vectors(0,0:it)
  Call Exponential_on_Vector(prop_mat_z%work,prop_mat_z%local_scratch,eig,eigen_vectors,it)
!
!          At this point, if we were converged, we would transform to the
!          original basis and return.  However, if we are not converged
!          we do not have to do that.  So, lets test convergence.
  ...
  IF( it+1 == maxvec) THEN
      ...
      DO i = 1, maximum_number_of_time_subintervals
         ...
         lanczos_tri(0:it) = 0.d0
         deltat=.5d0*deltat
         prop_mat_z%local_scratch(1:it) = eigen_vectors_previous(0,0:it-1)
         Call Exponential_on_Vector(prop_mat_z%work,prop_mat_z%local_scratch,eig_previous,eigen_vectors_previous,it-1)    
...
!         write(iout,*) prop_mat_z%work(0:it)
      ...
      deltat = save_deltat
      write(iout,4) t_interval, wfn_tst, t_end
      call cebcx(prop_mat_z%local_scratch,n3d,vec_z,n3d,prop_mat_z%work,maxvec+1,n3d,it+1,1)
      call Print_Matrix(type_complex_vector,prop_mat_z%local_scratch,      &
                        title='Final Value of Exp(-i*H*t) on Initial Vector Final')
      IF(log_iterative(10).or.log_iterative(9)) THEN     
         call Print_Matrix(type_complex_vector,prop_mat_z%local_scratch,title='Final Solution')
      END IF 
      RETURN
  END IF
  IF (it == 0) THEN
...
4 FORMAT(10x,'Iteration = ',i3,'  RMS = sqrt(< ( v_{i} - v_{i-1}) |( v_{i} - v_{i-1}) >) = ',e15.8,  &
             /,10x,'Converged',' Final Time = ',e15.8)
END SUBROUTINE Preconditioned_Lanczos_Conv_Test_z
!***********************************************************************
!***********************************************************************
