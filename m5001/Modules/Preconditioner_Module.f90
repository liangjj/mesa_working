...
!
  ...
  IMPLICIT NONE
  REAL(idp), DIMENSION(:)                :: triangle
  REAL(idp), DIMENSION(:)                :: matrix_diagonal
...
!
  SUBROUTINE Pack_Triangle_z (type,triangle,packed_columns,non_zero_columns, &
                              row_index,number,matrix_diagonal)
  COMPLEX(idp), DIMENSION(:)            :: triangle
  COMPLEX(idp), DIMENSION(:), OPTIONAL  :: matrix_diagonal
  COMPLEX(idp), DIMENSION(:)            :: packed_columns
  INTEGER, DIMENSION(:)                 :: non_zero_columns
  INTEGER, DIMENSION(:)                 :: row_index
  INTEGER                               :: number
  CHARACTER(LEN=*)                      :: type 
...
!
  SUBROUTINE Triangular_Solve_d (upper,solution,right_hand_side,trans,it)
  IMPLICIT NONE
  REAL(idp),  DIMENSION(:)                 :: upper
  REAL(idp),  DIMENSION(:)                 :: solution
  REAL(idp),  DIMENSION(:)                 :: right_hand_side
  CHARACTER(LEN=1)                         :: trans
  CHARACTER(LEN=3)                         :: itoc
  INTEGER                                  :: it  
!
  solution = right_hand_side
  Call dtpsv('u',trans,'n',n3d,upper,solution,1)
...
!
  SUBROUTINE Triangular_Solve_z (upper,solution,right_hand_side,trans,it)
  IMPLICIT NONE
  REAL(idp),  DIMENSION(:)                :: upper
  COMPLEX(idp),  DIMENSION(:)             :: solution
  COMPLEX(idp),  DIMENSION(:)             :: right_hand_side
  CHARACTER(LEN=1)                        :: trans
  CHARACTER(LEN=3)                        :: itoc
  INTEGER                                 :: it  
!
  solution = right_hand_side
  Call dztpsv('u',trans,'n',n3d,upper,solution,1)
  IF(print_internal_matrices) THEN
     title='Solution Vector Iteration = '//itoc(it)//' t = '//trans
...
!***********************************************************************
!***********************************************************************
!deck Triangular_Solve_zz
!***begin prologue     Triangular_Solve_zz
!***date written       061201   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords
!***author             schneider, barry (nsf)
...
!***references
!***routines called
!***end prologue       Triangular_Solve_zz
!
  SUBROUTINE Triangular_Solve_zz (upper,solution,right_hand_side,trans,it)
  IMPLICIT NONE
  COMPLEX(idp),  DIMENSION(:)             :: upper
  COMPLEX(idp),  DIMENSION(:)             :: solution
  COMPLEX(idp),  DIMENSION(:)             :: right_hand_side
...
!
  solution = right_hand_side
  Call ztpsv('u',trans,'n',n3d,upper,solution,1)
  IF(print_internal_matrices) THEN
     title='Solution Vector Iteration = '//itoc(it)//' t = '//trans
     call prntcmn(title,solution,n3d,1,n3d,1,iout,'e')
  END IF
!***********************************************************************
!***********************************************************************
  END SUBROUTINE Triangular_Solve_zz
!***********************************************************************
!***********************************************************************
!deck Packed_Solve_d
...
!***********************************************************************
!deck Packed_Solve_z
!***begin prologue     Packed_Solve_zz
!***date written       061201   (yymmdd)
!***revision date      yymmdd   (yymmdd)
!***keywords
!***author             schneider, barry (nsf)
...
!***references
!***routines called
!***end prologue       Packed_Solve_zz
!
  SUBROUTINE Packed_Solve_zz (solution,right_hand_side,non_zero_columns,row_index,         &
                             packed_columns,matrix_diagonal,number,trans,it)
  IMPLICIT NONE
  COMPLEX(idp),  DIMENSION(:)             :: solution
  COMPLEX(idp),  DIMENSION(:)             :: right_hand_side
  COMPLEX(idp),  DIMENSION(:)             :: packed_columns
  COMPLEX(idp),  DIMENSION(:)             :: matrix_diagonal
  COMPLEX(idp)                            :: conjg
  INTEGER, DIMENSION(:)                   :: non_zero_columns
  INTEGER, DIMENSION(:)                   :: row_index
  INTEGER                                 :: number
...
!***********************************************************************
!***********************************************************************
  END SUBROUTINE Packed_Solve_zz
!***********************************************************************
!***********************************************************************
