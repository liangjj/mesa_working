...
!
  IF ( type_calculation == 'real_time' ) THEN
       ALLOCATE( prop_mat_z%psi(1:n3d), prop_mat_z%v_scr(1:n3d) )
       IF ( i0stat == 'unperturbed_state_vector'                        &
                           .or.                                         &
            i0stat == 'perturbed_state_vector') THEN                   
            ALLOCATE(itemp(n3d,spdim),etemp(n3d))        
            CALL Initial_Vector(prop_mat_z%psi)          ! Fill up the initial vector
       ...
       Call Propagator ( prop_mat_z%psi, prop_mat_z%v_scr )       ! Do the heavy lifting
       DEALLOCATE( prop_mat_z%psi, prop_mat_z%v_scr )
  ELSE IF ( type_calculation == 'imaginary_time' ) THEN
       ALLOCATE( prop_mat_d%psi(1:n3d), prop_mat_d%v_scr(1:n3d) )
       IF ( i0stat == 'unperturbed_state_vector'                       &
                          .or.                                         &
            i0stat == 'perturbed_state_vector') THEN                   
            ALLOCATE(itemp(n3d,spdim),etemp(n3d))        
            CALL Initial_Vector(prop_mat_d%psi)          ! Fill up the initial vector
            DEALLOCATE(itemp,etemp)        
       ELSE IF( i0stat == 'superpose') THEN                           
            ALLOCATE(c_loc(spdim))
            DO i=1,spdim
       ...
       END IF
       Call Propagator_Setup                    ! Set up arrays
       Call Propagator (prop_mat_d%psi, prop_mat_d%v_scr )        ! Do the hevy lifting
       DEALLOCATE( prop_mat_d%psi, prop_mat_d%v_scr )
  ELSE
...
!
       ALLOCATE( prop_mat_d%vec(n3d,0:maxvec), prop_mat_d%h_vec(n3d) )
       ALLOCATE( eig(0:maxvec), sub_diagonal(0:maxvec),                                             &
                 eigen_vectors(0:maxvec,0:maxvec), prop_mat_d%lanczos_tri(0:maxvec),                &
                 prop_mat_d%work(0:lwork), rwork(0:lwork), eig_previous(0:maxvec),                  &
       ...
       ELSE IF(iterative_method == 'generalized_lanczos') THEN  ! An additional array is required
          ...
          WRITE(iout,3)
          ALLOCATE(prop_mat_d%local_scratch(n3d))
       ELSE IF(iterative_method == 'preconditioned_lanczos') THEN  ! An additional matrix is required
                                                                   ! for the preconditioned Lanczos
          WRITE(iout,4)
          ALLOCATE( prop_mat_d%s_vec(n3d,0:maxvec) )
       END IF
!
  ELSE IF(type_calculation == 'real_time') THEN
       ALLOCATE( prop_mat_z%vec(n3d,0:maxvec), prop_mat_z%h_vec(n3d))
       ALLOCATE( eig(0:maxvec), sub_diagonal(0:maxvec),                                             &
                 eigen_vectors(0:maxvec,0:maxvec), prop_mat_z%lanczos_tri(0:maxvec),                &
                 rwork(0:lwork), prop_mat_z%work(0:lwork), eig_previous(0:maxvec),                  &
                 eigen_vectors_previous(0:maxvec,0:maxvec), a(0:maxvec), b(0:maxvec) )
       IF(iterative_method == 'standard_lanczos') THEN
          Write(iout,2)
...
!
       ...
       ELSE IF(iterative_method == 'generalized_lanczos') THEN   
              ALLOCATE(prop_mat_z%local_scratch(n3d))     ! allocate some additional storage
              WRITE(iout,3)
       ELSE IF(iterative_method == 'preconditioned_lanczos') THEN
              WRITE(iout,4)
              ALLOCATE( prop_mat_z%s_vec(n3d,0:maxvec) )  ! allocate some additional storage
       END IF
  END IF
!
1 FORMAT('***********************************************'                                     &
...
!**end prologue       Propagator_d
  ...
  REAL(idp), DIMENSION(:)             :: input_vector                         
  REAL(idp), DIMENSION(:)             :: scratch_vector                       
  INTEGER                             :: t
  INTEGER                             :: i
...
!
  DEALLOCATE( prop_mat_d%vec, prop_mat_d%h_vec )
  DEALLOCATE( eig, sub_diagonal, eigen_vectors, prop_mat_d%lanczos_tri, &
              rwork, prop_mat_d%work, eig_previous,eigen_vectors_previous, a, b )
  IF(iterative_method == 'standard_lanczos') THEN
!
...
!**end prologue       Propagator_z
  ...
  COMPLEX(idp), DIMENSION(:)          :: input_vector                         
  COMPLEX(idp), DIMENSION(:)          :: scratch_vector                       
  INTEGER                             :: t
  INTEGER                             :: i
  CHARACTER (LEN=8)                   :: kewrd
!
...
!    Deallocate Memory
!
  DEALLOCATE( prop_mat_z%vec, prop_mat_z%h_vec )
  DEALLOCATE( eig, sub_diagonal, eigen_vectors, prop_mat_z%lanczos_tri,   &
              prop_mat_z%work, eig_previous,eigen_vectors_previous, a, b )
  IF(iterative_method == 'generalized_lanczos') THEN
!
     IF (packed_matrices) THEN
         DEALLOCATE( mat_var(3)%non_zero_columns,      &                       
