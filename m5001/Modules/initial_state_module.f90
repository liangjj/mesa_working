...
!
  IF( dollar('$initial_state',card,title,inp) ) THEN
      i0stat=chrkey(card,'initial_state','unit_vector',' ')
...
!
  ...
  IMPLICIT NONE
  REAL(idp), DIMENSION(:)                :: wave_function
  INTEGER                                :: i
  INTEGER                                :: nwfn
  INTEGER                                :: len
  INTEGER                                :: lenth
...
!                 File_Directory(4)(1:len_dir(4))//'/'//'Time_Dependent_Data')
!     Call IOsys('read real ground_state_eigenvector from time_dependent_data',  &
!                 nwfn,wave_function,0,' ')

     Call IOsys('read real initial_state_vector from tdse',nwfn,wave_function,0,0)
...
!  END IF
  ...
  END IF
  Call IOsys('write real initial_state_vector to tdse',nwfn,wave_function,0,0)
...
!
  ...
  IMPLICIT NONE
  COMPLEX(idp), DIMENSION(:)               :: wave_function
  COMPLEX(idp)                             :: cdotc
  COMPLEX(idp)                             :: seed
  INTEGER                                  :: i
  INTEGER                                  :: nwfn
  ...
  REAL(idp)                                :: t_ran
  CHARACTER(LEN=8)                         :: kewrd
  REAL(idp), DIMENSION(:), ALLOCATABLE     :: r_temp
  SAVE norm
  nwfn=size(wave_function,1)
!    The allowable right hand sides are; one, a state vector of the
!    unperturbed Hamiltonian, a specified superposition of 
...
!                 2*nwfn,wave_function,0,' ')
     Call IOsys('read real initial_state_vector from tdse',2*nwfn,wave_function,0,0)
!     Call IOsys('close time_dependent_data',0,0,0,' ')
  ELSE
     CALL lnkerr('error in initial state')
  END IF
  IF(prnton) THEN
     ...
     CALL print_psi(wave_function)
  END IF
  Call Iosys('write real initial_state_vector to tdse',2*nwfn,wave_function,0,0)
  WRITE(99) wave_function
1 FORMAT(/,1X,'initial state construction at first time = ',e15.8,  &
         /,1X,'driver                                   = ',a24)
2 FORMAT(/,5x,'Normalization Integral = ',e15.8)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  REAL(idp), DIMENSION(nphy(1))            :: wave_function
  REAL(idp), DIMENSION(nphy(1))            :: z_1
  wave_function(:) = z_1(:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  COMPLEX(idp), DIMENSION(nphy(1))        :: wave_function
  REAL(idp), DIMENSION(nphy(1))           :: z_1
  wave_function(:) = z_1(:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  COMPLEX(idp), DIMENSION(nphy(1))            :: wave_function
  COMPLEX(idp), DIMENSION(nphy(1))            :: z_1
  wave_function(:) = z_1(:)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  REAL(idp), DIMENSION(nphy(2),nphy(1))       :: wave_function
  REAL(idp), DIMENSION(nphy(1))               :: z_1
  REAL(idp), DIMENSION(nphy(2))               :: z_2
  INTEGER                                     :: i
  INTEGER                                     :: j
  DO i=1,nphy(1)
     DO j=1,nphy(2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  COMPLEX(idp), DIMENSION(nphy(2),nphy(1))        :: wave_function
  REAL(idp), DIMENSION(nphy(1))                   :: z_1
  REAL(idp), DIMENSION(nphy(2))                   :: z_2
  INTEGER                                         :: i
  INTEGER                                         :: j
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  COMPLEX(idp), DIMENSION(nphy(2),nphy(1))        :: wave_function
  COMPLEX(idp), DIMENSION(nphy(1))                :: z_1
  COMPLEX(idp), DIMENSION(nphy(2))                :: z_2
  INTEGER                                         :: i
  INTEGER                                         :: j
  DO i=1,nphy(1)
     DO j=1,nphy(2)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  REAL(idp), DIMENSION(nphy(3),nphy(2),nphy(1))    :: wave_function
  REAL(idp), DIMENSION(nphy(1))                    :: z_1
  REAL(idp), DIMENSION(nphy(2))                    :: z_2
  REAL(idp), DIMENSION(nphy(3))                    :: z_3
  INTEGER                                          :: i
  INTEGER                                          :: j
  INTEGER                                          :: k
  DO i=1,nphy(1)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  IMPLICIT NONE
  COMPLEX(idp), DIMENSION(nphy(3),nphy(2),nphy(1))    :: wave_function
  REAL(idp), DIMENSION(nphy(1))                       :: z_1
  REAL(idp), DIMENSION(nphy(2))                       :: z_2
  REAL(idp), DIMENSION(nphy(3))                       :: z_3
  INTEGER                                             :: i
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  COMPLEX(idp), DIMENSION(nphy(1))                  :: z_1
  COMPLEX(idp), DIMENSION(nphy(2))                  :: z_2
  COMPLEX(idp), DIMENSION(nphy(3))                  :: z_3
  INTEGER                                           :: i
  INTEGER                                           :: j
  INTEGER                                           :: k
  DO i=1,nphy(1)
...
!***end prologue       z_proj_d
  ...
  INTEGER                                :: n
  INTEGER                                :: n_p
  REAL(idp), DIMENSION(n)                :: q
  REAL(idp), DIMENSION(n)                :: wt
  REAL(idp)                              :: alpha
  REAL(idp)                              :: sigma
  REAL(idp)                              :: x_0
  REAL(idp)                              :: beta
  REAL(idp), DIMENSION(n)                :: z
  INTEGER                                :: i
...
!**end prologue       c_vect_2d_d
  ...
  INTEGER                                 :: root
  REAL(idp), DIMENSION(nphy(2),nphy(1))   :: wave_function
  REAL(idp), DIMENSION(nphy(2)*nphy(1))   :: etemp
...
!**end prologue       c_vect_3d_d
  ...
  INTEGER                                         :: root
  REAL(idp), DIMENSION(nphy(3),nphy(2),nphy(1))   :: wave_function
  REAL(idp), DIMENSION(nphy(3)*nphy(2)*nphy(1))   :: etemp
