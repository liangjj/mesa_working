*deck @(#)qmat.f	5.1  11/6/94
      subroutine qmat(jmat,kmat,h,nnp,ncoul,nexch,q,scfnum,shlmin,
...
c
      if (scfnum.eq.0.or.scfnum.eq.1.or.scfnum.eq.2) then
         ij=0
         do 91 i=1,nbf
            do 90 j=1,i-1
               ij=ij+1
 ...
 90         continue
            ij=ij+1
            if (diagnl.eq.0.or.scfnum.ne.0) then
               q(ij)=i
            end if
 91      continue
      end if
