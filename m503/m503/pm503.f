*deck @(#)pm503.f	5.1 11/6/94
      subroutine pm503(z,a)
c
...
c
      call getscm(need,z,maxcor,'m503',1)
c
      lenbuf=ntriang*nnp
c
      if(prnt) then
...
c
     ...
     $         z(jmat),z(kmat),bflabl,usesym,symlabl,z(salc),a(numso),
     $         a(lambda),lirrep,nirrep,a(orbsym),a(occsym),a(temp),
     $         a(kept))
c
...
c     ----- find out how much core is available -----
c
      call getscm(0,z,canget,'m503: how much core',0)
c
      wish=wpadti(values+nnp**2)
      if (wish.le.canget) then
         ntriang=nnp
