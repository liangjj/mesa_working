*deck @(#)scf.f	1.4  7/30/91
     ...
     $               orbsym,occsym,temp,kept,guessym)
c
...
c       also added option to write vectors in order expected for scattering
c       calculations.
...
c
c      if (logkey(ops,'scf=symmetry',.false.,' ')) then
      if (usesym) then
...
c     $               0,0,lirrep)
     ...
     $              nbf,orbsym,0,' ')
         write(iout,1015) (lirrep(i),i=1,nirrep)
         write(iout,1010) (numso(i),i=1,nirrep)            
         do 1030 i=1,nbf
            guessym(i)=lirrep(orbsym(i))
 1030    continue   
         write(iout,1020) (guessym(i),i=1,nbf)
         if(logkey(ops,'print=scf=salc',.false.,' ')) then
             write(iout,906)
...
c              guess vector
         ...
         if(logkey(ops,'scf=occsym',.false.,' ')) then
            ...
            do 107 shell=1,nshell
               do 106 i=shlmin(shell),shlmax(shell)
...
c      write(iout,*) 'finished transformation to ao basis'
c
      if (calc.eq.'gvb') then
         fcoef(1)=1.0d+00
         fcoef(2)=cicoef(1)**2
...
c      write(iout,*) 'finished symmetry'
c..bhl 4/19/90
c
...
c      write(iout,*) 'finished symrsp or degrsp'
cend
c
...
c----------------------------------------------------------------------c
      if (logkey(ops,'scf=symmetry=scattering-order',.false.,' ')) then
          call scatvec(c,eigval,orbsym,t1,t2,temp,shlmin,shlmax,nbf,
     1                 nshell,nirrep,nsmall,noabort)
      endif
...
c
    ...
    6    format (5x,'final vector:')
         if (chrkey(ops,'print=m503=vector',' ',' ').eq.'all') then
            nprint=nbf
         else
...
c
      return
 906  format(/,' salc matrix')
 1000 format(5x,'shell',i2,' occupations by symmetry')
 1005 format(8x,18a4)
 1010 format(6x,18i4)
 1015 format(5x,'number of symmetry orbitals in each irrep',/,8x,8a4)
 1020 format(5x,'guess vector symmetries',/,(8x,18a4))
      end
