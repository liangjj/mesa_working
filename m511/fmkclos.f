...
c                 --- form operator on phi(mu) and gradphi(mu).
                  call vmul(queue(1,2),queue(1,1),phi(1+ioff,mu),n)
                  call vmul(queue(1,3),queue(1,1),grad(1+ioff,mu,coord),
     $                      n)
...
c                    --- test the contribution of this block.
                     ...
                     if (abs(test4).ge.thresh) then
                        kmat(kl)=kmat(kl)
     $                    +sdot(n,queue(1,2),1,grad(1+ioff,nu,coord),1)
                     endif
                     if(abs(test5).ge.thresh) then
                        kmat(kl)=kmat(kl)
     $                    +sdot(n,queue(1,3),1,phi(1+ioff,nu),1)
                     endif
   70             continue
   80          continue
   90       continue
