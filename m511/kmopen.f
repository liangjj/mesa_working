...
c                  fix later.
     ...
     $                   dengridb,dengradb,minesz,scr,phibar,
     $                   gradbar,dmcut,dograd)
            call gridden(nbf,nnp,ng,mxgbsiz,d(1,2),phi,grad,
     $                   dengrida,dengrada,minesz,scr,phibar,
     $                   gradbar,dmcut,dograd)
            if(timeit) then
               call timing(dum4,dum5,dum6)
               timden=timden+dum4-dum1
            endif
            call iosys('write real "open density" on rwf',
     $                  mxgbsiz,dengrida,denptr,' ')
            call iosys('write real "open density" on rwf',
     $                  mxgbsiz,dengridb,denptr+mxgbsiz,' ')
            if (becke .or. lyp) then
               call iosys('write real "open density gradient" on rwf',
