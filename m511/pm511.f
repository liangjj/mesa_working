...
c     --- local variables ---
      ...
      integer rmax,lmax,atom,radshls,biggrd,maxr
...
c     new way, generate grid at lower level, one atom at a time.
      ian=top
      ngrid=ian+natoms
      c=iadtwp(ngrid+natoms)
      ptrad=wpadti(c+3*natoms)
      grid=ptrad+rmax
...
c         and the largest one.
      ...
      maxr=0
      do 10 atom=1,natoms
         call mkatmg(z(c),a(ian),z(grid+(atom-1)*3*mxgrd),
     $               z(wts+(atom-1)*mxgrd),rmax,lmax,
     $               nomega,nradial,natoms,a(ngrid+atom-1),mxgrd,
     $               z(vwts),z(rnuc),z(amu),z(pwtx),z(rr),adjust,
     $               z(radii),z(akl),grdtyp(atom),radshls,a(ptrad),
     $               .false.,z(vwts),atom)
         biggrd=max(biggrd,a(ngrid+atom-1))
         maxr=max(maxr,radshls)
   10 continue
c
c     --- repack the grid according to the largest atomic grid
c       and copy the grid to disk
      nugrd=rnuc
      nuwts=nugrd+3*biggrd*natoms
      nuvwts=nuwts+biggrd*natoms
      top=wpadti(nuvwts+biggrd*natoms)
      if(top.gt.canget) then
         write(iout,*) 'top,canget',top,canget
         call lnkerr('need more room to repack grid')
      endif   
      call pakgrd(z(grid),z(wts),z(vwts),mxgrd,
     $            z(nugrd),z(nuwts),z(nuvwts),biggrd,natoms)
      call iosys('write real "external grid" to rwf',
     $           3*biggrd*natoms,z(nugrd),0,' ')
      call iosys('write real "external grid weights" to rwf',
     $           biggrd*natoms,z(nuwts),0,' ')
...
c     --- do the scf iterations ---
     ...
     $          a(temp),a(kept),left,mxgrd,a(ngrid),dmcut,dencut,
     $          a(ptprim),a(noprim),nbtype,
