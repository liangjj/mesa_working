...
c***source             @(#)ftogrid2.f	5.1   11/28/95
...
c$$$c HACK: mult by r**l-1 because we want to divide by r, but  also want to
c$$$c       multipy by r**l, since we divided
c$$$c       by that upstairs---fix this when sure we want to keep it.
c$$$               do 113 grid=1,n
c$$$                  q=(ulm(ind(grid+joff)+1,lm)+ulm(ind(grid+joff),lm))
c$$$     $                 *(r(grid+joff)**(l-1))
c$$$                  vmax=max(vmax,abs(half*q))
c$$$  113          continue
               do 113 grid=1,n
...
c        --- do the components
         ...
         lm=1
         do 140 l=1,highl
c$$$c form r**l and store it temporarily in ctheta
c$$$            if (l.gt.1) call vpower(ctheta,r(1+joff),l-1,n)
            do 135 m=-l,l
               lm=lm+1
               call splint(rpts,ulm(1,lm),y2(1,lm),nr,r(1+joff),
     $                     fr(1+joff,lm),ind(1+joff),n)
               call vdiv(fr(1+joff,lm),fr(1+joff,lm),r(1+joff),n)
c$$$c KLUDGE: mult by r**(l-1) because we divided by r**l earlier and also need to
c$$$c divide by r to get the potential proper.
c$$$               if (l.gt.1) call vmul(fr(1+joff,lm),fr(1+joff,lm),
c$$$     $              ctheta,n)
  135       continue
  140    continue
         if(timeit) then
