*deck @(#)kmatrix.f	5.1  11/28/95
      subroutine kmatrix(values,d,dlast,nbf,nnp,kmat,nexch,grdwts,
     $     ndmat,nat,mxgrd,exc,slater,becke,lyp,vwn,hf,calce,calc,
...
c***source             @(#)kmatrix.f	5.1   11/28/95
...
c     --- input arrays (unmodified) ---
      ...
      integer nx(*),ny(*),nz(*)
      integer shlmin(ndmat),shlmax(ndmat)
...
c     --- local variables ---
     ...
     $        dtmp,tmpgwt,nzptrs,bigl,ngbtot
      ...
      logical slater,becke,lyp,vwn,calce,prteexch,dorhoup,called,hf
...
c     use scr space for strip mining density formation too. (takes minesz*nbf)
      ...
      if (becke .or. lyp) then
c need a little extra if doing the gradient of the density
         tea=scr2+minesz*nbf
      else
         tea=scr2+minesz
      endif
...
c$$$c           first time through work with full density.
c$$$            call vmove(values(dtmp),d,nnp)
c$$$            dorhoup=.false.
c$$$            call iosys('does "closed density" exist on rwf',
c$$$     $                  0,0,0,answer)
c$$$            ngbtot=1+(mxgrd/mxgbsiz)
c$$$            if(answer.eq.'no') then
c$$$               call iosys('create real "closed density" on rwf',
c$$$     $                     mxgbsiz*ngbtot*nat,0,0,' ')
c$$$               if(becke.or.lyp) then
c$$$                  call iosys('create real "closed density gradient" '
c$$$     $                     //' on rwf',mxgbsiz*ngbtot*nat*3,0,0,' ')
c$$$               endif
c$$$            endif
c$$$         else
c$$$c           subsequent entries work with density difference.
c$$$            call vsub(values(dtmp),d,dlast,nnp)
c$$$            dorhoup=.true.
c$$$         endif
c for now, don't mess with writing densities
c
...
c
     ...
     $     values(grad),values(scr),values(scr2),slater,becke,lyp,vwn,
     $     hf,
     ...
     $     values(gradbar),shlmax(1),coeff,bfcut,kmcut,grdfil)
      else 
c$$$         if (.not.called) then
c$$$            call vmove(values(dtmp),d,nnp*2)
c$$$            dorhoup=.false.
c$$$            call iosys('does "open density" exist on rwf',
c$$$     $                  0,0,0,answer)
c$$$            if(answer.eq.'no') then
c$$$               ngbtot=1+(mxgrd/mxgbsiz)
c$$$               call iosys('create real "open density" on rwf',
c$$$     $                     2*mxgbsiz*ngbtot*nat,0,0,' ')
c$$$               if(becke.or.lyp) then
c$$$                  call iosys('create real "open density gradient" '
c$$$     $                     //' on rwf',2*mxgbsiz*ngbtot*nat*3,0,0,' ')
c$$$               endif
c$$$            endif
c$$$         else
c$$$            call vsub(values(dtmp),d,dlast,nnp*2)
c$$$            dorhoup=.true.
c$$$         endif
     ...
     $     vwts,rnuc,amu,pwtx,rr,radii,akl,ptrad,
     $     rmax,lmax,nomega,nradial,grdtyp,adjust,minesz,values(phibar),
     $     values(gradbar),shlmax,coeff,bfcut,kmcut,grdfil)
      endif
c      
c
      do 10 i=1,nexch
