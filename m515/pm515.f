*deck @(#)pm515.f	5.1  11/28/95
      subroutine pm515(z,a)
c
c***begin prologue     pm515.f
...
c        this should be changed.
     ...
     $                pmxgrd,ngrid,grid,wts,vwts,ptrad,pmaxr,top)
         call iosys('write real "poisson grid" to '//grdfil(1:3),
     $        3*pmxgrd*natoms,z(grid),0,' ')
         call iosys('write real "poisson grid weights" to '
     $        //grdfil(1:3),pmxgrd*natoms,z(wts),0,' ')
         call iosys('write integer "poisson atomic grid size" to '
     $        //grdfil(1:3),natoms,a(ngrid),0,' ')
c         call iosys('write real "poisson voronoi weights" to '
c     $        //grdfil(1:3),pmxgrd*natoms,z(vwts),0,' ')
         call iosys('write integer "poisson radial pointers"'
     $        //' to '//grdfil(1:3),pmaxr*natoms,a(ptrad),0,' ')
c sigh.  But we clobbered ngrid for the external grid, so now we have
c to fix that.  
         call icopy(a(grdsiz),a(ngrid),natoms)
      endif
c
