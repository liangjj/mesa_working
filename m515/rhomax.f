...
c***source             @(#)rhomax.f	5.1   11/28/95
c***purpose            
c***description
c     
...
c
     ...
     $           ' ')
         ...
         if (ispar) call brdcst(906+MSGDBL,toang,mdtob(1),0)
         called=.true.
      endif
c
c
      if(type.eq.'bragg') then
         ...
         itype=2
         if(atnum.gt.86) then
            call plnkerr('clementi radii not available for Z>86',900)
         endif
      else if(type.eq.'slater') then
         itype=3
         if(atnum.gt.36) then
            call plnkerr('slater radii not available for Z>36',902)
         endif
      endif
c
c     --- assign the radii
      if(atnum.le.18) then
         ...
         call plnkerr('radii missing for Z=87 in rhomax',904)
      else if(atnum.le.95) then
         rhomax = rhomx6(atnum-87)/toang
      else
         call plnkerr('radii missing for Z>95 in rhomax',903)
      endif
c
c
      return
