...
c     --- input arrays (scratch) ---
      real*8 z(maxcor)
c     --- output arrays ---
      real*8 v(nnp,ngrid),vel(ngrid),vnuc(ngrid)
c     --- output variables ---
c     --- scratch arrays ---
c     --- local variables ---
      integer inp,iout,mxpts
      integer jatom,itype,jtypmx,jtype
...
c         this is a simple modification of v0.f to do just those functions
c         on a single atom.
c         temporary version uses way too much space and does way too much
c         work.
         call rzero(v,ngrid*nnp)
         jatom=iatom
...
c
c     --- evaluate the potential on the grid.
      do 40 catom=1,ngrid
         call trtosq(z,d,nbf,nnp)
         call trtosq(z(1+nbf*nbf),v(1,catom),nbf,nnp)
         vel(catom)=sdot(nbf*nbf,z,1,z(1+nbf*nbf),1)
   40 continue
c
c     --- compute nuclear contribution.
      do 36 catom=1,ngrid
         vnuc(catom)=zero
         do 35 at=1,nat
