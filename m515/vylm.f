...
c     --- local variables ---
      ...
      logical called,timeit
      real*8 timylm,timlgn,timphi
...
c
      ...
      data called/.false./
      data timylm,timlgn,timphi/3*0.0d0/
      save called,pi,timylm,timlgn,timphi
c
      common/io/inp,iout
c
c     --- set up constants ---
...
c            note the norms used for real sine,cosine functions.
   ...
   50    continue
         if(timeit) then
            call timing(dum4,dum5,dum6)
            timphi=timphi+dum4-dum1
         endif
   60 continue
c
c
      if(timeit) then
         write(iout,*) 'timylm,timlgn,timphi',
     $                  timylm,timlgn,timphi
      endif
      return
      end
