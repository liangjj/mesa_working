*deck @(#)mcdriv.f	5.1  11/6/94
      subroutine mcdriv(cr,icr,nwint,npflg,prtflg)
c
...
c      skipping the transfer of "mo 1pdm" and "mo 2pdm" from
c      mcscr to moden if doing quadratic closed shell scf, since
c      only the active orbitals are used to generate them in hfden.
c    2 february 1992  rlm at lanl
c      adding bhl code to do state averaged mcscf
...
c
     ...
     $     thrcah/1.d-6/,iauto/1/,osqcdf/1.d0/,autoso/1.d-3/,mcroot/0/
      ...
      data oldso/5.0d-03/
      save nbf,nfob,ncob,naob,nob
      save nato,nstate,istate,itcorh,iocore,itflag,itran,thrsh
      save iobsym,iobcas,icas,throv,thmcdv,nporb,mcmthd,nmcit,thrmc
      save nvcset,nleqps,nleqit,thrleq,nheig,thre,nviter,innorm,iprtci
      save lbuf84,nhd,ldar,ipfdm,igeom,opnscf,iprtg
      save smlld,shftd,icutah,nmaxah,thrcah,iauto,osqcdf,autoso,mcroot
      save icphf,lbcphf,iorder,jktrn,lsect,lbufso,iprtmo,ksym,ifock
      save iprtlag
      save lendia,nphes,lbufh,oldso
c
...
c     ----- first set options from options string -----
      ...
      toler=intkey(ops,'mcscf=tolerance',0,' ')
      if(toler.ne.0) thrmc=10.0d0**(-toler)
      call intarr(ops,'mcscf=method',mcmthd,50,' ')
      call fparr(ops,'mcscf=small-diagonals',smlld,25,' ')
c
c     ----- state-averaged mcscf -----
      navg=intkey(ops,'mcscf=navg',1,' ')
      if(navg.ne.1) then
         mcroot=navg
         if(navg.gt.20) then
            write(iout,*)' navg must be lt 20 ..stop'
            call lnkerr('m553: navg too big')
         end if
         call fparr(ops,'mcscf=weight',weight,navg,' ')
c        -----normalize the weight array
         xx=0.0d0
         do 11011 i=1,navg
            xx=xx+weight(i)
11011    continue
         xx=1.d0/xx
         call sscal(navg,xx,weight,1)
c        
         write(iout,8801)thrmc,navg,(weight(ik),ik=1,navg)
8801     format(5x,'state-averaged mcscf',/,
     $          5x,'   convergence = ',d15.7,/,
     $          5x,'   navg        = ',i7,/,
     $          5x,'   weights     = ',10(2x,f5.2))
      end if
...
c
     ...
     $               1,naob,0,' ')
      ...
      call  intarr(ops,'mcscf=iobcas',iobcas,100,' ')
...
c
     ...
     $              itcorh=0
         ...
         if (logkey(mcinp,'print=m553=vector',.false.,' ')) iprtmo=1
...
c
 1000 continue
      if(debug) then
         write(iout,*) 'mcdriv;1000: begin mcscf iteration loop'
      endif
      iter = iter + 1
      iaugh = 1
      icicup = 0
c
      if(iauto.ne.0.and.osqcdf.lt.autoso) then
         mcmthd(iter)=2
         iseco=1
      end if
      if(iauto.ne.0) then
         if(iseco.ne.0.or.osqcdf.lt.oldso) mcmthd(iter)=2
         if(ilast.eq.1.and.icphf.ne.0) mcmthd(iter)=2
      endif
      if(debug) then
         write(iout,*) '      iteration:',iter,' method:',mcmthd(iter),
     $                 ' iseco:',iseco
      endif
...
c
     ...
     $     lbufso,ilast,itcorh,navg,weight,mcroot,ops)
c
      if(debug) then
         write(iout,*) 'after mciter'
      endif
      call iosys('read real "mcscf energy" from rwf',1,energy,0,' ')
      if(ilast.ne.1) then
         if(navg.ne.1)  write(iout,9112)
 9112    format(t10,' averaged energy')
         write (iout,9102) iter,energy,sqcdf
...
c     convergence test.
      if (sqcdf .le. thrmc)  then
         if(ilast.eq.0) then
            ilast=1
c           increment maximum cycles variable to take care of the unusual
c           case in which it converges on the very last iteration.
            if(iter.eq.nmcit) then
               nmcit=nmcit+1
            endif
            ipfdm=1
            write(iout,1903)
...
c
c
c     for closed shell quadratic scf, these do not exist as there are
