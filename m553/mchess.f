...
c
     ...
     $                     nsym,noci,temp,tv,itran,thrsh,mr,ms,ndab,
     $                     nfiv,nfav,lhaa,lhac,lhca,lhcc, xjbuf,noct,
     $                     nbft,nbufj,jstrt,jnij,jmij,jnkl,jmkl,jkcore,
     $                     bufix,bufax,lbufso,lxjsav,ireadj,jstsav,irdj,
     $                     nbufjj,mblkd,lblkd)
            endif
c
c
...
c
     ...
     $                  c(isymc(mr)),c(isymc(ms)),nbfr,nbfs,hess,mr,ms,
     $                  tv,temp,xj)
c
c
c-----------------------------------------------------
...
c
     ...
     $                    ndab,nfiv,lhaa,lhac,lhca,lhcc, noct,nbft,
     $                    nbufk,kstrt, knij,kmij,knkl,kmkl,jkcore,bufix,
     $                    bufax,lbufso,lxksav,ireadk,kstsav,irdk,nbufkk,
     $                    mblkd,lblkd)
c
            endif
c
...
c
            ...
            if(incorh.eq.0) then
               if(.not.called) then
                  called=.true.
                  call iosys('does mc_ao_ymatrix exist on rwf',
     $                       0,0,0,ans)
                  if(ans.eq.'no') then
                     nthes=(nocc(1)*(nocc(1)+1)/2)*nbf(1)*nbf(1)
                     call iosys('create real mc_ao_ymatrix on rwf',
     $                           nthes,0,0,' ')
                  end if
               end if  
...
c
         if(debug) then
            write(iout,*)'  out-of-core construction of hessian '
         endif
c
c
...
c
         call sorter('start',asort,asort,wpadti(ncors),lhmo,0,-1,0,0,
...
c
         if(debug) then
            write(iout,*)'  naor ncor ',naor,ncor
         endif
c
 1300    continue
c
