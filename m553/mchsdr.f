...
c
      if(debug) then
         write (iout,9001)
 9001    format(/'0************ orbital hessian section ************')
      endif
c
c
c-----------------------------------c
...
c
      if(debug) then
         write(iout,*)'  before mchess lbufh ',lbufh
      endif
c
      lhbuf=lbufh
c
...
c
      if(debug) then
         write(iout,*)'   after mchess lbufh ',lbufh
         write(iout,*)'  after mchess nphes  ',nphes
      endif
c
c.io      call srew(nfhm)
c
...
c
      if(debug) then
         write(iout,*) ' lagrangian '
         nnnn=nco(1)+nao(1)
         call vecout(grad,nob(1),nnnn)
      endif
c
c=====
