...
c      adding bhl state-averaged mcscf capability.
...
c
      ...
      save locsym,isymc,isymm,lfab,ldab,lfmm
c
c
...
c
     ...
     $                    ' ',mcroot,'mc')
            ...
            nda=id2+ntnao
            ncora=ncore-nda
            call rzero(cr(iad1),nnao)
            call rzero(cr(iad2),ntnao)
            do 9901 i=1,navg
               if (citype.eq.'m902') then
                  call mn902(cr(nda),cr(nda),ncora,'density',
     $                       cr(nda),cr(nda),'mcscr','mcscr',
     $                       'mcscf',i,'mc')
                  call mn830(cr(nda),cr(nda),ncora,'mcscr','mcscr',
     $                       'mcscf','guga density matrix','mo 1pdm',
     $                       'mo 2pdm')
               else if (citype.eq.'m903') then
                  call mn903(cr(nda),cr(nda),ncora,'density',
     $                       cr(nda),cr(nda),'mcscr','mcscr',' ',i,'mc')
               end if
c
               call iosys('read real "mo 1pdm" from mcscr',
     $                     nnao,cr(id2),0,' ')
               do 9902 j=1,nnao
                  cr(iad1-1+j)=cr(iad1-1+j)+cr(id2-1+j)*weight(i)
 9902          continue
c
               call iosys('read real "mo 2pdm" from mcscr',
     $                     ntnao,cr(id2),0,' ')
               do 9903 j=1,ntnao
                  cr(iad2-1+j)=cr(iad2-1+j)+cr(id2-1+j)*weight(i)
9903           continue
9901        continue
            call iosys('write real "mo 1pdm" to mcscr',
     $                  nnao,cr(iad1),0,' ')
            call iosys('write real "mo 2pdm" to mcscr',
     $                  ntnao,cr(iad2),0,' ')
         end if
...
c
         write(iout,90901)
90901    format(10x,'second order option invoked .. ')
c
