     ...
     $                  c,nmo,bftoso,csym,lsym,s,t1,t2,t3,t4,norm)
...
c
 ...
 1015 format(1x,'broken symmetry: ignored components --')
 1020 format(/6x,'mo:',i4,2x,'irrep:',a8)
c
c     start with a copy of the vectors in csym.
      call vmove(csym,c,nbf*nbf)
c
      salc=1
...
c              if there is anything left, we wish to keep it.
               if(ovrlap.ge.ovrtst) then
                  ...
                  call vmove(t3(1,nkept),t1(1,keep),nbf)
                  call vmove(csym(1,keep),t1(1,keep),nbf)
                  lsym(keep)=irrep
