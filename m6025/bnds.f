*deck bnds
      subroutine bnds(unit6,unit9,debug)
...
c     v(k) below or on l bd
   56 write(unit9,5) k,v(k),bl(k)
      v(k) = v(k) + eps
      vma  = vma + eps
      if (vma .lt. 0.d0) stop
c     nbd(k)=1, l bd only, v(k) above l bd
   58 u(ind)=1.0d0/sqrt(vma)
      go to 80
c     nbd(k)=2, l and u bds
   60 vma=v(k)-bl(k)
      sum1=0.5d0*(bu(k)+bl(k))
      if(vma)62,62,64
c     l and u bds, v(k) below or on l bd
   62 write(unit9,10) k,v(k),bl(k),bu(k)
...
c     l and u bds, v(k) between
      if (vmb .lt. 0.d0)   go to 68
      write (unit9,10) k,v(k),bl(k),bu(k)
      v(k) = v(k)-eps
      vmb  = vmb -eps
      if (vmb .gt. 0.d0) stop
   68 sum2=(bu(k)-bl(k))*.5d0
      sum1=v(k)-sum1
      sum2=sum1/sum2
      u(ind)=gtan(sum2)
      go to 80
...
c     u bd only, v(k) above or on u bd
   76 write(unit9,11) k,vk,bu(k)
c     u bd only, v(k) below u bd
      bmv = bmv + eps
      v(k) = v(k) - eps
      if (bmv .lt. 0.d0) stop
   78 u(ind)=1.0d0/sqrt(bmv)
   80 continue
      if (irdb.eq.1)  go to 161
      if (debug) then
          write(unit6,7)
          write(unit6,8) (u(k),k=1,nuvar)
      endif
      return
...
c     nbd(k)=1, l bd only
   94 v(k)=bl(k)+1.0d0/uksq
      go to 112
c     nbd(k)=2, l and u bds
  106 sum=u(ind)
      v(k)=0.5d0*(bu(k)+bl(k) +(bu(k)-bl(k))*gatan(sum))
      go to 112
c     nbd(k)=3, u bd only
  110 v(k)=bu(k)-1.0d0/uksq
  112 continue
      if (ngrad .ne. 1 )           go to 161
  167 ind=0
      do 160 k=1,nu
      if(nuflag(k)) 160,168,160
  168 ind=ind+1
      if(nbd(k)-2) 170,169,170
  169 us=1.d0-abs(gatan(u(ind)))
      if(us-usqd) 171,171,160
  171 if(rus .lt. 0.d0)  go to 160
      lter=lter+1
   ...
   16 format(' bnds 171 v(',i2,') = ',e20.12,'nbdsum=',e20.12,'us=',e20.
     112,'no.  indep var red. to ',i3  )
      write(unit9,16) k,v(k),nbdsum,us,nuvar
      go to 172
  170 uksq=u(ind)**2
      lter = lter + 1
      if ( lter .lt. 13 )          go to 160
      lter = 0
      if(uksq .lt. usqmx .or. rus .lt. 0.d0 ) go to 160
      nuvar=nuvar-1
      write(unit9,15) k,v(k),nbdsum,uksq,nuvar
...
c     l bd only
  126 sum=-sum
      go to 130
c     nbd(k)=2, l and u bds
  128 sum=(1.0d0+abs(u(ind  )))**2
      sum=.5d0*(bu(k)-bl(k))/sum
  ...
  150 continue
      if (debug) then
          write(unit6,12)sum1
          write(unit6,14)(gradu(k),k=1,nuvar)
      endif
  154 return
    1 format(36i2)
    2 format(e8.2)
