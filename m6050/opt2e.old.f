...
c***end prologue       m6050
      ...
      logical logkey, grdtyp, logky, uvec
      character *4096 ops
      character *1600 card
      character *13 grdnam
...
c----------------------------------------------------------------------c
     ...
     1      10*nolam*pntbuf+4*pntbuf+110
      ...
      scr=vlamda+nolam*10*pntbuf
      fact=scr+4*pntbuf
      lval=wpadti(fact+101)
      mval=lval+nobfn
      ...
      if (.not.uvec) then
          ...
          l2=eigvec
          do 20 i=1,nolam
             call posinp('$term-'//itoc(i),cpass)
             call cardin(card)
             call fparr(card,'eigenvalue',z(l1),2,' ')
             l1=l1+2
             call fparr(card,'eigenvector',z(l2),2*nobfn,' ')
             l2=l2+2*nobfn
   20     continue
      else
...
c----------------------------------------------------------------------c
             do 300 ncall=1,4         
                call vlam(z(vlamda),z(grid),z(eigvec),z(scr),z(gam),
     1                    z(del),charge,z(fact),a(lval),a(mval),nobfn,
     2                    nolam,noptrg,ncall,spin,logky(5),logky(7))
  300        continue
             call sumlam(z(vlamda),z(grid),noptrg,nolam)
