...
c***end prologue       gnquad
      program gnquad 
      parameter ( nocen=10 , numshl=300 )
      ...
      character*1600 card
      integer wpadti
      logical logkey, posinp, prnt, voron, noscat, nonsep, yukon
      logical nodisk, onegrd
      common/io/ inp,iout
...
c
      ...
      niter=intkey(ops,'m6200=number-of-voronoi-iterations',3,' ')
      alpha=fpkey(ops,'m6200=exponential-scattering-grid-cutoff',
     1                 1.d0,' ')
      prnt=logkey(ops,'print=m6200=grid',.false.,' ')
      voron=logkey(ops,'m6200=voronoi=off',.false.,' ')
      cutoff=fpkey(ops,'m6200=scattering-grid-cutoff',10.d0,' ')
      noscat=logkey(ops,'m6200=no-scattering-center',.false.,' ')
      onegrd=logkey(ops,'m6200=only-one-grid',.false.,' ')
      if (onegrd) then
          noscat=.false.
          voron=.true. 
      endif
      nodisk=logkey(ops,'m6200=no-disk-output',.false.,' ')
      yukon=logkey(ops,'m6200=yukawa=on',.false.,' ')
      yn='yes'
      if (noscat) then
          yn='true'
      endif    
...
c scattering center coordinates are read in here. the default is (0.,0.,0.)
  ...
  500 continue
      call iosys ('write integer "max number of shells" to '//
     1            'lamdat',1,maxshl,0,' ') 
     ...
     2          ncplus*ncplus
      if (nonsep) then
          nwords=nwords+3*maxang+max(nrmax*nrmax*maxang,maxgrd)
      else
          nwords=nwords+max(nrmax*nthmax*nphmax,maxgrd)+nthmax*nphmax          
      endif
      ...
      call memory(nwords,pz,ngot,'grid',0)
      j1=1
      j2=j1+nrmax*maxshl
      j3=j2+max(nthmax,maxgrd)
      j4=j3+nphmax
      jadd=j4+nrmax*(nrmax-1)*maxshl
      j5=jadd+nrmax
      j6=j5+nthmax
      j7=j6+nphmax
      j8=j7+nthmax
...
c        
     ...
     1               z(jadd),z(j5),z(j6),z(j16),z(j17),z(j5),z(j7),
     2               z(j8),z(j9),z(j10),z(j11),z(j13),z(j14),z(j15),
     3               nshell,nrmax,nthet,nphi,ngrid(i),nwts(i),ncent,
     4               ncplus,prnt,nleb,nang,nonsep,yukon,nodisk)
         j13=j13+3*ngrid(i)
         j14=j14+nwts(i)
         j15=j15+3*ngrid(i)
  600 continue
     ...
     1                  z(j11),z(j2),alpha,cutoff,z(j17),ncent,ncplus,
     2                  ngrid,nwts,niter,nr,nrmax,maxshl,noscat,
     3                  nonsep,prnt,yukon,nodisk)  
      else
