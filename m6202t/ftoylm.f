...
c***end prologue       m6202
      ...
      parameter ( dimcen=10 , dimshl=100, ngauss=100 )
      character*4 chvr1
      ...
      character*80 title
      character*800 card
      logical logkey, prnty, prntlm, tstnrm, ylmon
      logical prntyu, prntg, nonsep, ovride
      logical spln, onlysp, nodisk
      real*8 z, znuc, center, alpha
...
c----------------------------------------------------------------------c
      ...
      prntyu=logkey(ops,'print=m6202=gaussian',.false.,' ')
...
c----------------------------------------------------------------------c
c                     read in gaussians                                c
c----------------------------------------------------------------------c
      call posinp('$gaussians',cpass)
      call cardin(card)
...
c----------------------------------------------------------------------c
     ...
     1                     (center(ii,nc(i)),ii=1,3)
             call gsn(z(gauss),nx(i),ny(i),nz(i),alpha(i),
