...
!***********************************************************************                
  ...
  INTEGER                               :: i
  CHARACTER(LEN=8), DIMENSION(1)        :: rowlab          
...
!
  ...
  END IF
!
  ALLOCATE( thet_ang%q(1:thet_ang%n_pts),thet_ang%sin_theta(1:thet_ang%n_pts),             &
  ...
  DO i=1,thet_ang%n_pts
     thet_ang%sin_theta(i)=SQRT((1.d0-thet_ang%q(i)*thet_ang%q(i)))
  END DO
  IF (print_gauss(1) == .true. ) THEN
      rowlab='Cos(theta)'
      Call Print_Matrix(type_real_vector,thet_ang%q,title='Cos Theta',frmt='fr')
      rowlab='Sin(theta)'
      Call Print_Matrix(type_real_vector,thet_ang%sin_theta,title='Sin Theta',frmt='fr')
      rowlab='weights'
      Call Print_Matrix(type_real_vector,thet_ang%wt,title='Theta Weights',frmt='fr')
  END IF
!*****************************************************************************80
