...
!deck Lebedev_Quadrature
!***begin prologue     Lebedev_Quadrature
!***date written       20140706  (yyyymmdd)                                                
!***revision date                (yyyymmdd)                                                
!***keywords           Lebedev_Quadrature
...
!
  DO  m = 1, order
      ...
      zn(m,0) = 1.0D+00
      DO k = 1, n
         xn(m,k) = xn(m,k-1) * ang%q(1,m) * ang%q(1,m) 
         yn(m,k) = yn(m,k-1) * ang%q(2,m) * ang%q(2,m) 
         zn(m,k) = zn(m,k-1) * ang%q(3,m) * ang%q(3,m) 
      END DO
  END DO
!
  err_max = 0.0D+00
...
1 Format (/,10x,'The maximum error in the test integral = ',E20.12)
!
!*****************************************************************************80
...
!*****************************************************************************80
!*****************************************************************************80
  SUBROUTINE GEN_OH ( ang, q, w )
  IMPLICIT NONE
  TYPE(LEBEDEV)                               :: ang
  REAL(idp), DIMENSION(3,1)                   :: q
...
!
  ...
  call gen_oh(ang,ang%q(1,ang%num), ang%w(1))
!
  ang%num = ang%num - 1
!*****************************************************************************80
...
!
  ...
  call gen_oh (ang, ang%q, ang%w)
!
  ang%code = 3
  ang%wt   = 0.7500000000000000D-01
  call gen_oh( ang, ang%q(1,ang%num), ang%w(ang%num))
!
  ang%num = ang%num - 1
!*****************************************************************************80
...
!
  ...
  INTEGER                                     :: i
  REAL(idp)                                   :: r
  CHARACTER(LEN=1), DIMENSION(3)              :: rowlab
  rowlab(1)='x'
  rowlab(2)='y'
  rowlab(3)='z'
  write(iout,*) 'order = ',order
  Call Print_Matrix(type_real_matrix,ang%q,3,order,title='Cartesian Points on'  &
                    //' unit sphere',frmt='fr',rowlab=rowlab)
  Call Print_Matrix(type_real_vector,ang%w,title='Cartesian Weights'  &
                    //' on unit sphere',frmt='fr')
!*****************************************************************************80
  END SUBROUTINE Print_XYZ
!*****************************************************************************80
!*****************************************************************************80
  SUBROUTINE XYZ_to_THETA_PHI (ang)
!*****************************************************************************80
!
  IMPLICIT NONE
  TYPE(LEBEDEV)                               :: ang
  ...
  REAL(idp)                                   :: r
  REAL(idp)                                   :: atan2
  CHARACTER(LEN=8), DIMENSION(6)              :: rowlab
  rowlab(1)='theta'
  rowlab(2)='cos theta'
  rowlab(3)='sin theta'
  rowlab(4)='phi'
  rowlab(5)='cos phi'
  rowlab(6)='sin phi'
  ALLOCATE(ang%phi(1:order), ang%cos_phi(1:order), ang%sin_phi(1:order), ang%theta(1:order),   &
           ang%cos_theta(1:order), ang%sin_theta(1:order))
  write(iout,*) 'order = ',order
  ang%cos_theta(:)= ang%q(3,:) ! Cos theta
  ang%sin_theta(:) = sqrt ( (1.d0 - ang%cos_theta(:)*ang%cos_theta(:) ) ) ! Sin theta
  ang%theta(:) = acos ( ang%cos_theta(:) ) ! Theta
  ang%phi(:)=ATAN2(ang%q(2,:),ang%q(1,:))
  ang%cos_phi(:) = cos ( ang%phi(:) )  ! Cos phi
  ang%sin_phi(:) = sqrt ( (1.d0 - ang%cos_phi(:)*ang%cos_phi(:) ) ) ! Sin theta
  write(iout,*)  ' FINISHED'
  IF (print_leb(2) == .true. ) THEN
      Call Print_Matrix(type_real_vector,ang%theta,title='Theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%cos_theta,title='Cos theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%sin_theta,title='Sin theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%phi,title='Phi points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%cos_phi,title='Cos Phi Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%sin_phi,title='Sin Phi Points',frmt='fr')
  END IF
!*****************************************************************************80
  END SUBROUTINE XYZ_to_THETA_PHI
!*****************************************************************************80
!deck Gauss_Ang.f
!***begin prologue     Gauss_Ang
...
!
  ...
  REAL(idp)                      :: r
  REAL(idp)                      :: atan2
  ALLOCATE(ang%cos_theta(1:ang%nang),ang%phi(1:ang%nang),ang%sin_theta(1:ang%nang),ang%cos_phi(1:ang%nang),  &
           ang%sin_phi(1:ang%nang))
  DO i=1,ang%nang
     r = sqrt(ang%q(1,i)*ang%q(1,i) + ang%q(2,i)*ang%q(2,i) + ang%q(3,i)*ang%q(3,i))
     ang%cos_theta(i)=ang%q(3,i)/r
     ang%sin_theta(i)=SQRT(1.d0-ang%cos_theta(i)*ang%cos_theta(i))
     IF (ang%sin_theta(i) /= 0.d0) THEN
         ang%cos_phi(i)=ang%q(1,i)/(r*ang%sin_theta(i))
         ang%sin_phi(i)=ang%q(2,i)/(r*ang%sin_theta(i))
     ELSE
         ang%cos_phi(i)=1.d0
         ang%sin_phi(i)=0.d0
     END IF
     ang%phi(i)=ATAN2(ang%sin_phi(i),ang%cos_phi(i))
  END DO
  IF (print_leb(3) == .true. ) THEN
      Call Print_Matrix(type_real_vector,ang%cos_theta,title='cos theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%sin_theta,title='sin theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%cos_phi,title='cos phi Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%sin_phi,title='sin phi Points',frmt='fr')
