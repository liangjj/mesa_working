...
!
!  Call Ang(ang)
  IF ( print(1) == .true. ) THEN
       write(iout,*) 'Calling Print XYZ'
       Call Print_XYZ(ang)
  END  IF
  IF ( print(2) == .true. ) THEN
       write(iout,*) 'Calling XYZ to Angles'
...
!
  ...
  Call Print_Matrix(type_real_matrix,ang%q,3,order,title='Cartesian Points', &
                    frmt='fr',rowlab=rowlab)
  Call Print_Matrix(type_real_vector,ang%w,frmt='fr',title='Cartesian Weights')
!*****************************************************************************80
...
!
  ...
  INTEGER                                     :: i
  REAL(idp)                                   :: r
  CHARACTER(LEN=8), DIMENSION(2)              :: rowlab
  rowlab(1)='phi'
  rowlab(2)='theta'
  ALLOCATE(ang%pts(1:2,1:order))
  write(iout,*) 'order = ',order
  DO i = 1, order
     ang%pts(2,i) = acos ( ang%q(3,i) ) ! Theta
     r = sqrt(ang%q(1,i)*ang%q(1,i) + ang%q(2,i)*ang%q(2,i) )
     IF ( 0.0D+00 < r ) THEN
          ang%pts(1,i) = acos ( ang%q(1,i) / r )
     ELSE
          ang%pts(1,i) = acos ( ang%q(1,i) )
     END IF
     IF ( ang%q(2,i) < 0.0D+00 ) THEN
          ang%pts(1,i) = - ang%pts(1,i) 
     END IF
     ang%pts(1,i) = ang%pts(1,i) * 180.d0 / pi  
     ang%pts(2,i) = ang%pts(2,i) * 180.0D+00 / pi
  END DO
  write(iout,*)  ' FINISHED'
  IF (print(2) == .true. ) THEN
      Call Print_Matrix(type_real_matrix,ang%pts,2,order,title='Angular Points', &
                        frmt='fr',rowlab=rowlab(1:5))
  END IF
  DEALLOCATE(ang%pts)
!*****************************************************************************80
...
!
  ...
  DO i=1,ang%nang
     ...
     ang%thetpt(i)=ang%q(3,i)/r
     ang%sin_thet(i)=SQRT(1.d0-ang%thetpt(i)*ang%thetpt(i))
     IF (ang%sin_thet(i) /= 0.d0) THEN
         ang%cos_phi(i)=ang%q(1,i)/(r*ang%sin_thet(i))
         ang%sin_phi(i)=ang%q(2,i)/(r*ang%sin_thet(i))
     ELSE
         ang%cos_phi(i)=1.d0
         ang%sin_phi(i)=0.d0
     END IF
     ang%phipt(i)=ATAN2(ang%sin_phi(i),ang%cos_phi(i))
  END DO
  IF (print(1) == .true. ) THEN
      Call Print_Matrix(type_real_vector,ang%thetpt,title='cos theta Points',frmt='fr')
      Call Print_Matrix(type_real_vector,ang%sin_thet,title='sin theta Points',frmt='fr')
      ...
      Call Print_Matrix(type_real_vector,ang%phipt,title='phi points',frmt='fr')
  END IF
!*****************************************************************************80
