...
c
      ...
      real*8 expon(nv),xyz(nv,0:imax,0:jmax,3,0:nderiv,2),h(mxpts)
      real*8 a(nv,-nderiv:nderiv),b(nv),bp2(nv),bm2(nv),wt(mxpts)
...
c
      if (n.ne.nv) then
         write (iout,99) n,nv
   99    format (//,' ##### m704: tints, error, n and nv:',2i8,//)
         call lnkerr('m704: tints error in vector lengths')
      end if
c
...
c
      call rzero(xyz,n*(imax+1)*(jmax+1)*3*2*(nderiv+1))
c
...
c
                  ...
                  do 11 i=1,n
                     xyz(i,ni,nj,coord,0,2)=xyz(i,ni,nj,coord,0,2)+
     #                           (nj*(nj-1)*bm2(i)-
     #                     (2*nj+1)*b(i)*alpha(i,2)+
     #                         bp2(i)*alpha(i,2)**2)*(-0.5d+00)*
     #                     a(i,0)*wt(point)
c
                     xyz(i,ni,nj,coord,0,1)=xyz(i,ni,nj,coord,0,1)+
     #                            a(i,0)*b(i)*wt(point)
c
                    xyz(i,ni,nj,coord,1,2)=xyz(i,ni,nj,coord,1,2)+
     #                           (nj*(nj-1)*bm2(i)-
     #                     (2*nj+1)*b(i)*alpha(i,2)+
     #                         bp2(i)*alpha(i,2)**2)*(-0.5d+00)*
     ...
     #                         wt(point)
c
                     xyz(i,ni,nj,coord,1,1)=xyz(i,ni,nj,coord,1,1)+
     #                                   b(i)*
     #                     (2*alpha(i,1)*a(i,+1)-ni*a(i,-1))*
     #                         wt(point)
   11             continue
                  if (nderiv.eq.2) then
                     do 12 i=1,n
                        xyz(i,ni,nj,coord,2,2)=xyz(i,ni,nj,coord,2,2)+
     #                       (nj*(nj-1)*bm2(i)-
     #                       (2*nj+1)*b(i)*alpha(i,2)+
     #                       bp2(i)*alpha(i,2)**2)*(-0.5d+00)*
     #                       (4*alpha(i,1)**2*a(i,+2)-
     $                       (2*ni+1)*2*alpha(i,1)*a(i,0)+
     #                       ni*(ni-1)*a(i,-2)) * wt(point)
c
                     xyz(i,ni,nj,coord,2,1)=xyz(i,ni,nj,coord,2,1)+
     #                                   b(i)*
     #                       (4*alpha(i,1)**2*a(i,+2)-
     $                       (2*ni+1)*2*alpha(i,1)*a(i,0)+
     #                       ni*(ni-1)*a(i,-2)) * wt(point)
...
c
      do 9 j=0,jmax
         do 8 i=0,imax
            do 78 k=0,nderiv
               call vmul(xyz(1,i,j,3,k,1),xyz(1,i,j,3,k,1),expon,n)
               call vmul(xyz(1,i,j,3,k,2),xyz(1,i,j,3,k,2),expon,n)
   78       continue
    8    continue
    9 continue
c
