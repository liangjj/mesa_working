...
c
               ...
               if(space.lt.lenmin) then
                  ...
                  if(mincor.gt.maxtop) then
                     write(iout,*)' insufficient memory for m711: '
...
22011                format(' nij nkl fac1 fac2 npint lenblk',
     $                      6(2x,i8))
                  ...
                  call getscm(mincor,z,maxcor,' m711:bhl ',0)
                  space=maxcor-tmptop-100
               endif
c
c
...
c
               ...
               if (top.gt.maxcor) then
                 call getscm(top+1000,z,maxcor,'m711:driver',0)
                 write(iout,*)' m711:  top maxcor ',top,maxcor
...
c
               if (logkey(ops,'m711=print=local-density-matrices',
     $                   .false.,' ')) then
...
C         call lnkerr('energies do not agree !!!')
      end if
c
c
      return
