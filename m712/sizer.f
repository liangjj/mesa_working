*deck @(#)sizer.f	5.1  11/6/94
     ...
     #                  nshell,mincor)
c
c***begin prologue     sizer
c***date written       851113   (yymmdd)
c***revision date      870322   (yymmdd)
c
...
c
               if (abs(top1-(tmptop+fac1*len)).gt.4) then
                  call lnkerr('internal error in sizer with top1')
               end if
c
               if (nmax+mmax.lt.0) then
                  temp1=ab
...
c
               if (abs(top2-(tmptop+fac2*len)).gt.4) then
                  call lnkerr('internal error in sizer with top2')
               end if
c
               top=max(top1,top2)
               maxc=max(top,maxc)
