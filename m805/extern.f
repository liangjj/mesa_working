      ...
      do 10 lev=2,orbfrm+1
         ...
         do 9 row=levpt(lev)+1,levpt(lev)+levnr(lev)
            ...
            do 8 case=1,4
               rowm1=arc(case,row)
               if (rowm1.gt.0) then
                  if (ia.eq.1.and.case.eq.4) wtab(levm1)=wt(case,row)
                  if (ia.eq.1.and.case.eq.3) wtw(levm1,is)=wt(case,row)
                  if (ib.eq.2.and.case.eq.2) wtx(levm1,is)=wt(case,row)
                  if (ib.eq.1.and.case.eq.2) wty(levm1)=wt(case,row)
               end if
    8       continue
    9    continue
   10 continue
