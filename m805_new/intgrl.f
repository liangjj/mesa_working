*deck @(#)intgrl.f	5.1  11/6/94
     ...
     #,                 ijww,klww,nklww
     #,                 nbf,norbs,symorb,numij,nsym,orbfrm,out,errout
     #,                 absmax,nmax,ngroup)
c
c***********************************************************************
c                                                                      *
...
c
      ...
      integer numint
c
c      common /option/ option(16),label(26),ident(26)
c      common /bloksz/ blksiz,absmax,maxsiz
c      common /tapes/  out,errout,input,drttap
c      common /dimens/ nbf,nsym,norbs,nrowsp,nrows4p,nrows,nrows4
c     #,               nlevs,nrefs,nrowoc,nrow4o,nwks,nwksoc,nlevoc
c     #,               orbfrm,symorb,numij,ngroup,numint,nmax,nspc,nvref
c     #,               nijvir
c
...
c
      call izero(kadd,symorb)
      call izero(ladd,symorb)
c
      no34x=0
c      if (option(5).lt.0) no34x=-1
c      if (option(5).ne.0.and.option(5).ne.-1) blksiz=abs(option(5))*100
c      if (option(5).ne.0.and.option(5).ne.-1) absmax=blksiz
      blksiz=absmax
c
      numint=0
      jadmax=0
...
c
  300 continue
   ...
   45 continue
      nmax=blksiz
c     if (maxsiz-2*nwks.lt.blksiz) nmax=maxsiz-2*nwks
...
c
   59 continue
      ngroup=group
      write (out,21)
      write (out,22) numint,ngroup,nmax
   21 format(t6,'integral information:')
   22 format (t10,'number of integrals:',i9,
     $      /,t10,'number of groups:   ',i9,
     $      /,t10,'group size:         ',i9)
c
