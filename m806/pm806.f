*deck @(#)pm806.f	5.1  11/6/94
      subroutine pm806(z,a,maxcor)
c
c***begin prologue     m806
...
c
      write(ioutq,*) ' m806:kohn drt generation'
...
c
c      if (nsym.eq.0) call iosys('read integer nsym from rwf',1,nsym,0,' ')
      if (nsym.eq.0) nsym=1
...
c
         if (pandq.and.pq.eq.1) then
            ...
            call iosys('write integer levpt to pdrt',nlevs,z(levpt),0,
     $                  ' ')
            call iosys('write integer levnr to pdrt',nlevs,z(levnr),0,
     $                  ' ')
            ...
            call iosys('write integer arc to pdrt',4*nrows,z(arc),0,
     $                  ' ')
            call iosys('write integer nlwks to pdrt',nrows,z(nlwks),0,
     $                                                           ' ')
            nwksp=nwks
            nrowsp=nrows
c
...
c
         call iosys('read integer levpt from pdrt',nlevs,z(plevpt),0,
     $               ' ')
         call iosys('read integer levnr from pdrt',nlevs,z(plevnr),0,
     $               ' ')
         ...
         call iosys('read integer arc from pdrt',4*nrowsp,z(parc),0,
     $               ' ')
         call iosys('read integer nlwks from pdrt',nrowsp,z(pnlwks),0,
     $               ' ')
c
...
c
      if (pass.eq.0) then
      call iosys('write integer "symmetries in ci" to rwf',1,
     $            nsym,0,' ')
...
c
      ...
      call iosys('write integer bfcode to rwf',nrefs*nbf,z(bfcode),0,
     $            ' ')
      ...
      call iosys('write integer arc to rwf',4*nrows,z(arc),0,' ')
      ...
      if (orbfrm.gt.0) then
         ...
         call iosys('write integer wtw to rwf',orbfrm*nsym,z(wtw),0,' ')
         call iosys('write integer wtx to rwf',orbfrm*nsym,z(wtx),0,' ')
     ...
     #                         1,nrows,0,' ')
      call iosys('write integer "'//symchr//' maxb" to rwf',1,maxb,0,
     $                                                          ' ')
c
     ...
     #                         nlevs,z(levnr),0,' ')
      call iosys('write integer "'//symchr//' a" to rwf',nrows,z(a),0,
     $                                                           ' ')
      call iosys('write integer "'//symchr//' b" to rwf',nrows,z(b),0,
     $                                                           ' ')
       call iosys('write integer "'//symchr//' x" to rwf',nrows,z(x),0,
     $                                                            ' ')
      call iosys('write integer "'//symchr//' s" to rwf',nrows,z(s),0,
     $                                                           ' ')
