*deck @(#)m811.f	1.3  7/30/91
      program m811
c***begin prologue     m811
c***date written       850601   (yymmdd)
c***revision date      910225   (yymmdd)
c
c  15 december 2005   bis at nsf
c       making use of unique drt files to generate integrals.
c 
...
c
c        find out from the input what file the scattering 
...
c
c        from the key we now know which drt we are going to use.
c
...
c        in the case that the ci to be used is m901, m811 should 
c        be followed by m821 to sort the guga integrals into 
c        supermatrix format.
c
c        if one is doing full ci(m903), the standard canonical 
c        transformed integral set is needed, and so toguga is turned off 
c        and tocan turned on.
c        this is also the case if one is doing some form of ci 
c        optimization. the canonical transformed integrals are need 
c        by the coupled perturbed equation links.  this link 
c        produces canonical integrals, and is followed by m820 
c        which sorts to guga order, and then m821 which forms
c        supermatrices to be read by m901 or m902.
c
c        if one is doing a scattering calculation which requires a step to
c        produce a "zero'd" integral file, which is dome in m819, then you
c        need to produce a canonical set.  the canonical set is zeroed in m819
c        and then resorted to guga order in m822 to make it available for the
c        m940 CI.
c
         toguga=.true.
...
c
         ...
         schmrr=fpkey(ops,'schmidt-tol',1.d-08,' ')
...
c
     ...
     $               1,nbf,0,' ')
c
c     ----- here we need to decide whether we want 
c           guga or canonical integral files based on what
c           come next.
c
...
c
c     -----  start writing to the dsk file what is needed.
c
...
c
     ...
     #                  a1(iout),a1(orbsym),end,ngroup,nrefs,symorb,
     #                  numij,nbf,norbs,levfrm,nlevs,nrows,nsym,dsk)
         end if
c
...
c
         ...
         if(.not.logkey(ops,'mcscf',.false.,' ')) then
            call iosys('read real '//occ//' from rwf',nbf,
     #                  z2(tempe),0,' ')
         end if
c
...
c
c    ----- read the one elctron integrals and process them
c
...
c
         call chknrm(z2(tempc),z3(s),z3(t1),z3(t2),z3(t3),
     #               schmrr,nbf,nnp)
c
...
c      
c
c    ----- process the two electron integrals.  to do this
c          requires some sorting which is done using
c          the general sorting routines in the library
c
