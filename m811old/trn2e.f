...
c   11 july, 1993      rlm at lanl
c        modifying so as to destroy the ints file as soon as the half
c        transformation is finished.
c
...
c
c
c     see if we are to remove the ints file after the half
c     transformation(the default), or have been asked to keep
c     them around for some reason.
      kpints=logkey(ops,'destroy-sorted-integral-file',.false.,' ')
c
...
c    #                   val,lab,bin,numij,prnt)
            call sorter('with bin',asort,asort,0,numij,lab,bin,
     #                   val,0,0,0,prnt)
    3    continue
    4 continue
c     
c     --- destroy the ints file, if we no longer need it.
      if (kpints) then
...
c
     ...
     #                 0,0,0,'guga integrals','gints',prnt)
      end if
      if (tocan) then
