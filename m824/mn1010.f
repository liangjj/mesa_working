*deck @(#)mn1010.f	5.1  11/6/94
      subroutine mn1010(z,a,der)
c
...
c      if (prnt) then
c         write(iout,2)
c         write(iout,3) maxcor
c         if(ntriang.eq.nnp) then
c            write(iout,4)
c         else
c            write(iout,5) ntriang
c         endif
c      endif
c
...
c
      call iosys('read real "nuclear charges" from rwf',-1,z(zan),
     #           0,' ')
...
c
      ...
      call iosys('read real alpha from rwf',nshell**2,z(alpha),
     $            0,' ')
      call iosys('read real beta from rwf',nshell**2,z(beta),
     $            0,' ')
...
c..bhl
     ...
     #              0,0,0,ans)
         if(ans.eq.'no') then
            call iosys('create real "mo derivative hf lagrangian"'
     ...
     #               0,0,0,ans)
         if(ans.eq.'no') then
            call iosys('create real "ao derivative hf lagrangian"'
     #                 //' on rwf',num**2*3*natoms,0,0,' ')
         else
     ...
     #                 0,0,0,' ')
         ...
         call iosys('does "ao derivative overlap integrals" exist'
     $              //' on dints',0,0,0,ans)
         if(ans.eq.'no') then
            call iosys('create real "ao derivative overlap integrals"'
     #                 //' on dints',nnp*3*natoms,0,0,' ')
         endif
...
c
      call sder(z(h),z(d),num,nnp,z(t1),z(t2),z(grad),der)
c
