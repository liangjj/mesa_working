*deck @(#)initst.f	5.1  11/6/94
      subroutine initst(int,c,s,kadd,ladd,wtw,wtx,wab,ss,
     #                  ci,si,cj,sj,h1,h2,ijxx,nklxx,ijww,nklww,
     #                  klxx,klww)
      implicit real*8 (a-h,o-z)
c
      integer xor
      integer arr,tr1,tr2,asm,os,wtw,wtx,wab,ss,symorb
      integer bmax,orbfrm
      dimension kadd(symorb),ladd(symorb),wtw(orbfrm,nsym)
      dimension wtx(orbfrm,nsym),wab(orbfrm),ss(norbs)
      integer ijxx(numij ),nklxx(nsym,orbfrm),ijww(numij )
      integer nklww(nsym,orbfrm)
...
c***************************** xx 4x ***********************************
c
      entry xx4x(int,c,s,ijxx,nklxx,wtx,ss)
c
      jminh=jming
...
c***************************** ww 4x ***********************************
c
      entry ww4x(int,c,s,ijww,nklww,wtw,wab,ss)
c
      jminh=jming
      if (jming.eq.1) jminh=0
c
...
c******************************** yx ***********************************
c
      entry yx(int,c,s,h1,ci,si,wtx,ladd)
c
      iyx20=iyx20+1
      a=val1
      b=val1*val2
...
c
  711 continue
      ...
      call hi    (h1(1),isym,numi,int,ladd)
      do 1006 ivc=1,nvc
         call apbct (s(ia,ivc),h1(1),c(jas,ivc),1,numi,numj)
         call atpbc(s(jas,ivc),h1(1),c(ia,ivc),numi,1,numj)
 1006 continue
...
c****************************** xy *************************************
c
      entry xy(int,c,s,ijxx,nklxx,h1,ci,si,wtx,ladd)
...
c******************************** yw ***********************************
c
      entry yw(int,c,s,h1,ci,si,wtw,wab,ladd)
c
      iyw19=iyw19+1
      a=val1
      b=val1*val2
...
c****************************** wy *************************************
c
      entry wy(int,c,s,ijww,nklww,h1,wtw,wab,ci,si,ladd)
c
...
c********************************** xx *********************************
c
      entry xx(int,c,s,h1,h2,ci,cj,si,sj,kadd,wtx)
c
...
c
   ...
   10 continue
      ...
      do 11 i=1,nsym
         ...
         call hiis  (h1(1),isym,numi,int,kadd)
         do 1030 ivc=1,nvc
            call square(c(ias,ivc),ci(1),numi)
...
c
   20 continue
      ...
      do 21 isym=1,nsym-1
         ...
         call hiis  (h1(1),isym,numi,int,kadd)
         call hiis  (h2(1),jsym,numj,int,kadd)
         do 1031 ivc=1,nvc
...
c
      ...
      do 31 i=1,nsym
         ...
         call hii   (h1(1),isym,numi,int,kadd)
         do 1032 ivc=1,nvc
            call square(c(ias,ivc),ci(1),numi)
 ...
 1032    continue
   ...
   35 continue
      ...
      do 36 i=2,nsym
         ...
         call hii   (h1(1),isym,numi,int,kadd)
         call hii   (h2(1),jsym,numj,int,kadd)
         do 1033 ivc=1,nvc
...
c
   40 continue
      ...
      do 42 i=1,nsym
         ...
         jas=ja+wtx(mn(isym+1),js+1)
         intal=1
         call hij   (h1(1),isym,ksym,numi,numk,int,kadd)
         do 1035 ivc=1,nvc
            call square(c(ias,ivc),ci(1),numi)
 ...
 1035    continue
   ...
   41    continue
         ...
         intal=3
         call hij   (h1(1),ksym,isym,numk,numi,int,kadd)
         do 1036 ivc=1,nvc
            call square(c(ias,ivc),ci(1),numi)
...
c
   63    continue
         ...
         jas=ja+wtx(mn(isym+1),js+1)
         intal=1
         call hij   (h1(1),jsym,ksym,numj,numk,int,kadd)
         do 1043 ivc=1,nvc
 ...
 1043    continue
   ...
   64    continue
         ...
         jas=ja+wtx(mn(isym+1),js+1)
         intal=3
         call hij   (h1(1),ksym,jsym,numk,numj,int,kadd)
         do 1044 ivc=1,nvc
...
c********************************** ww *********************************
c
      entry ww(int,c,s,kadd,h1,h2,ci,cj,si,sj,wtw,wab)
c
...
c******************************* wx ************************************
c
      entry wx(int,c,s,kadd,wtw,wab,wtx,h1,h2,ci,cj,si,sj)
c
...
c******************************* xw ************************************
c
      entry xw(int,c,s,kadd,wtw,wab,wtx,h1,h2,ci,cj,si,sj)
c
