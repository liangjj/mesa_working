...
c
      call iosys('does '//file//' exist on '//dunit,0,0,0,
     $     answer)
      if (answer.eq.'no') then
         call iosys('create real '//file//' on '//dunit,
     $        ngroup*nmax,0,0,' ')
      end if
c
      call iosys('rewind '//file//' on '//dunit,0,0,0,' ')
c
      do 2000 group=1,ngroup
         call rzero(ints,nmax)
         imin=imngrp(group)
         imax=imxgrp(group)
...
c
c                 ----- form this contribution to the density matrix -
c
                  loop=sdot(nlwks(inxt),s(iwt+1),1,c(jwt+1),1)+
     $                 sdot(nlwks(inxt),c(iwt+1),1,s(jwt+1),1)
                  if (iwt.eq.jwt) loop=loop*0.5d+00
c
...
c
 411              continue
                     ints(ijkl+1)=ints(ijkl+1)+acoef(levm1)*loop
                     go to 430
 412              continue
                     ints(ijkl+2)=ints(ijkl+2)+acoef(levm1)*loop
                     go to 430
  413             continue
                     ints(ijkl+3)=ints(ijkl+3)+acoef(levm1)*loop
                     go to 430
  414             continue
                     ints(ijkl+1)=ints(ijkl+1)+acoef(levm1)*loop
                     ints(ijkl+3)=ints(ijkl+3)+acoef(levm1)*d*loop
                     go to 430
  415             continue
                     ints(ijkl+3)=ints(ijkl+3)+acoef(levm1)*loop
                     ints(ijkl+2)=ints(ijkl+2)+acoef(levm1)*d*loop
                     go to 430
  416             continue
                     ints(ijkl+3)=ints(ijkl+3)+acoef(levm1)*loop
                     ints(ijkl+2)=ints(ijkl+2)+acoef(levm1)*loop
  ...
  420             continue
                     ints(ijkl+2)=ints(ijkl+2)+acoef(levm1)*loop
                     ints(ijkl+1)=ints(ijkl+1)+acoef(levm1)*d*loop
  430             continue
c
c
                  go to 400
...
c
         ...
         go to 201
 1000 continue
         call iosys('write real '//file//' to '//dunit//
