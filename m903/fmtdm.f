*deck @(#)fmtdm.f	5.1  11/6/94
      subroutine fmtdm(uparc,dnarc,upwt,dnwt,upnwks,dnnwks,a,b,sval,
     #                  levpt,levnr,rowoff,
...
c***description
c
c          fmtdm forms the product vector s of tha hamiltonian matrix
...
c
      real*8 acoef(nlevs),z(*),bcoef(nlevs)
      integer iz(maxcor)
...
c
      ...
      left=maxcor-top+1
c
      call onetdm(uparc,dnarc,upwt,dnwt,upnwks,dnnwks,a,b,
     #           levpt,levnr,rowoff,
     #           nrows,nlevs,norbs,
     #           acoef,irowsv,jrowsv,iwtsv,jwtsv,pagesv,segsv,
...
c
         ...
         ptsym=offsym(sym)+1
c
         call twotdm(uparc,dnarc,upwt,dnwt,upnwks,dnnwks,a,b,
     #              levpt,levnr,rowoff,
     ...
     #              acoef,irowsv,jrowsv,iwtsv,jwtsv,pagesv,segsv,
     #              levdiv,nnps,c,nwks,s,h,g(ptsym),bcoef,coeffs,
     #              maxwks,sym-1,sval,isval(pt),z(iadtwp(top)),iz(top),
     #              left,iz(botnum),iz(topnum),iz(dnnum),iz(upnum),
     #              jmin,jmax,nnp,ijpt)
    1 continue
c
      return
