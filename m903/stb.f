*deck @(#)stb.f	5.1  11/6/94
      subroutine stb(gec,nnp,nkwks,up,iup,wpti4,ntops,
     #                dn,idn,nbots,jdnnwk,kdnnwk,
     #                offset,s,nwks,asort,jsort,jnsort,aov,jov,
...
c           calling routine.
c
      real*8 gec(nnp,nkwks),up(4,ntops),dn(4,nbots)
      integer ijpt(nij)
      integer iup(wpti4,ntops),idn(wpti4,nbots)
c
...
cxxx$ ivdep
            do 30 k=1,kdnnwk
               ij=ijpt(ii+jnsort(k))
               s(jsort(k)+jtop)=s(jsort(k)+jtop)+
     #                up(4,top)*asort(k)*gec(ij,k+ktop)
   30       continue
c
            do 35 i=1,n
               ij=ijpt(ii+jnov(i))
               s(jov(i)+jtop)=s(jov(i)+jtop)+up(4,top)*aov(i)*
     #                                gec(ij,kov(i)+ktop)
   35       continue
c
   36       continue
