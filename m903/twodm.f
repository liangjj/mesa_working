*deck @(#)twodm.f	5.1  11/6/94
      subroutine twodm(uparc,dnarc,upwt,dnwt,upnwks,dnnwks,a,b,
     #                  levpt,levnr,rowoff,
...
c
         topcor=ec+nnp*nkwks
c
         if (topcor.gt.maxwp) call lnkerr('core allocation for ec')
c
         maxk=0
 2100    continue
            mink=maxk+1
...
ci            call ebc(z(gec),g,z(ec),nnp,nnp,nkwks)
            call apbct(g,z(ec),z(ec),nnp,nkwks,nnp)
c
