*deck @(#)twoel.f	5.1  11/6/94
      subroutine twoel(uparc,dnarc,upwt,dnwt,upnwks,dnnwks,a,b,
     #                  levpt,levnr,rowoff,
...
c         if (nup.lt.1) call lnkerr('not enough core')
         nkwks=nup*kdnnwk
c
         gec=ec+nnp*nkwks
         topcor=gec+nnp*nkwks
c
         if (topcor.gt.maxwp) call lnkerr('core allocation for gec')
c
         maxk=0
 2100    continue
            mink=maxk+1
...
c
 ...
 3100       continue
c
            call ebc(z(gec),g,z(ec),nnp,nnp,nkwks)
c
            pt=1
            do 3500 jrow=jmin,jmax
               deltaa=a(jrow)-ia(irow)
               deltab=b(jrow)-ib(irow)
c
               if (deltaa.eq.0.and.(deltab.eq.-1.or.deltab.eq.1).or.
...
c
                  ...
                  pt1=pt+ntops*4
                  nbots=botnum(jrow)
                  if (ntops.eq.0) go to 3400
                  if (nbots.eq.0) go to 3400
c
                  call stb(z(gec),nnp,nkwks,z(pt),z(pt),wptoin(4),
     #                      ntops,z(pt1),z(pt1),nbots,dnnwks(jrow),
     #                      idnnwk(irow),rowoff(jrow),s,nwks,
     #                      z(asort),z(jsort),z(jnsort),z(aov),
     #                      z(jov),z(kov),z(jnov),maxov,
     #                      mink,maxk,nij,ijpt)
c
 3400             continue
                  pt=pt+4*(ntops+nbots)
 ...
 3500       continue
c
            do 3200 jrow=jmin,jmax
               deltaa=a(jrow)-ia(irow)
               deltab=b(jrow)-ib(irow)
               deltau=xor(xor(sval(jrow),sval(1)),xor(isval(irow),sym))
               deltal=xor(sval(jrow),isval(irow))
c
               if (deltaa.ne.0.or.deltab.ne.0) go to 3200
c
               jupnwk=upnwks(jrow)
               jdnnwk=dnnwks(jrow)
c
               if (deltau.eq.0) then
                  ...
                  pt1=pt+4*ndnlps
c
                  call sdn(z(gec),nnp,nkwks,z(pt),z(pt),wptoin(4),
     #                  ndnlps,kdnnwk,jdnnwk,jupnwk,rowoff(jrow),
...
c
               if (deltal.eq.0) then
                  ...
                  pt1=pt+nuplps*4
c
                  call sup(z(gec),nnp,nkwks,z(pt),z(pt),wptoin(4),
     #                     nuplps,kdnnwk,jdnnwk,rowoff(jrow),
     #                     s,nwks,z(pt1),mink,maxk)
c
                  pt=pt1+jdnnwk
