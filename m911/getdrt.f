...
c
      call iosys('does wtab exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer wtab from rwf',-1,wab,0,' ')
      end if
c
      call iosys('does wtw exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer wtw from rwf',-1,wtw,0,' ')
      end if
c
      call iosys('does wtx exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer wtx from rwf',-1,wtx,0,' ')
      end if
c
      call iosys('does wty exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer wty from rwf',-1,wty,0,' ')
      end if
c
      call iosys('does ijxx exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer ijxx from rwf',-1,ijxx,0,' ')
      end if
c
      call iosys('does klxx exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer klxx from rwf',-1,klxx,0,' ')
      end if
c
      call iosys('does nklxx exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer nklxx from rwf',nsym,nklxx,0,' ')
      end if
c
      call iosys('does ijww exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer ijww from rwf',-1,ijww,0,' ')
      end if
c
      call iosys('does klww exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer klww from rwf',-1,klww,0,' ')
      end if
c
      call iosys('does nklww exist on rwf',0,0,0,answer)
      if(answer.eq.'yes') then
         call iosys('read integer nklww from rwf',nsym,nklww,0,' ')
      end if
c
    9 continue
