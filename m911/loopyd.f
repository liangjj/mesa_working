*deck @(#)loopyd.f	5.1  11/6/94
     ...
     *                  hdwgt,hdseg,hdpnt,imxgrp,imngrp,jmxgrp,jmngrp,
     #                  aint,aint1,file,unit,vector,sigma,
     #                  wtw,wtx,wty,wab,ss,
     #                  ci,si,cj,sj,h1,h2,
     #                  ijxx,nklxx,ijww,nklww,
     #                  klxx,klww)
c
c
c
      ...
      real*8 aint(nmax),aint1(norbs*norbs)
      real*8 vector(nwks,*),sigma(nwks,*)
...
c     common/dry1/idry,jdry,ksegsv,indxs(100),magic
...
c
      ...
      save nlcsmn
c
      save crite,root2,rootn2,toor2,toorn2,hsegmx,norb2
c
      crite = 0.00001d+00
      root2 = sqrt(2.0d+00)
      rootn2 = -root2
...
c------------------------------------------------------loopy---
c
      entry loopy(nabc,nlwks,           iwght,nabca,ijadd
     *,                 ijgrp,kadd,ladd,inext,iarc,jmnnxt,jmxnxt
     *,                 nabcs,isym,            isegm,jsegm,imain
     *,                 isub,iuwkmn,iuwksb,itrack,lmin,acoef,bcoef
     *,                 hdwgt,hdseg,hdpnt,imxgrp,imngrp,jmxgrp,jmngrp,
...
c..rlm     entry loopy
      lsave=levfrm
c..rlm removed variable magic from /dry1/
c     if(magic.ne.0)levfrm=0
c
c

      do 500 iblock=1,ngroup
c..rlm   call nxtblk
         call rzero(aint,nmax)
cbl      write(itape6,187)iblock,imxgrp(iblock),imngrp(iblock)
 187     format(' process dm elements from group',i4,' first level',i4
     #,      ' last',i4)
 188     continue
         imaxg=imxgrp(iblock)
         iming=imngrp(iblock)
...
c
c     ----- search for upper walks from loops -----
c
...
c        go to 1050
c
c        if(lev.gt.levfrm) go to 1050
c        if((nabca(npt)*2+nabc(npt)).le.2) go to 1060
         if (lev.ne.levfrm) go to 1050
         if ((nabca(npt).eq.1)) go to 1060
...
c..rlm        call allext
         call allext(aint,aint1,vector,sigma,ijadd,kadd,ladd,wtw,wtx,
     #               wty,wab,ss,
...
c        print,'after 320, kseg,ksegmx,lev',kseg,ksegmx,lev
         ...
         ladt=lad
c..rlm        call loopin
         call loopin(aint,aint1,vector,sigma,ijadd,kadd,ladd,wtw,wtx,
     #               wty,wab,ss,
     #               ci,si,cj,sj,h1,h2,ijxx,nklxx,ijww,nklww,
     #               klxx,klww)
         go to 320
...
c        if(levm.le.levfrm)goto 460
c temp
         if (levm.eq.levfrm) go to 460
c end
  410    lev=levm
         levm=lev-1
         isegm(lev)=iseg
...
c  finish loops with 3 or fewer external indices
         ...
         ipt2=iasb+iasb+ibsb
c temp
c        if (ipt1.eq.1.and.ibsb.eq.2) go to 410
c end
 ...
 470     continue
         ladt=kad
         jsmt=lsm
c..rlm475     call loopex
 475     call loopex(aint,aint1,vector,sigma,ijadd,kadd,ladd,wtw,wtx,
     #               wty,wab,ss,
     #               ci,si,cj,sj,h1,h2,ijxx,nklxx,ijww,nklww,
     #               klxx,klww)
         goto 200
...
c..rlm
 500  continue
c
c     finished writing dm
c     put square one electron density on tape49
c..rlm     call putone
      call iosys('write real "guga square 1pdm" to rwf without'//
     $           ' rewinding',norb2,aint1,0,' ')
c     write (itape6,*) 'wrote',norb2,'words to square 1pdm'
c..rlm
      levfrm=lsave
c
      return
