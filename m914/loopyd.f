...
c
      ...
      if(ibhl.eq.1) then
       path='diagonal form'
c      call iosys('create real "'//path//'" on ciform',
c     $  10000000,0,0,' ')
      else
       path='ci form'
c      call iosys('create real "'//path//'" on ciform',
...
c
      if(ixf.eq.maxbuf) then
       ...
       ibuf(2,1)=iblock
       ibuf(3,1)=0
c       call iosys('write real "'//path//'" to ciform without rewinding',
c     $ 30*maxbuf,xbuf,0,' ')
       ixf=1
       noutf=noutf+1
      end if
...
c
      if(ibhl.eq.1) then
       ...
       call iosys('write integer ndci to rwf',1,noutf,0,' ')
      else
       write(itape6,502) noutf
       call iosys('write integer nfci to rwf',1,noutf,0,' ')
      end if
