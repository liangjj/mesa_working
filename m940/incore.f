...
c
      ...
      if (logkey(ops,'print=ci=full',.false.,' ')) then
          hamtit='eigenvalues'
          call sadd(t1,eigval,rep+fzcore,nroots)
          call prntrm(hamtit,t1,nroots,1,nroots,1,iout)
          hamtit='eigenvector matrix'
          call prntrm(hamtit,eigvec,nwks,nroots,nwks,nroots,iout)
      end if
c
c
      call iosys('read real '//title(3)//' from hamiltonian',
     1            nwks,t1,0,' ')
      call eig940(nwks,nroots,eigval,eigvec,t1,rep+fzcore,prtflg,
     #            calc,code)
c
c
      return
 1    format(/,15x,'direct diagonalization',/,/,5x,
     1             'size of matrix  = ',i7,/,5x,
     2             'number of roots = ',i5)
      end
