*deck  @(#)loops.f	1.3 7/30/91
      subroutine loops(arc,wt,nlwks,ijadd,kadd,ladd,orbsym,
     ...
     $     imngrp,imxgrp,jmngrp,jmxgrp,ngroup,unit,ioff,joff,
     $     ops,bufnam)
c
c***begin prologue     loops
c***date written       870807   (yymmdd)
c***revision date      910614   (yymmdd)
c     
c     14 june   1991   bhl at llnl
c        revised to include p and q options 
c
c***keywords           ci coupling coefficients, loops
c***author             saxe, paul (lanl)
c***source             @(#)loops.f	1.1   11/30/90
c
...
c
c***end prologue       loops
c
c
      implicit integer (a-z)
c
      character*(*) ops, bufnam
      character*16 unit
      integer arc(4,nrows)
      integer wt(4,nrows)
      integer nlwks(nrows)
...
c
      ...
      hpq=sirow.eq.2.and.sjrow.eq.1
c
c     this routine is used for both optical potential calculations and normal
c     ci runs.  the parameter byron tells you which is which. 
c     note that loops is called several times when doing hpq or
c     hqp calculations; the diagonals are zeroed out the first time
c     only. on the other hand, this routine may be called several times
c     in the course of an mcscf calculation, in which case the diagonals
c     should be zeroed each time.
      if(hpq .or. hqp) then
         byron=.false.
      else
	 byron=.true.
...
c
         if (logkey(ops,'print=ci=guga-integrals',.false.,' ')) then
            write (iout,9001) (ints(iq),iq=1,nmax)
 9001       format (5x,'m910: guga integrals',/,
     $           (5x,5f12.6))
         end if
c
         imin=imngrp(group)
         imax=imxgrp(group)
      do 1000 level=imax+1,max(2,imin+1),-1
c
...
c
 411              continue
  ...
  414             continue
                     ...
                     go to 430
  415             continue
                     loop=acoef(levm1)*(ints(ijkl+3)+d*ints(ijkl+2))
                     go to 430
  416             continue
                     loop=acoef(levm1)*(ints(ijkl+3)+ints(ijkl+2))
                     go to 430
  417             continue
  ...
  418             continue
     ...
     $                 ints(ijkl+2)+ints(ijkl+3))
                     go to 430
  419             continue
                     loop=acoef(levm1)*
     $                 (ints(ijkl+1)+d*ints(ijkl+2))
                     go to 430
  420             continue
                     loop=acoef(levm1)*(ints(ijkl+2)+d*ints(ijkl+1))
  430             continue
c
c
                  if (abs(loop).gt.cutoff) then
                     if (iwt.eq.jwt.and..not.(hpq.or.hqp)) then
                        do 432 walk=1,nlwks(inxt)
                           diag(iwt+walk+ioff)   =
     1                     diag(iwt+walk+ioff)+loop
 432                    continue
                     else
                        do 435 walk=1,nlwks(inxt)
                           ...
                           if (n.gt.lnbuf) then
                              ntotal=ntotal+lnbuf
                              call iosys('write integer '//bufnam//
     ...
     $                              wptoin(lnbuf),rbuf,0,' ')
                           ...
                           if(byron) then
                              ibuf(1,n)=jwt+walk+joff
                              ibuf(2,n)=iwt+walk+ioff
                           else
                              if (hqp) then
                                  ibuf(1,n)=jwt+walk+ioff
                                  ibuf(2,n)=iwt+walk+joff
                              else
                                  ibuf(2,n)=jwt+walk+joff
                                  ibuf(1,n)=iwt+walk+ioff
                              end if
                           endif
                           rbuf(n)=loop
 435                    continue
...
c
  ...
  217          continue
                  ...
                  go to 230
  218          continue
                  loop=acoef(levm1)*(ints(ijkl+1)+ints(ijkl+2)+
     $              ints(ijkl+3))
                  go to 230
  219          continue
                  loop=acoef(levm1)*(ints(ijkl+1)+d*ints(ijkl+2))
                  go to 230
  220          continue
                  loop=acoef(levm1)*(ints(ijkl+2)+d*ints(ijkl+1))
  230          continue
               ...
               if (abs(loop).gt.cutoff) then
                   if (iwt.eq.jwt.and..not.(hpq.or.hqp)) then
                       do 232 walk=1,nlwks(inxt)
                          diag(iwt+walk+ioff)=diag(iwt+walk+ioff)
     1                                       + loop
 232                   continue
                   else
