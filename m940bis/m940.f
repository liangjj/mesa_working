*deck @(#)m940.f	1.4  8/3/91
      program m940
c
c***begin prologue     m940
...
c***description        important points to note.  the Hamiltonian
c***                   matrices are written as files with only the
c***                   non-zero elements and their indices stored.
c***                   a given (I,J) pair may have contributions
c***                   which occur more than once on the file.
c***                   the ordering is always I greater than or
c***                   equal to J and thus a given element may
c***                   contribute to any quantity with I greater
c***                   than or less than J.  the manner in which
c***                   the matrix elements are stored if space
c***                   is partitioned into a P and Q space is also
c***                   important.  the configuration link m806 is
c***                   designed to put the Q space configurations
c***                   first in the list.  in addition, the code
c***                   also assumes GUGA ordering of the integrals.
c***                   this means that the coupling of orbitals is
c***                   backward.  later codes which construct 
c***                   matrix elements over the configurations will
c***                   correct for this and the matrices will be
c***                   in MO order.
c
c***references
c
...
c
      ...
      logical target
      character*8 prtflg
      character*8 dsk, dskt
      character*4 itoc
      character*128 namham, nmcnfg, tmp, nmtarg, namchk
      character*3 answer
...
c
      pfile=.true.
c
c     decide on the type of ci you are doing.  special code
c     has been added which tells the CI that this is a target
c     calculation with an integral set with zeroed integrals.
c     in this way we can fool the code to think it is doing
c     an (n-1) electron calculation as an n electron calculation.
c     this is a neat trick but it is essential to make sure that
c     the phases for the (n-1) and n electron calculation are
c     similarly defined.  Schneider invented the idea and
c     Lengsfield has nade sure that this the phases are
c     properly accounted for.
c
...
c
     ...
     #               //'" from chk',0,0,0,nmcnfg)
      ...
      citype='m940'
c
c     if the conham option is turned on the hamiltonian or
c     some part of it will be explicitly formed.  this is not
c     just the non-zero elements but the full (sub) matrix.
c     the option was added so that users could transparently
c     take the Hamiltonian matrix to other, non-Mesa codes.
c
c     if ppon, pqon and/or qqon are turned on, the code will
c     construct the non-zero matrix elements of that sub-block
c     of the Hamiltonian.  it is in iosys format.
c
c     the hamon option was created to enable the creation
c     of a non-iosys version of the non-zero elements of the
c     Hamiltonian.  this was done for compatibility with
c     the needs of other users.
c 
...
c         the internal set of orbitals is defined so that any
c         arbitrary CI involving that set of orbitals is done
c         correctly.  integrals between the internal and
c         external set of orbitals may be set to zero in order
c         to perform scattering calculations using contracted
c         CI functions in the internal basis.  a special integral
c         file named ZINT is formed to perform such calculations.
c         the idea is to be able to place the (n+1) electron or
c         external orbital on to a target CI wavefunction which
c         is contructed using the internal orbital set.  this
c         can be used to form a P space made from targets having
c         correlation.
c
...
c
c    ----- write the ci information to the dsk file.  it is
c          needed in later links.
c
          call iosys('write integer "target roots" to '//dsk,1,
     1                roots,0,' ')
          call iosys('write integer "target orbitals" to '//dsk,1,
     1                trgorb,0,' ')
          call iosys('write integer "target walks" to '//dsk,
     1               1,nwks,0,' ')
          call iosys('write integer roots to rwf',1,roots,0,' ')
          write(iout,1) trgorb, porb, nwks, nwksp, roots, conham,
     1                  hamon, diaham
      elseif( logkey(ops,'ci=p',.false.,' ') ) then
          ...
          nwks=nwksp+nwksq
          write (iout,2) nwks, nwksp, proots, partit, conham, diaham
     ...
     1               'rwf',1,qroots,0,' ')
      ...
      elseif (logkey(ops,'ci=pq',.false.,' ')) then
          ...
          call iosys('read integer nwksq from '//dsk,1,nwksq,0,' ')
          nwks=nwksp+nwksq
          write (iout,4) nwks, nwksp, nwksq, conham
      elseif (logkey(ops,'ci=p-and-q',.false.,' ')) then
          ...
          proots=min(proots,nwksp)
          qroots=min(qroots,nwksq)
          write (iout,5) nwks, nwksp, proots, nwksq, qroots, conham,
     1                   diaham, contrq
     ...
     1                      ' must both be on'
          ...
          call iosys('write integer roots to rwf',1,roots,0,' ')
          write (iout,6) nwks, nwksp, nwksq, roots, conham, 
     1                   hamon, diaham
      else
          normal=.true.
          nwksp=0
          ...
          write(iout,7) nwks, roots, conham, diaham
          call iosys('write integer roots to rwf',1,roots,0,' ')
      endif
...
c           for the ci.  this is read off the dsk file generated
c           in the m806 link.
c
...
c
      lnbuf=intkey(ops,'ci=buffer-length',1000000,' ')
c
...
c----------------------------------------------------------------------c
c                           BEGIN
c           section to form and write out the full hamiltonian matrix.
c           only the non-zero elements and their indices are stored.
c
      title(1)='buffers'
...
c
     ...
     #            ops,unit,dsk,prtflg,title,prnt)
      time(1)=secnds(0.0)
      delta(2)=time(1)-time(2)
c
c                           END
c
c
...
c
c                 If conham is true, explicitly form H_pp
c
          if(conham) then
             hpp=diagp+nwksp
...
c
c                 If diaham is true, diagonalize H_pp
c
          if(diaham) then
             call diagh('ci',rep,fzcore,lnbuf,proots,nwksp,
     1                   ops,title)
             time(1)=secnds(0.0)
             delta(4)=time(1)-time(2)            
          endif
      endif
      if ( qqon ) then
          time(1)=secnds(0.0)
...
c
c                this piece of code is to form the off-diagonal
c                H_pq matrices.
     ...
     2               conham,title,prnt)
          time(2)=secnds(0.0)
          delta(7)=time(2)-time(1)
          call getmem(-ngot,p,idum,'hpq',idum)
      endif
      if (pandqon.or.normal) then
         if(conham.or.hamon) then
c
c           Here we construct the full Hamiltonian but recognizing
c           that it is partitioned.
c
            time(1)=secnds(0.0)
            title(1)='buffers'
            title(2)='"number of elements"'
...
c         this allows us to use correlated target CI
c         wavefunctions.
c
          call tranpp(a(ibuf),z(rbuf),z(diag),z(pvec),z(trpvec),z(scr),
     1                lnbuf,nwks,nwksp,ntot,title,prnt,dsk)
c
...
c
     ...
     1                nwksp,nwksq,ntot,title,prnt)
          if(contrq) then
             ...
             if(need.gt.canget) then
                call lnkerr('cannot get enough memory for '//
     ...
     1                   lnbuf,nwksp,nwksq,qroots,ntot,prnt)
      ...
      write(iout,12)
      write(iout,13) (delta(i),i=1,2)
      if(ppon) then
         write(iout,14) (delta(i),i=3,4)
      endif
      if(qqon) then
         write(iout,15) (delta(i),i=5,6)
      endif
      if(pqon) then
         write(iout,16) delta(7)
      endif
      if (pandqon.or.normal) then
         if(conham.or.hamon) then
            write(iout,17) delta(8)
         endif
         if(diaham) then
            write(iout,18) delta(9)
         endif
      endif
 ...
 1    format(//,25x,'target calculation',/,5x,
     ...
     5            'no. target roots                     = ',i7,/,5x,
     6            'forming the target hamiltonian       = ',l1,/,5x,
     7            'produce an unformatted hamiltonian   = ',l1,/,5x,
     8            'diagonalizing the target hamiltonian = ',l1)
 2    format (//,25x,'forming the p-space hamiltonian'
     #             ,/,5x,
     1               'total no. configurations                   = '
     #          ,i7,/,5x,
     2               'no. p-space configurations                 = '
     #          ,i7,/,5x,
     3               'no. p-space roots                          = '
     #          ,i7,/,5x,
     6               'forming the p-space hamiltonian            = '
     #          ,l1,/,5x,
     7               'produce an unformatted p-space hamiltonian = '
     #          ,l1,/,5x,
     8               'diagonalizing the hamiltonian              = ',l1)
 3    format (//,25x,'forming the q-space hamiltonian'
     #             ,/,5x,
     1               'total no. configurations                   = '
     #          ,i7,/,5x,
     2               'no. q-space configurations                 = '
     #          ,i7,/,5x,
     3               'no. q-space roots                          = '
     #          ,i7,/,5x,
     6               'forming the q-space hamiltonian            = '
     #          ,l1,/,5x,
     7               'produce an unformatted q-space hamiltonian = '
     #          ,l1,/,5x,
     8               'diagonalizing the hamiltonian              = ',l1)
 4    format (//,25x,'forming the p-q block of the hamiltonian',
     ...
     4               'forming the pq-space hamiltonian = ',l1)
 5    format (//,25x,'forming partitioned  hamiltonian'
     #         ,/,5x,
     1               'total no. configurations            = ',i7,/,5x, 
     2               'no. p-space configurations          = ',i7,/,5x,
     3               'no. p-space roots                   = ',i7,/,5x,
     4               'no. q-space configurations          = ',i7,/,5x,
     5               'no. q-space roots                   = ',i7,/,5x,
     6               'forming the partitioned hamiltonian = ',l1,/,5x,
     7               'diagonalizing the hamiltonian       = ',l1,/,5x,
     8               'contracting q-space                 = ',l1)
 6    format (//,25x,'forming p+q hamiltonian'
     #         ,/,5x,
     1               'total no. configurations            = ',i7,/,5x,
     2               'no. p-space configurations          = ',i7,/,5x,
     3               'no. q-space configurations          = ',i7,/,5x,
     ...
     6               'forming the partitioned hamiltonian = ',l1,/,5x,
     7               'produce an unformatted hamiltonian  = ',l1,/,5x,
     8               'diagonalize the hamiltonian         = ',l1)
 7    format (//,25x,'forming hamiltonian'                
     #         ,/,5x,
     1               'no. configurations          = ',i7,/,5x,
     2               'no. roots                   = ',i7,/,5x,
     3               'forming the hamiltonian     = ',l1,/,5x,
     4               'diagonalize the hamiltonian = ',l1)
 ...
 12   format(/,25x,'Timings')
 13   format(/,5x,'Time for Basic Setup                   = ',e15.8,
     1       /,5x,'Time for full Hamiltonian Construction = ',e15.8)
 14   format(/,5x,'Time to Construct HPP                  = ',e15.8,
     1       /,5x,'Time to Diagonalize HPP                = ',e15.8)
 15   format(/,5x,'Time to Construct HQQ                  = ',e15.8,
     1       /,5x,'Time to Diagonalize HQQ                = ',e15.8)
 16   format(/,5x,'Time to Construct HPQ                  = ',e15.8)
 17   format(/,5x,'Time to Construct H                    = ',e15.8)
 18   format(/,5x,'Time to Diagonalize H                  = ',e15.8)
      end
