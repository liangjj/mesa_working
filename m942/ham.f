...
c***routines called    
c***end prologue       ham
      subroutine ham(ibuf,rbuf,diag,hpp,hqp,pvec,spp,energy,
     #               header,nwksp,nwksq,nwks,npvec,
     #               lenbuf,nonz,nodsk,nen,prnt)
      implicit integer (a-z)
      real*8 rbuf, diag, hpp, hqp, spp, tmp, pvec, energy
      character*80 title
      character*(*) header(7)
      character*16 fptoc
      logical nodsk, prnt
      ...
      call rzero(hqp,nwksq*npvec)
      if(nodsk) then
         call rdham(ibuf,rbuf,header(1),lenbuf,nonz) 
         call hmul(ibuf,rbuf,pvec,spp,hqp,
 ...
 4000 continue   
     ...
     #           'hamiltonian',npvec,diag,0,' ')
      call iosys('create integer '//header(3)//' on hamiltonian',
     #            -1,0,0,' ')
      n=0
      nonzpp=0
      do 5000 i=1,npvec
 ...
 5000 continue
      if (n.gt.0) then
          nonzpp=nonzpp+n
          call iosys('write integer '//header(3)//
     #               ' to hamiltonian without rewinding',2*n,
     #               ibuf,0,' ') 
          call iosys('write integer '//header(3)//
     ...
     #                wptoin(n),rbuf,0,' ')        
      ...
      call iosys('write integer '//header(4)//
     #           ' to hamiltonian',1,nonzpp,0,' ')
      write(iout,1) nonzpp
      if(prnt(1)) then
...
10000    continue         
 9000 continue
     ...
     #                rbuf,0,' ')        
      ...
      call iosys('endfile '//header(5)//' on hamiltonian',0,0,0,' ')
 ...
 2    format(/,1x,'number of non-zero contracted qp-space '
     1            'matrix elements = ',i5 )
      end       
