...
if [ "x$PBS_ROOT" != "x" ]; then
    ...
    export FFLAGS="-I${LAPACK95_MODULES} -Mvect=sse -Mscalarsse -Mcache_align -O2 -tp=x64"
    ...
    export LDFLAGS="-llapack -lblas"
elif [ "x$machine" == "xfeynman.mps.nsf.gov" ]; then
    export PATH=/usr/local/openmpi/bin:$PATH
    export LDFLAGS="${LAPACK95_LIBRARY} ${MD_LIB}"
    export FFLAGS="-I${LAPACK95_MODULES} -g"
fi
