...
!**author             schneider, b. i.(nsf)
!**source             Time_Propagation
!**purpose            multiply a matrix on the lanczos vector.
!**description        
!**                   
!**references
!**routines called    
...
!  Do special case of L = L_Max
   ...
   call column_packed_matrix_on_vector (                            &
                    ...
                    dipole_mat(L-1)%dipole_row_index,               &
                    .true.)                
   v_out(1:final_total_matrix_size) = electric_field                &
                                           *                        &
                                   v_out(1:final_total_matrix_size)
   v_out(1:final_total_matrix_size) =                               &
...
!
  IF(packed_matrices) THEN
     IF (use_atomic_symmetry == 'on') THEN
         Call h_on_lanczos_vector (lanczos_basis(:,it),                 &
                                   h_vec_d(:) )
     ELSE
