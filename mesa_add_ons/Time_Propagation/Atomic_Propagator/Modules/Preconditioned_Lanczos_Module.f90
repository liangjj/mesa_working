...
!***                  algorithm when the underlying problems is a generalized eigenvalue problem.
!***                  Explicit interfaces are used to allow a transparent use of generic subroutines which work
...
!
                           INTERFACE h_on_lanczos_vector
             MODULE PROCEDURE h_on_lanczos_vector_d,                                    &
                              h_on_lanczos_vector_z  
                       END INTERFACE h_on_lanczos_vector
!
                           INTERFACE s_on_lanczos_vector
             MODULE PROCEDURE s_on_lanczos_vector_d,                                    &
                              s_on_lanczos_vector_z  
                       END INTERFACE s_on_lanczos_vector
!***********************************************************************
!***********************************************************************
                              CONTAINS
!***********************************************************************
!***********************************************************************
!**begin prologue     h_on_lanczos_vector_d
...
!**author             schneider, b. i.(nsf)
!**source             Time_Propagation
!**purpose            multiply overlap on lanczos vector.
!**description        
!**                   
!**references
!**routines called    
!**end prologue       h_on_lanczos_vector_d
!***********************************************************************
  SUBROUTINE h_on_lanczos_vector_d(v_in,v_out)
  IMPLICIT NONE
...
!
  ...
  ELSE IF (type == 'finite_element') THEN
           CALL finite_element_m_v( v_in(:), v_out(:) )
  END IF
END SUBROUTINE h_on_lanczos_vector_d
!***********************************************************************
!***********************************************************************
!**begin prologue     h_on_lanczos_vector_z
!**date written       960718   (yymmdd)
!**revision date               (yymmdd)
!**keywords           time, dvr, Lanczos, propagation
!**
...
!**references
!**routines called    
!**end prologue       h_on_lanczos_vector_z
!***********************************************************************
  SUBROUTINE h_on_lanczos_vector_z(v_in,v_out)
  IMPLICIT NONE
  ...
  ELSE IF (type == 'finite_element') THEN
           CALL finite_element_m_v( v_in(:), v_out(:) )
  END IF
END SUBROUTINE h_on_lanczos_vector_z
!***********************************************************************
!***********************************************************************
!**begin prologue     s_on_lanczos_vector_d
!**date written       960718   (yymmdd)
!**revision date               (yymmdd)
!**keywords           time, dvr, Lanczos, propagation
!**
...
!**references
!**routines called    
!**end prologue       s_on_lanczos_vector_d
!***********************************************************************
  SUBROUTINE s_on_lanczos_vector_d(v_in,v_out,s_v_in)
  IMPLICIT NONE
  REAL*8, DIMENSION(:)                           :: v_in
  REAL*8, DIMENSION(:)                           :: v_out
  REAL*8, DIMENSION(:)                           :: s_v_in
!
  ...
  ELSE
      call matrix_vector_multiply( triangle_overlap_d,                 &
                                   v_in, s_v_in )
  END IF
END SUBROUTINE s_on_lanczos_vector_d
!***********************************************************************
!***********************************************************************
!**begin prologue     s_on_lanczos_vector_z
!**date written       960718   (yymmdd)
!**revision date               (yymmdd)
!**keywords           time, dvr, Lanczos, propagation
!**
...
!**references
!**routines called    
!**end prologue       s_on_lanczos_vector_z
!***********************************************************************
  SUBROUTINE s_on_lanczos_vector_z(v_in,v_out,s_v_in)
  IMPLICIT NONE
  COMPLEX*16, DIMENSION(:)                       :: v_in
  COMPLEX*16, DIMENSION(:)                       :: v_out
  COMPLEX*16, DIMENSION(:)                       :: s_v_in
!
  local_time=secnds(0.0)
  IF (packed_matrices) THEN 
      call column_packed_upper_triangle_on_vector                      &
  ...
  ELSE
      call matrix_vector_multiply( triangle_overlap_d,                 &
                                   v_in, s_v_in )
  END IF
END SUBROUTINE s_on_lanczos_vector_z
!***********************************************************************
!***********************************************************************
...
!
  local_time=secnds(0.0)
  Call s_on_lanczos_vector(v_in,v_out,s_vec_d(:,0))
...
!
    ...
    IF( convergence == 'maximal') THEN
        WRITE(iout,4) eig(0)
        Call s_on_lanczos_vector(v_out,h_vec_d,s_vec_d(:,0))
...
!
!
! Input an S-normalized initial guess
!
  local_time=secnds(0.0)
  Call s_on_lanczos_vector(v_in,v_out,s_vec_z(:,0))
...
!
        ...
        WRITE(iout,4) eig(0)
        Call s_on_lanczos_vector(v_out,h_vec_z,s_vec_z(:,0))
        norm = 1.d0/sqrt(cdotc(n3d,v_out,1,s_vec_z(:,0),1))
        v_in = v_out
        IF(compute_energy) THEN 
           Call h_on_lanczos_vector(v_in,h_vec_z)
...
!
  local_time=secnds(0.0)
  Call h_on_lanczos_vector(lanczos_basis(:,it), h_vec_d)
...
!
     ...
     ELSE
         Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),v_out,'t',it)
     END IF
!
!
...
!
     ...
     ELSE
         Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),                &
                               lanczos_basis(:,it+1),                        &
                               'n',it)
     END IF
...
!
         ...
         Call s_on_lanczos_vector(lanczos_basis(:,it+1),h_vec_d(:),v_out(:))
!
...
!        Normalize it.
!
         Call s_on_lanczos_vector( v_out(:), h_vec_d(:),                     &
         ...
         ELSE
             Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),            &
                                   s_on_lanczos_basis(:,it+1),'t',it)
             Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),            &
                                   lanczos_basis(:,it+1),'n',it)
         END IF
...
!
  local_time=secnds(0.0)
  Call h_on_lanczos_vector( lanczos_basis(:,it), h_vec_z(:) )
  total_time(2) = secnds(0.0) - local_time + total_time(2)
  IF(log_iterative(10).or.log_iterative(3)) THEN
     title='H_on_Lanczos_Vector iteration = '//itoc(it)
...
!
     ...
     IF (packed_matrices) THEN
         Call Packed_Solve (lanczos_basis(:,it+1),                           &
                            ...
                            mat_var(2)%number,'t',it)
     ELSE
         Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),v_out,'n',it)
     END IF
!
!
!         Compute b(it+1)  b(i) = Sqrt ( <V_i|V_i> )
...
!
     IF (packed_matrices) THEN
         Call Packed_Solve (lanczos_basis(:,it+1),                            &
                            ...
                            mat_var(1)%number,'n',it)
     ELSE
         Call Triangular_Solve(upper_d,lanczos_basis(:,it+1),lanczos_basis(:,it+1),  &
                               'n',it)
     END IF
     total_time(7) = secnds(0.0) - local_time + total_time(7)
     lanczos_basis(:,it+1) =  lanczos_basis(:,it+1) / b(it+1)
...
!
     IF( b(it+1) <= eps ) THEN
         ...
         END DO
         Call s_on_lanczos_vector(lanczos_basis(:,it+1),h_vec_z(:),v_out(:))
         DO i=0,it
            v_out(:) = v_out(:)                                        &
                                 -                                     &
                       cdotc(n3d,v_out,1,lanczos_basis(:,i),1 )        &
                                 * s_on_lanczos_basis(:,i)
         END DO
         Call s_on_lanczos_vector( v_out(:), h_vec_z(:),               &
                                   s_on_lanczos_basis(:,it+1) )
...
!  write(iout,*) eig(0:it)
!  write(iout,*) local_scratch(1:it+1)
!  write(iout,*) eigen_vectors(0,0:it)
