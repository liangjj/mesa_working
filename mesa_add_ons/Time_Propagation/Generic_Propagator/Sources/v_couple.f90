...
!**end prologue       v_couple
  ...
  IF(dollar('$v_couple',card,cpass,inp) ) THEN
     vtyp(1)=chrkey(card,'v_space_time','none',' ')
     vtyp(2)=chrkey(card,'v_non_linear','none',' ')
     natom=0
     IF(vtyp(2) /= 'none') THEN
        atom=chrkey(card,'atom','cs',' ')
        sctlen=fpkey(card,'scattering_length',52.d0,' ')
        natom=intkey(card,'number_of_atoms',0,' ')
     END IF
     WRITE(iout,1) vtyp
     WRITE(iout,2) units
     fretyp=logkey(card,'convert_frequency',.false.,' ')
     scale=fpkey(card,'electric_field_strength',1.d0,' ')
     scale=scale*efieldau
     omega=fpkey(card,'electric_field_frequency',1.d0,' ')
     omega=omega/timau
     IF(vtyp(1) == 'cosine_gaussian_pulse'.OR.  &
        vtyp(1) == 'sine_gaussian_pulse') THEN
        width=fpkey(card,'width_of_gaussian_pulse',0.d0,' ')
        shift=fpkey(card,'shift_of_gaussian_pulse',0.d0,' ')
     END IF
     IF(fretyp) then
        omega=2.d0*pi*omega
     END IF
     IF(units == 'atomic_units') THEN
        ...
        scale=scale/efieldau
     END IF
     IF(vtyp(1) == 'cosine_pulse') THEN
        WRITE(iout,3) omega, scale
     ELSE IF(vtyp(1) == 'sine_pulse') THEN
        WRITE(iout,4) omega, scale
     ELSE IF(vtyp(1) == 't') THEN
        WRITE(iout,5)
     ELSE IF(vtyp(1) == 'cosine_dipole_field') THEN
        WRITE(iout,6) omega, scale
     ELSE IF(vtyp(1) == 'sine_dipole_field') THEN
        WRITE(iout,7) omega, scale
     ELSE IF(vtyp(1) == 'cosine_gaussian_pulse') THEN
        WRITE(iout,8) scale, omega, width, shift
     ELSE IF(vtyp(1) == 'sine_gaussian_pulse') THEN
