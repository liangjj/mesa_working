...
c***                   in an (x,y,z) region bounded by the variables
c***                   left, right, bottom and top.
c***
c***                   the equation under consideration is;
c***                   - ( del**2 + k*k ) u = f in 3-d
...
c
      ...
      common z(1)
      real*8 z, x0, xn, y0, yn, z0, zn, stpg, tol, fpkey, cstep
      real*8 hf, k, rms, hsq, hsqi, ksq, fac, ifac
...
c
      ...
      prnrho=logkey(ops,'print=m1203=input-rho',.false.,' ')
      prnsol=logkey(ops,'print=m1203=coarse-solution',.false.,' ')
      if (posinp('$multigrid',cpass) ) then
          call cardin(card)
      endif          
      x0=fpkey(card,'x0',-1.d0,' ')
      xn=fpkey(card,'xn',1.d0,' ')
      y0=fpkey(card,'y0',x0,' ')
      yn=fpkey(card,'yn',xn,' ')
      z0=fpkey(card,'z0',x0,' ')
      zn=fpkey(card,'zn',xn,' ')
      ncorse=intkey(card,'number-of-coarse-grid-points',3,' ')
      cstep=(xn-x0)/(ncorse-1)
     ...
     1             100,' ')
      ...
      if (type.eq.'wave') then
          ...
          tststp=k*k*cstep*cstep
          if (tststp.ge.1.d0) then
              write (iout,*) '  coarsest grid not fine enough.
              write (iout,*) '        will adjust grid'
              cstep=.9d0/k
              ncorse = 1 + (xn-x0)/cstep
              write(iout,*) '  adjusted number of coarse grid points = '
     1                         ,ncorse
          endif     
      endif
...
c     allocate memory        
      ...
      do 30 i=1,2
         ibeg=ioff
         do 40 ig=1,ng
            nptcub=nptg(ig)*nptg(ig)*nptg(ig)
            u(ig)=ibeg
            rhs(ig)=u(ig)+nptcub
            rho(ig)=rhs(ig)+nptcub
            res(ig)=rho(ig)+nptcub
            ibeg=res(ig)+nptcub
 40      continue
         uhold=ibeg
         ibeg=ibeg+nptcub
...
c     input exact solution and right hand sides for all grids.
      do 50 ig=1,ng
         ...
         call stabil(stpg(ig),k)
 50   continue
      nscub=nptg(ng)*nptg(ng)*nptg(ng)
      call copy(z(rho(ng)),z(u(ng)),nscub)
      call copy(z(rho(ng)),z(uhold),nscub)
c     calculate solution on coarsest grid      
      if (ecrse) then
          write(iout,5)
     ...
     1                nptg(1),maxit,ittyp,prnsol)
          do 60 j=2,ng
             nscub=nptg(j)*nptg(j)*nptg(j)
c            take coarse grid solution of previous level and
c            interpolate it as a starting guess for this finer level.             
             write(iout,6) j
             call interp(z(u(j)),z(u(j-1)),nptg(j),nptg(j-1))
c            copy the inhomogeneity on this level to rhs                            
             if(j.ne.ng) then
                 call copy(z(rho(j)),z(rhs(j)),nscub) 
             else
                 call copy(z(uhold),z(rhs(j)),nscub)
             endif
             do 70 jcycle=1,ncycle
                write(iout,7) jcycle
c               for jj=j in the next loop we are relaxing on the current
c               approximation to the solution.  as we proceed downward we
c               are relaxing on the residual.                
                do 80 jj=j,2,-1
c               perform some relaxations on the current level to remove
c               the high frequecy components of the error in the
c               solution vector or the residual vector.
                   nf=nptg(jj)
                   hf=stpg(jj)
                   hsq=hf*hf
                   hsqi=1.d0/hsq
                   ...
                   ifac=1.d0/(6.d0-ksq)
                   do 90 jpre=1,npre
                      call relax(z(u(jj)),z(rhs(jj)),hf,k,
     1                           hsq,ifac,nf)
 90                continue
                   fac=6.d0*hsqi-k*k
c                  compute the residual on the current level.                   
                   call resid(z(res(jj)),z(u(jj)),z(rhs(jj)),hf,k,
     1                        hsqi,fac,nf,rms)
                   write(iout,8) jj, rms
                   nc=nptg(jj-1)
c                  restrict the residual to the next coarser grid.  it will be
c                  the rhs vector when we return to the head of the loop.                    
                   call rstrct(z(rhs(jj-1)),z(res(jj)),nc,nf)
c                  zero the initial guess for the residual.  it will be used
c                  in the relaxation when we return to the head of the loop.                     
                   call rzero(z(u(jj-1)),nc*nc*nc) 
 80             continue
                call rzero(z(u(1)),nptg(1)*nptg(1)*nptg(1))
...
c           perform some relaxations for further smoothing
                   do 200 jpost=1,npost
                      call relax(z(u(jj)),z(rhs(jj)),hf,k,hsq,
     1                           ifac,nf)
 200               continue
                   call resid(z(res(jj)),z(u(jj)),z(rhs(jj)),hf,k,
     1                        hsqi,fac,nf,rms)
