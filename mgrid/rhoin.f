...
c***                   input here on a uniform grid in (x,y).  it
c***                   is output scaled by the square of the stepsize
c***                   to make the rest of the code more efficient.
c***                   in addition, the exact solution is tabulated for
c***                   the model problem.
c***references         multigrid methods, siam, s. mccormick ed. 
c
c***routines called    iosys, util and mdutil
c***end prologue       rhoin
      subroutine rhoin(rho,ex,h,grid,n,type,prnt)
c
      implicit integer (a-z)
      real*8 rho, ex, h, x, y, xx, yy, f1, f2, f3, f4
      logical prnt
      character*80 title
      character*2 itoc
      character*(*) type
      dimension rho(n,n), ex(n,n)
      common/io/inp, iout
