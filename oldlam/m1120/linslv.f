...
c***                   saxpy(clams)
      ...
      real *8 wt(n)
      real *8 thresh, consln, delmax, conold
      real *8 anorm, snorm, diff, sum
      dimension ia(*)
...
c
      elseif (oper.eq.'generate orthogonal vectors') then
c
c----------------------------------------------------------------------c
...
c----------------------------------------------------------------------c
c              no schmidt orthonormalization of vectors                c
c----------------------------------------------------------------------c
      elseif (oper.eq.'generate vectors') then
              ...
              ia(1)=ntrial
              if (iwrit.gt.0) then
                  do 800 i=nold+1,nvec
                     do 850 j=1,i
                        v1(i,j)=snorm(a1(1,i),a1(1,j),wt,n,nowgt)
  850                continue
  800             continue
                  do 900 i=nold+1,nvec
                     write (iout,280) i,(v1(i,j),j=1,i)
  900             continue
                  do 1000 i=1,nvec
                     write (iout,320) i,(a1(j,i),j=1,n)
...
c----------------------------------------------------------------------c
              do 130 i=1,nold
                 do 131 j=nstart,nvec
                    a3(i,j)=snorm(a1(1,i),a1(1,j),wt,n,nowgt)
                    a3(j,i)=a3(i,j)
                    a3(i,j)=a3(i,j)+snorm(a1(1,i),a2(1,j),wt,n,nowgt)
                    a3(j,i)=a3(j,i)+snorm(a1(1,j),a2(1,i),wt,n,nowgt)
                    a4(i,j)=a3(i,j)
                    a4(j,i)=a3(j,i)
  131            continue
  130         continue
...
c----------------------------------------------------------------------c
                 ...
                 status='converged'
                 call sgemm (n,nvec,nrhs,a1,n,v1,m,v3,n,0,1)
                 if (iwrit.gt.0) then
                     do 270 i=1,nrhs
