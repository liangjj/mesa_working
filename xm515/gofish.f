...
c***source             @(#)gofish.f	1.2 11/28/95
...
c        temporary sleaze
     ...
     $                   pwtx,rr,adjust,radii)
            if (nat.eq.2 .and. grdtyp(2).eq.'nogrid') then
               call vfill(itch(tvwts),1.0d0)
               write(ierr,*)' HACK'
            endif
            if (dogrdio) then
...
c        --- decompose density into ylm components
     ...
     $               nr,vnomega,nlm,itch(angwt),itch(scr),ylmerr)
         if(debug) then
            write(iout,*) 'rholm'
            do 111 i=1,nlm
               write(iout,*) 'lm',i,(itch(rholm+(i-1)*nr+k),k=0,nr-1)
  111       continue
         endif
         if(timeit) then
            call timing(dum4,dum5,dum6)
...
c$$$c TEST: most pessimal way of doing this: divide all vlm's by r**l
c$$$c in valiant attempt to improve fit.  If this works, then need to do this
c$$$c in rsolver instead.
c$$$         do 7070 l=1,vlmax
c$$$            do 7073 i=1,nr
c$$$               itch(scr+i-1)=rpts(i,iatom)**l
c$$$ 7073       continue 
c$$$            do 7071 m=-l,l
c$$$c i=1 is r=0, and v=0 there
c$$$               do 7072 i=2,nr
c$$$                  vlm(i,aitch(ptlm+l+(m+vlmax)*(vlmax+1)),iatom)=
c$$$     $                 vlm(i,aitch(ptlm+l+(m+vlmax)*(vlmax+1)),iatom)/
c$$$     $                 itch(scr+i-1)
c$$$ 7072          continue 
c$$$ 7071       continue 
...
c$$$            do 1111 ig=1,jngrid
c$$$               write(ierr,*) 'wt(',ig,')=',grdwts(ig)
c$$$ 1111       continue 
         else
...
c$$$               if(debug) then
c$$$                  write(ierr,*) 'fitted v'
c$$$                  write(ierr,*) (itch(v+i-1),i=1,ngj)
c$$$               endif
c$$$               write(iout,*) 'fitted v'
c$$$               do i=1,ngj
c$$$                  write(iout,*) 'v(',i,')=',itch(v+i-1)
c$$$               end do
c$$$               do i=1,ngj
c$$$                  write(iout,*) 'wt(',i,')=',grdwts(i)
c$$$               end do
   75       continue
c            if(timeit) then
c               call timing(dum4,dum5,dum6)
