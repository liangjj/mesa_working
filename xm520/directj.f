...
c
c  28 april,    1995   rlm at lanl
c     adding flag and code to do only the three-center integrals
c     (II|JK), (IJ|KK). used in conjunction with the poisson solver.
...
c***source             @(#)directj.f	5.3   6/20/95
...
c     --- input variables -----
      ...
      real*8 cutexp,qtest
      logical prnt,rhotest,doiijk,doiikk
                  ...
                  do 4000 katom=1,iatom
                     ...
                     do 3000 ktype=1,ktmax
                        ...
                        do 2000 latom=1,latmax
                           symcen(4)=latom
                           iandl=.false.
                           if(iatom.eq.latom) iandl=.true.
                           jandl=.false.
                           if(jatom.eq.latom) jandl=.true.
                           kandl=.false.
                           if(katom.eq.latom) kandl=.true.
...
c                                skip out if only (IJ|KK), (II|KL)
...
c                  threshhold to test against is cutexp/max(dpijmx,dpklmx) 
     ...
     $                     pi252,z(t0))
               ...
               if(kl.ne.nkl) then
                  call lnkerr('kl.ne.nkl after prefac')
               endif
               if(debug) then
...
c                     note that g and i2 are overlapped
     ...
     $                 3*(imax+1)*(jmax+1)*(mmax+1)*(lmax+1)*nv*nroots
                  ...
                  if(top.gt.left) then
                     ...
                     call lnkerr('need more memory in directj')
                  endif
c
...
c
     ...
     $                           float(nonzer)*100.0d0/float(ntotal)
      if(timeit) then
         write(iout,*) '1dm,exp,pre,prm,ghi,fmj,ptj,trn',tim1dm,timexp,
