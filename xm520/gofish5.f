*deck @(#)gofish5.f	5.3 6/20/95
      subroutine gofish5(itch,d,nbf,nnp,jmat,ncoul,
     ...
     $     vncrule,nlm,rpts,vlm,y2,zdiag,zoffdiag,rmdiag)
c***begin prologue     gofish5.f
c***date written       940304      (yymmdd)  
c***revision date      6/20/95      
c
c***keywords           poisson, coulomb, potential, density
c***author             martin, richard(lanl) 
c***source             @(#)gofish5.f	5.3   6/20/95
...
c         distribution.
      if(dogrdio) then
         if(.not.called) then
            call iosys('create real "poisson voronoi weights" on rwf',
     $                  pmxgrd*nat,0,0,' ')
         endif
         call iosys('rewind "poisson voronoi weights" on rwf',
     $              0,0,0,' ')
      endif
c
...
c        temporary sleaze
         ...
         if(dogrdio.and.called) then
            call iosys('read real "poisson voronoi weights" from rwf'
     $                  //' without rewinding',pmxgrd,itch(tvwts),0,' ')
         else
c           --- calculate them the first time through.
            call voronoi(nat,c,itch(grid),itch(twts),ngrid,ngrid,
     $                   iatom,itch(tvwts),rnuc,amu,
     $                   pwtx,rr,adjust,radii)
            call iosys('write real "poisson voronoi weights" to rwf'
     $               //' without rewinding',pmxgrd,itch(tvwts),0,' ')
         endif
         if(debug) then
