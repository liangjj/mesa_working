...
c            sum=zero
c            do 5 i=1,n
c               sum=sum+abs(phi(ioff+i,mu))
c    5       continue
c            phibar(mu)=sum/float(n)
c   10    continue
c        --- loop over basis functions and determine density.
         call rzero(scr,minesz*nbf)
         kl=0
         do 40 mu=1,nbf
c            if(phibar(mu).gt.bfcut) then
            do 30 nu=1,mu
               kl=kl+1
c              kl=(mu*(mu-1)/2)+nu
               foo=d(kl)
...
c               test=foo*phibar(mu)*phibar(nu)
c               if (abs(test).gt.bfcut) then
c               if (abs(test).ge.dmcut) then
               if (abs(d(kl)).ge.dmcut) then
...
c                  sum=zero
c                  do 55 i=1,n
c                     sum=sum+grad(ioff+i,mu,coord)
c   55             continue
c                  gradbar(mu)=sum/float(n)
c   60          continue
               call rzero(scr,minesz*nbf)
               kl=0
               do 80 mu=1,nbf
