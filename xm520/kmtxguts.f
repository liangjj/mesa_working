*deck @(#)kmtxguts.f	5.3 6/20/95
      subroutine kmtxguts(dengrida,dengrada,ga,
     $     fout,itch,phi,grad,scr,scr2,slater,becke,lyp,vwn,d,nbf,nnp,
     ...
     $     gradbar,nocc,coeff,bfcut,kmcut)
c
...
c***source             @(#)kmtxguts.f	5.3   6/20/95
...
c         make sure we can really fit in as much as we think
      ...
      if(wpadti(top).gt.ngot) then
         ...
         call lnkerr('need more memory in kmtxguts')
      endif
c
...
c        needs nothing
      ...
      if(wpadti(top).gt.ngot) then
         write(iout,*) 'need,have',wpadti(top),ngot
         call lnkerr('need more core in kmtxguts')
      endif
c     
...
c        --- generate grid, calc number of grid blocks
     ...
     $                 //' without rewinding',mxgrd,grdwt,0,' ')
         else
...
c    $                      dengrida,dengrada,minesz,scr,
c    $                      phibar,gradbar,dmcut,dograd)
            if(timeit) then
               call timing(dum4,dum5,dum6)
               timden=timden+dum4-dum1
            endif
c
            call iosys('write real "closed density" on rwf',
     $                  mxgbsiz,dengrida,denptr,' ')
            if (becke .or. lyp) then
               call iosys('write real "closed density gradient" on rwf',
     $                     mxgbsiz*3,dengrada,gradptr,' ')
            endif
