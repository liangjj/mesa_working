...
c     --- input arrays (unmodified) ---
      ...
      integer ptprim(natoms,ntypes),noprim(natoms,ntypes)
      integer nocart(ntypes),mintyp(ntypes)
      integer ptcont(natoms,ntypes),start(natoms,ntypes)
      integer nobf(ntypes),minmom(ntypes)
      integer nocont(ntypes)
      integer maxmom(ntypes)
      integer nx(lenxyz),ny(lenxyz),nz(lenxyz)
      integer pstart(natoms,ntypes),prtoao(npf,nbf)
      character*(*) ops
...
c
      ...
      parameter (oldest=.false.,older=.true.,newest=.false.)
...
c        --- make sure we have room and get estimates of integrals.
     ...
     $              valuesi,pstart,nbf,nnp,nnshl,qint)
         ...
         timest=timest+dum4-dum1
         if(timeit) then
            write(iout,*) ' time for estmq',
...
c     --- initialize the loop for iterations ---              
      ...
      call izero(nonzer,mxiter)
      call izero(ntotal,mxiter)
      if (prnt) then
         if(dirj.or.poisj) then
            write(iout,1065)
         else
...
c     --- form the j and k matrices ---
      call timing(dum1,dum2,dum3)
      if (dirj.and.(.not.poisj)) then
c        -- direct j-matrix
c           do all the two-electron integrals analytically.
...
c        --- what the hell do we do for open shells?
     ...
     $        rhotest,prtoao,pstart,dpr,method)
c        --- add the difference j-matrix to the last one ---
         call vadd(jmat,jmat,jlast,nnp*(nshell-1))
      else
c        -- form j-matrix from integrals on disk.
