...
c***keywords           m6202, link 6202, spherical harmonics
...
c----------------------------------------------------------------------c
      ...
      prntyu=logkey(ops,'print=m6202=yukawa',.false.,' ')
     ...
     1                 -1,0,0,fillam)
      ...
      defp='off'
      prntnt=logkey(ops,'print=m6202=partial-integrals',.false.,' ')
      ...
      tstint=logkey(ops,'m6202=test-yukawa-integral',.false.,' ')
      defnt='off'
      if (tstint) then
          defnt='on'
      endif    
...
c     call iosys ('read integer "max number of shells" from '//
c    1            'lamdat',1,maxshl,0,' ')
c     if (maxshl.gt.dimshl) then
c         call lnkerr('quit. number of shells too big')
c     endif    
...
c----------------------------------------------------------------------c
     ...
     1                   z(scr),nrtot,lval,mval,nshell,nr,nints,rtyp,
     2                   prntnt)
...
c----------------------------------------------------------------------c
   ...
   40 continue
      ...
      call chainx(0)
      stop
    1 format (//,20x,'***** m6202: solve wave equation *****')
