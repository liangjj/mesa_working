*deck @(#)drvint.f	1.2  10/27/94
c***begin prologue     drvint
c***date written       930922   (yymmdd)
c***revision date      yymmdd   (yymmdd)
c***keywords           drvint, m6203 y(l,m), projections
...
c***end prologue       drvint
      subroutine drvint (flm,psilm,j,y,wt,scr,nrad,l,m,nshell,nr,nints,
     1                   type,prnt)
      implicit integer (a-z)
      real*8 psilm, flm, j, y, wt, scr
      character*(*) type
      character*80 title
      character*3 itoc
      logical prnt
      dimension flm(nrad,*), psilm(nints,*), j(nrad,0:l), y(nrad,0:l)
      dimension wt(*), nr(nshell), scr(*)
      common /io/ inp, iout
      locpf=1
c----------------------------------------------------------------------c
...
c--------------------------------------------------------------------- c     
     ...
     1                      scr,l,mu,nrad,prnt)
            elseif (type.eq.'newton-cotes') then
                call fbnint(flm(1,locpf),psilm(1,locpf),j,y,wt,
