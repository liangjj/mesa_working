...
c***end prologue       plmtof
      subroutine plmtof (f,plm,phifn,ylm,flm,scr,pt,l,m,nr,nth,nph,
     1                   nang,nonsep,prnt)
      implicit integer (a-z)
      real*8 f, plm, phifn, ylm, flm, scr, pt
      logical prnt, nonsep
      character*80 title
      ...
      if (nonsep) then
          ...
          do 10 mu=0,m
             ...
             do 20 count=1,no
                call apbct(f,flm(1,cntlm),ylm(1,cntlm),nr,ndim,nang)
                cntlm=cntlm+ndim
   20        continue
   10     continue
      else
          dimang=nth*nph
          call rzero(f,nr*dimang)
          flmcnt=1
          plmcnt=1
          phicnt=1
          do 30 mu=0,m
   ...
   40        continue
             call apbct(f,scr,phifn(1,phicnt),nr*nth,no,nph)
             plmcnt=plmcnt+ndim
             phicnt=phicnt+no
   30     continue
      endif
      if (prnt) then
          ...
          call prntfm(title,f,nr,dimang,nr,dimang,iout)
      endif
      return
      end
