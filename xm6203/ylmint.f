...
c***end prologue       ylmint
      ...
      if (m.eq.n) then
          ...
          do 10 i=1,nm
             do 20 j=1,i
                ii=ii+1
                call ebtc(ovlp,ylm(1,m,i),ylm(1,m,j),dimm,
     1                    nang,dimm)
                title='normalization of spherical harmonics for '//
     1                'mi = mj = '//itoc(m)//'  matrix = '//itoc(ii)
                call prntfm(title,ovlp,dimm,dimm,dimm,dimm,iout)
   20        continue                
   10     continue
      else
          ii=0
          do 30 i=1,nm
             do 40 j=1,nn
                ii=ii+1
                call ebtc(ovlp,ylm(1,m,i),yln(1,n,j),dimm,
     1                    nang,dimn)
                title='normalization of spherical harmonics for '//
     1                'mi = '//itoc(m)//' mj = '//itoc(n)//
     2                '  matrix = '//itoc(ii)
                call prntfm(title,ovlp,dimm,dimn,dimm,dimn,iout)
   40        continue
   30     continue
      endif                                
      return
